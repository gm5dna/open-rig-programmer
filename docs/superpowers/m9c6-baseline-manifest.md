...
| 7.1 | `490c38c` | the unconditional `TagDisplay{Unknown}` constant restored in `chirp.go` | all three new csvio tests **plus** the app-level `TestImportCHIRP_FTdx10MergedChannelCarriesUnavailableTagDisplay` (app/importexport_test.go). An earlier row here misattributed the app leg to `TestGetUISpec_RegisteredFTdx10_EveryBankUnavailable`, which never runs a CHIRP import and cannot fire on this revert — corrected at the milestone review (Codex finding 2). |
