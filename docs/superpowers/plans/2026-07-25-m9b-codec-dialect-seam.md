...
> **Scoped post-merge, 26/07/2026 (Codex per-commit review).** The Architecture paragraph above, and the Task 54 row in the table below, both say every grammar entry point *and every helper it delegates to* consults the receiver. As executed that holds for every method consulting **dialect data** — mode set, slot space, EX inventory and the bounds derived from it — but **not** for the deferred frame-shape constants: `mtTagMaxBytes` still bounds `BuildMTSet`, `Dialect.ParseMTAnswer` and `validMTCommand` from a package const, and `mtClearTag` is still emitted from one. Those methods take a receiver and read a global. That is a scope boundary rather than the same defect left open, because `Dialect` carries no MT field to derive them from — closing them means *adding* dialect data, which is M9c's work. The statements are left as written because they record what was intended before execution; `.superpowers/sdd/m9b-milestone-summary.md` §2.1/§8.9 is the authoritative account of what was delivered.
