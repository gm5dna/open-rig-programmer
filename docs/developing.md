...
| Path | Contents |
| --- | --- |
| `core/` | The library: CAT codec (`cat`, plus `cat/ftdx10`, `cat/ftdx101`, `cat/ft891`, `cat/ft991a`) for the ASCII-framed Yaesu models, binary-CAT codec (`bincat`) for the FT-890/FT-900/FT-1000MP family's own 5-byte opcode framing, CI-V codec (`civ`, plus `civ/ic7610`, `civ/ic7300`, `civ/ic7300mk2`, `civ/ic705`, `civ/ic9700`, `civ/ic905`, `civ/ic7851`, `civ/ic7760`, `civ/ic7100`, `civ/icr8600`, `civ/ic7800`, `civ/ic7600`, `civ/ic7410`, `civ/ic7700`, `civ/ic9100`) for the Icom models, Kenwood codec (`kw`, plus `kw/ts480`, `kw/ts590`, `kw/ts2000`, and `kw/ma` — one package holding both `MA0` layouts, both hand-written record codecs and its own outbound gate, for the TS-890S and TS-990S) for the Kenwood models, clone-mode transport (`clonewire`) — a SIBLING of `cat`/`civ`/`kw`/`bincat`, not a dialect of any of them: an unprompted whole-image transfer, never a query/reply exchange, for the Yaesu FT-817/FT-817ND/FT-818/FT-857/FT-857D/FT-897/FT-897D clone-mode family (read-only; FT-847 excluded, its own manual disagreeing with itself on the menu number) — capability model (`spec`), codeplug model and diff (`codeplug`), CSV I/O (`csvio`), serial transport (`transport`), radio drivers (`driver/ft710`, `driver/ftdx10`, `driver/ftdx101`, `driver/ft891`, `driver/ft991a`, `driver/ftdx5000` for Yaesu, sharing their write/settings/probe bodies via `driver/internal/yaesu`; `driver/ft890900` (FT-890 and FT-900, one shared 5-byte binary-CAT package) and `driver/ft1000mp` (FT-1000MP/Mark-V, its own 5-byte binary-CAT package) for the two v1.9.0 binary-CAT Yaesu families — `core/bincat` is their shared codec, not `driver/internal/yaesu`; `driver/ic7610`, `driver/ic7300` (IC-7300 and IC-7300MK2), `driver/ic705`, `driver/ic9700`, `driver/ic905`, `driver/ic7851`, `driver/ic7760`, `driver/ic7100`, `driver/icr8600`, `driver/ic7800`, `driver/ic7600`, `driver/ic7410`, `driver/ic7700`, `driver/ic9100`, `driver/ic7200` for Icom; `driver/ts480`, `driver/ts590` (both 590 rows), `driver/ts890`, `driver/ts990` and `driver/ts2000` for Kenwood), and the safe send choreography (`clone`). |
| `cmd/rigprog/` | The CLI. |
| `app/` | Wails v2 + Svelte desktop GUI. |
| `internal/` | The radio simulators — `fakeradio`, `fakedx10`, `fakedx101`, `fakeft891`, `fakeft991a`, `fakeftdx5000` for Yaesu; `fakeic7610`, `fakeic7300`, `fakeic7300mk2`, `fakeic705`, `fakeic9700`, `fakeic905`, `fakeic7851`, `fakeic7760`, `fakeic7100`, `fakeicr8600`, `fakeic7800`, `fakeic7600`, `fakeic7410`, `fakeic7700`, `fakeic9100`, `fakeic7200` for Icom; `fakets480`, `fakets590`, `fakets890`, `fakets990`, `fakets2000` for Kenwood; `fakeyaesuclone` (one fake covering every clone-mode Profile — FT-817/FT-817ND/FT-818/FT-857/FT-857D/FT-897/FT-897D); sharing the one protocol-free plumbing they may all import, `fakepipe` — composition-root wiring, the shared settings store the CLI and GUI both use for unverified-write consent (`userconfig`), menu-table generator (`extable`), and the import-graph guard tests (`guards`). |
