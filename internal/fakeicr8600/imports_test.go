...
// This file is internal/fakeic905's imports_test.go, COPIED — copied because
// the rule it enforces forbids importing anything to share (see doc.go, THE
// HARD RULE, and its "a sibling, not a refactor" section), and because that
// copy is itself internal/fakedx101's copy of internal/fakedx10's copy of
// internal/fakeradio's, extended: fakeradio's version is NON-RECURSIVE, using
// parser.ParseDir("."), which reads one directory and stops.
//
// The recursive version is the one copied here, and it is copied BEFORE this
// package has any subdirectory at all, deliberately — the order every earlier
// package took. A fence that arrives after the directory it needs to cover is
// a fence somebody has already walked round; a fence that arrives first is one
// the next directory lands inside. TestScanForbiddenImports_CatchesAForbidden-
// ImportInASubdirectory below proves it bites there against a synthetic tree,
// so the proof does not wait on a directory that may never exist.
//
// Nothing about the copy is edited except the package clause and the prose: the
// scan, the vacuity guards and the red proof are the ones already standing over
// four sibling fakes.
...
// fakeicr8600 must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/icr8600 — the dialect this fake speaks and must not consult", "github.com/gm5dna/open-rig-programmer/core/civ/icr8600", true},
		...
		{"core/driver/icr8600 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/icr8600", true},
		{"internal/fakeradio — a sibling fake, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakeradio", true},
		{"internal/fakeic905 — this package's structural exemplar, equally forbidden", "github.com/gm5dna/open-rig-programmer/internal/fakeic905", true},
		{"internal/extable", "github.com/gm5dna/open-rig-programmer/internal/extable", true},
		{"fakeicr8600 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeicr8600", true},
		{"stdlib", "io", false},
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeicr8600 must not
// import anything project-internal — not core/civ, not core/civ/icr8600, not
// core/codeplug, not core/spec, not internal/fakeradio, not internal/fakeic905
// and not internal/fakeic7610 — so that a systematic bug in the production codec
// cannot pass end-to-end tests invisibly. It covers this directory AND EVERY
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeicr8600 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// a generator would take), one violating _test.go beside it, and one violating
// file under testdata. Only the subdirectory's non-test file may be reported.
...
// placed in a subdirectory, and that it bites there and nowhere else — the
// _test.go file beside it and the testdata fixture are both skipped by design.
//
// It is the whole reason this file lands with the package core rather than
// after it: the fence is proven effective against a subdirectory that does not
// exist yet, so whatever arrives there later arrives inside it. The fixture
// tree is synthetic and independent of any real subdirectory, which is what
// keeps this a check on the SCAN rather than on whatever some generator happens
// to import today.
