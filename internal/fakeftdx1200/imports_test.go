...
// doc.go, THE HARD RULE), and because that package's version is already the
...
// which fakeftdx1200 must never have, in this directory or any beneath it
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/cat/ftdx1200 — the dialect this fake speaks and must not consult", "github.com/gm5dna/open-rig-programmer/core/cat/ftdx1200", true},
		...
		{"core/driver/ftdx1200 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ftdx1200", true},
		...
		{"fakeftdx1200 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeftdx1200", true},
...
// scanResult is what one scan of a directory tree observed.
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeftdx1200 must
// not import anything project-internal — not core/cat, not
// core/cat/ftdx1200, not core/codeplug, not core/spec, and not any sibling
// fake — so that a systematic bug in the production codec cannot pass
// end-to-end tests invisibly. It covers this directory AND EVERY DIRECTORY
// BENEATH IT.
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeftdx1200 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// miss rather than asserting it in a comment: ParseDir over the same tree's
// root reports no violation at all, because it never descends into gen/.
