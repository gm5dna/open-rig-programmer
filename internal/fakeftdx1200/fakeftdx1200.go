...
type Radio struct {
	...
	catID string
	// ai is P1 of the AI command, '0' or '1'. Started at '0' — see doc.go's
	// register entry MC (CURRENT CHANNEL SELECTION) IS NOT
	// IMPLEMENTED (AI is modelled for core/transport.Engine.Init's
	// unconditional "AI0;", not because the matrix documents this radio's
	// own AI legend).
	ai byte
}

// New constructs a *Radio and starts its servicing goroutine. Without a
// WithFactoryImage option the slot map defaults to DefaultImage(); without
// WithFFT1NotFitted this Radio answers ID as "0582" (doc.go).
...
// that ever reads or writes the pipe, so no synchronisation is needed
// around the connection itself — only around the shared state behind
