...
func TestPROVENANCECarriesA22Verbatim(t *testing.T) {
	...
	// A1 is deliberately NOT among them: its pad/trim rule is TS-990S ONLY,
	// and this row carries P13 verbatim. Nor are A9 and A10: no image is
	// shipped for 100-119, so nothing here rides on what either class holds.
	//
	...
	raw := string(b)
	for _, entry := range []string{"A2", "A4", "A5", "A6", "A16", "A17", "A21", "A22"} {
	...
	for _, entry := range []string{"A1", "A9", "A10"} {
		if strings.Contains(raw, "- **"+entry+"**") {
...
// not a move, and what to do if the cross-check ever fires.
func TestPROVENANCERecordsTheTranscriptionBCopy(t *testing.T) {
	...
	for _, want := range []string{
		"core/kw/ma/testdata/transcription-b-890s.csv",
		"f45226d2bce269c76b966de596909b3783757477a5b09a66c3a955d9e6d12560",
		"890:2273-2280",
		"UNVERIFIED",
	} {
