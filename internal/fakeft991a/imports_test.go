...
// cover. internal/fakeft991a/gen — the stdlib-only generator for this radio's
// transcription B — arrives with the EX task of this milestone's plan, and it
// is the piece most likely to reach for internal/extable, the A-side machinery
// whose Digits parsing was a known defect locus, which is exactly the import
// this package must not have: one parser on both sides of the EX cross-check
// would reproduce a shared parsing bug into both inventories invisibly.
// TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory below proves
// the walk WOULD bite there before the directory exists, which is the whole
...
// fakeft991a must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/cat/ft991a — the dialect this fake speaks and must not consult", "github.com/gm5dna/open-rig-programmer/core/cat/ft991a", true},
		...
		{"core/driver/ft991a — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ft991a", true},
		...
		{"fakeft991a itself", "github.com/gm5dna/open-rig-programmer/internal/fakeft991a", true},
		{"stdlib", "io", false},
...
// are what will let a later test assert WHICH files were reached, once gen/
// exists.
...
//     internal, but core/driver/ft991a's tests will import both this package
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeft991a must not
// import anything project-internal — not core/cat, not core/cat/ft991a, not
// core/codeplug, not core/spec, and not any sibling fake — so that a systematic
// bug in the production codec cannot pass end-to-end tests invisibly. It covers
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeft991a MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
//
// It is the test that makes the fence meaningful BEFORE gen/ exists.
