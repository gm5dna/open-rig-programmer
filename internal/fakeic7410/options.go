...
func defaultConfig() config {
	return config{addr: radioAddrDefault, model: "IC-7410", recordLen: RecordLen, channels: make(map[int][]byte)}
}

// WithModelName sets the bytes a 19 00 request is answered with. The
// reply value is UNDOCUMENTED (matrix §3.12); "IC-7410" is INVENTED, not
// read off the manual.
...
// via the front-panel DIAL, 01h-DFh.
func WithRadioAddress(addr byte) Option {
	return func(c *config) {
		if addr == 0xfe || addr == 0xfd {
			panic("fakeic7410: framing byte cannot be a radio address")
		}
		c.addr = addr
	}
}

// WithRecordLength overrides RecordLen (40, the matrix's corrected
// figure) — see doc.go, "Record length, and proving an alternative".
func WithRecordLength(n int) Option {
	return func(c *config) {
		if n < 1 {
			panic("fakeic7410: record length must be positive")
		}
		c.recordLen = n
	}
}
...
// tier-wide convention fakeic7610, fakeic7851 and fakeic7200 all carry.
func WithAddressedFlood(d time.Duration) Option { return func(c *config) { c.addressed = d } }

// WithChannel seeds one channel at construction. addr is "001".."099",
// "P1" or "P2".
func WithChannel(addr string, record []byte) Option {
	return func(c *config) {
		ch, ok := parseChannel(addr)
		if !ok {
			panic(fmt.Sprintf("fakeic7410: invalid channel %q", addr))
		}
		if len(record) != c.recordLen {
			panic(fmt.Sprintf("fakeic7410: channel %q record has %d bytes, want %d", addr, len(record), c.recordLen))
		}
...
// record. addr is a three-digit memory channel "001".."099", or "P1"/"P2"
// for the two scan edges (matrix §1 row 5, §1b banks).
