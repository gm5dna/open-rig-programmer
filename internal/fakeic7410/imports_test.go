...
// This file is internal/fakeic7851's imports_test.go, COPIED — the same
// convention that file itself documents: a package's TestNoCoreImports
// fence is standing infrastructure, carried sibling to sibling,
// mechanism-for-mechanism, with only the package clause and the prose
// updated. Nothing about the scan, the vacuity guards or the two red
// proofs is edited.
...
// which fakeic7410 must never have, in this directory or any beneath it
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/ic7410 — the dialect this fake is the other witness to", "github.com/gm5dna/open-rig-programmer/core/civ/ic7410", true},
		{"core/driver — the layer under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver", true},
		{"core/driver/ic7410", "github.com/gm5dna/open-rig-programmer/core/driver/ic7410", true},
		...
		{"internal/fakeic7200 — this wave's own precedent, equally forbidden", "github.com/gm5dna/open-rig-programmer/internal/fakeic7200", true},
		{"fakeic7410 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeic7410", true},
		...
		{"clone dir name is not the module path either", "codex-icom-ic7410/core/civ", false},
...
//     a false positive on a fixture. Nothing in this package puts Go
//     source there today.
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeic7410 must
// not import anything project-internal — not core/civ, not
// core/civ/ic7410, not core/driver, not core/codeplug, not core/spec and
// not any sibling fake — so that a systematic misreading of the IC-7410
// memory record cannot agree with itself on both sides of a test and
// pass invisibly. It covers this directory AND EVERY DIRECTORY BENEATH
// IT.
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic7410 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// — the shape a later generator would take), one violating _test.go
// beside it, and one violating file under testdata. Only the
// subdirectory's non-test file may be reported.
func fenceTestTree(t *testing.T) string {
	t.Helper()
	return writeTree(t, map[string]string{
		"clean.go":           "package p\n\nimport \"io\"\n\nvar _ io.Reader\n",
		"gen/main.go":        "package main\n\nimport _ \"github.com/gm5dna/open-rig-programmer/core/civ/ic7410\"\n",
...
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/civ/ic7410"; got.path != want {
...
// this file diverges from internal/fakeradio's non-recursive
// parser.ParseDir version, by demonstrating the miss rather than
// asserting it in a comment: parser.ParseDir over the same tree's root
