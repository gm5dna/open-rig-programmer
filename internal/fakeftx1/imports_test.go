...
// This file is internal/fakeftdx3000's imports_test.go, COPIED — copied
// because the rule it enforces forbids importing anything to share (see
// doc.go, THE HARD RULE), and because that package's version is the
// RECURSIVE one: it walks the tree rather than calling
// parser.ParseDir("."), which reads one directory and stops. THE SCAN IS
// RECURSIVE so that if this package ever grows a subdirectory, the next
// import lands inside the fence rather than in front of it.
...
// fakepipeImport is the ONE project-internal import this package may
// have — internal/fakepipe is PROTOCOL-FREE plumbing (doc.go).
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/cat/ftx1 — a future dialect this fake must not consult", "github.com/gm5dna/open-rig-programmer/core/cat/ftx1", true},
		...
		{"core/driver/ftx1 — the driver being written concurrently, elsewhere", "github.com/gm5dna/open-rig-programmer/core/driver/ftx1", true},
		{"internal/fakeftdx3000 — the sibling fake this package is shaped after, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakeftdx3000", true},
		{"internal/fakeradio — the FT-710's", "github.com/gm5dna/open-rig-programmer/internal/fakeradio", true},
		{"fakeftx1 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeftx1", true},
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeftx1 must
// not import anything project-internal — not core/cat, not
// core/cat/ftx1, not core/driver/ftx1, not core/codeplug, not core/spec,
// and not any sibling fake — so that a systematic bug in a future
// production codec cannot pass end-to-end tests invisibly. It covers
// this directory AND EVERY DIRECTORY BENEATH IT.
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeftx1 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
