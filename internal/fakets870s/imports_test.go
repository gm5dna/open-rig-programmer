...
// fakets870s must never have, in this directory or any beneath it (see
...
// nothing else: not a framing byte, not a field layout, not a reply. It is
// the single share Stuart permitted on 06/09/2026, and it is safe precisely
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/kw/ts870s — the layout value and the codec this fake must never import", "github.com/gm5dna/open-rig-programmer/core/kw/ts870s", true},
		...
		{"core/driver/ts870s — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ts870s", true},
		{"internal/fakeft891 — a sibling fake, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakeft891", true},
		{"internal/fakets480 — the other 22-byte-adjacent Kenwood fake, whose legends differ", "github.com/gm5dna/open-rig-programmer/internal/fakets480", true},
		{"internal/fakets590 — the other Kenwood fake, whose legends differ", "github.com/gm5dna/open-rig-programmer/internal/fakets590", true},
		{"fakets870s itself", "github.com/gm5dna/open-rig-programmer/internal/fakets870s", true},
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakets870s must not
// import anything project-internal — not core/kw, not core/kw/ts870s, not
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakets870s MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// in the root, one VIOLATING file in a subdirectory (gen/, by name), one
// violating _test.go beside it, and one violating file under testdata. Only
// the subdirectory's non-test file may be reported.
...
// _test.go file beside it and the testdata fixture are both skipped by
// design.
...
// file uses a walk rather than parser.ParseDir, by demonstrating the miss
...
func TestScanForbiddenImports_IsRecursiveWhereParseDirIsNot(t *testing.T) {
	...
	if missed != 0 {
		t.Fatalf("parser.ParseDir found %d forbidden imports in the tree's root — it appears to recurse now, so this package's walking scan is no longer needed", missed)
	}
