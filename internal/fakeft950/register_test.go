...
// for readability only: entries are cited BY NAME, never by position.
var registerEntries = []string{
	"EMPTY-SLOT AND OUT-OF-DOMAIN ANSWERS",
	`THE "NO SELECTION YET" CHANNEL IS "118"`,
...
// to one space, so that a register name wrapped across two comment lines is
...
// inline comment beside the code that implements it. A register nobody
// checks drifts from the code it describes within a milestone.
