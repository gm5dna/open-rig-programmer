...
func TestNew_DefaultsToFT950(t *testing.T) {
	r := New()
	defer r.Close()
...
func TestPort_RoundTrip(t *testing.T) {
	...
	if got, want := buf[:n], []byte("ID0310;"); !bytes.Equal(got, want) {
...
func TestWithSlot_Overlay(t *testing.T) {
	...
	if _, ok := r.SlotState("000"); !ok {
		t.Error("WithSlot dropped the default image's channel 000")
	}
}
