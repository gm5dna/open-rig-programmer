...
// This file is fakeft950's own, independent byte-level CAT parser and reply
// builder. It is derived from the Yaesu FT-950 CAT Operation Reference Book
// (doc code EC030H120) and from docs/superpowers/ft950-capability-matrix.md —
// NOT from core/cat or core/driver/ft950 (doc.go, THE HARD RULE). The manual
// is gitignored (docs/fixtures-private/manuals/), so "layout:N" citations
// name where the chart is, not a link.
...
// "P1 000 - 117: Memory Channel Number / 000 - 099: Regular Memory Channel /
// 100: P1L 101: P1U ~ 116: P9L 117: P9U" (layout:795-802). MW and MR draw the
// same span implicitly ("P1 Memory Channel Number", layout:844, :876) — one
...
// §1.5).
//
// THIS RADIO'S OWN DELTA: memoryLo is 0 (000), not every sibling's 1 (001) —
// doc.go, matrix §1.5.
...
// entry 2) — never a valid REQUEST slot, since it falls outside 000-117. It
// CANNOT be "000" the way fakeft2000's none-form is: 000 is a real, populated
// channel on this radio (memoryLo is 0), so the lowest available numeral
// outside the span sits immediately ABOVE it instead.
const slotNoneWire = "118"
...
// "Clarifier Direction +: Plus Shift, --: Minus Shift" (layout:876-877).
...
// CTCSS OFF 1: CTCSS ENC/DEC 2: CTCSS ENC" (layout:857/898) — the ordinary
// legacy domain, no DCS member (matrix §4).
...
// IS STORED AT ITS PRINTED WIDTH, NOT RANGE-CHECKED: CN's separate 00-49
...
// Simplex 1: Plus Shift 2: Minus Shift" (layout:857/898).
...
// (docs/superpowers/ft950-capability-matrix.md §2, layout:844-862 and
// :876-899). Positions are 0-indexed offsets INTO THE BLOCK — i.e. into a
...
// counted 27-byte MW/MR frame (matrix §2).
...
// mwSetKindFixed is MW's own Set-direction P7, "P7 0: (Fixed)" (layout:899):
...
// MR's Read legend's two members, "0: VFO 1: Memory" (layout:857), a channel
...
// --- MW: MEMORY CHANNEL WRITE (availability "O X X X", layout:130; block
// layout:876-899) ---
//
...
// --- MR: MEMORY CHANNEL READ (availability "X O O X", layout:177; block
// layout:844-862) ---
//
...
// --- MC: MEMORY CHANNEL (availability "O O O X", layout:172; block
// layout:795-802) ---
//
...
// --- ID (availability "X O O X", layout:161; block layout:676-682) ---
//
// No Set. Read "ID;", a 7-byte Answer: "ID" + the 4-digit CATID + ';'. The
// VALUE is this radio's own fixed constant: "P1 0310 (Fixed value)"
// (layout:676-682) — unlike fakeft2000's per-row map, there is only ever one
// row here (doc.go).

func buildIDAnswer() []byte { return []byte("ID0310;") }
