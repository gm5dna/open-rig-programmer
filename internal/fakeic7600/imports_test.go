...
// This file is internal/fakeic7851's imports_test.go, copied per the brief
// ("imports_test.go as in fakeic7851") — copied because the rule it
// enforces forbids importing anything to share (see doc.go, THE
// QUARANTINE). Only the package name and the forbidden-import examples
// differ: the examples name THIS radio's codec and driver, which are the
// two packages this one must never see.
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/ic7600 — the codec this fake must not consult", "github.com/gm5dna/open-rig-programmer/core/civ/ic7600", true},
		{"core/driver — the layer under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver", true},
		{"core/driver/ic7600 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ic7600", true},
		...
		{"internal/fakeic7610 — a template, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakeic7610", true},
		...
		{"internal/fakeic7800 — a sibling built to the same driver shape, equally forbidden", "github.com/gm5dna/open-rig-programmer/internal/fakeic7800", true},
		{"internal/fakeradio", "github.com/gm5dna/open-rig-programmer/internal/fakeradio", true},
		{"fakeic7600 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeic7600", true},
		...
		{"worktree dir name is not the module path either", "ft710-wt/core/civ", false},
...
// TestNoCoreImports enforces THE QUARANTINE (see doc.go): fakeic7600 must
// not import anything project-internal — not core/civ, not
// core/civ/ic7600, not core/driver/ic7600, not any sibling fake — so that a
// systematic misreading of the IC-7600 memory record cannot agree with
// itself on both sides of a test and pass invisibly. Covers this directory
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic7600 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE QUARANTINE)", v.file, v.path)
	}
...
func fenceTestTree(t *testing.T) string {
	...
	return writeTree(t, map[string]string{
		"clean.go":           "package p\n\nimport \"io\"\n\nvar _ io.Reader\n",
		"gen/main.go":        "package main\n\nimport _ \"github.com/gm5dna/open-rig-programmer/core/civ/ic7600\"\n",
...
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/civ/ic7600"; got.path != want {
...
// this file walks rather than calling parser.ParseDir.
