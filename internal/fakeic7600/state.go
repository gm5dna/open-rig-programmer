...
// The CI-V addresses this package knows. MANUAL-EVIDENCED, matrix §3.4: 0x7A
// is the IC-7600's default address, 0xE0 the controller's.
//
// AddrBroadcast (0x00) is ASSUMED, not evidenced — matrix §3.5(b). See
// doc.go, "Two floods".
const (
	AddrRadio      byte = 0x7A
...
// CodeOK and CodeNG are the matrix's own fixed reply codes (PDF p.168 folio
// 159, "OK message"/"NG message to controller"). MANUAL-EVIDENCED.
const (
	CodeOK byte = 0xFB
	CodeNG byte = 0xFA
)
...
// a name shorter than NameLen: 0x20 (ASSUMED, matrix §3.9(iv)). NOTHING IN
// THIS PACKAGE WRITES EITHER — see doc.go.
const (
	NameLen      = 10
	NamePad byte = 0x20
)
...
// "Record length", on the byte-③ undivided-enum reading and the byte-⑪
// data_mode/tone_mode nibble assignment this radio carries, neither of
// which this package ever looks at.
...
// matching core/civ/ic7600/testdata's own transcription encoding label
// ("bcd_packed") for field "①,②". See doc.go, "Channel selectors".
...
func (r *Radio) SetSlot(ch int, m MemState) {
	if _, _, ok := selectorFor(ch); !ok {
		panic(fmt.Sprintf("fakeic7600: channel %d is not addressable — the matrix gives memory channels 1..99 (selectors 00 01 .. 00 99) and the two scan edges ChanP1 (01 00) and ChanP2 (01 01), and nothing else", ch))
	}
	if len(m.Raw) != r.recordLen {
		panic(fmt.Sprintf("fakeic7600: record for channel %d is %d bytes, want %d — one record has one accepted length (see doc.go, \"Record length\"; WithRecordLength changes it)", ch, len(m.Raw), r.recordLen))
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	r.slots[ch] = m.clone()
