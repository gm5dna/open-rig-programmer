...
// ic7600-capability-matrix.md alone — see doc.go, THE QUARANTINE.
...
type frame struct {
	raw  []byte // as received, preamble and all
	to   byte
	from byte
...
// answerFrame builds an answer-direction frame: FE FE E0 7A <payload...> FD.
...
// FE FE <to> 7A 19 00 <id token...> FD — the ID answer with `to` swapped.
...
// silence.
//
// The ADDRESS FILTER happens first: a frame addressed elsewhere gets no
// answer, no state change, no CommandLog entry. See doc.go, "Framing".
