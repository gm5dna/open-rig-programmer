...
// This file is internal/fakeic9700's imports_test.go, COPIED IN MECHANISM —
// copied rather than imported, because the rule it enforces forbids importing
// anything to share it with (see doc.go, THE HARD RULE). Only the mechanism was
// taken: the module prefix, the predicate, the recursive walking scan and the
// four tests below. Nothing about what fakeic9700 knows, answers or contains
// was read across, and this package's knowledge of the IC-7100 record came from
// the two quarantined artefacts PROVENANCE.md names and from nothing else.
//
// The scan WALKS, and that is the point rather than a stylistic preference:
// internal/fakeradio's ancestor of this file is NON-RECURSIVE — it uses
// parser.ParseDir("."), which reads one directory and stops — so a forbidden
// import placed one directory down would sail past it.
//
// That hole would be a real one here. This fake exists to be the OTHER witness
// to what an IC-7100 does with a 1A 00 frame. If it could import
// core/civ/ic7100, core/driver/ic7100, that profile's golden vectors or its
// field ledger — from this directory or any subdirectory anyone later adds —
// then the fake and the code under test would share one reading of the record,
// a systematic misreading would agree with itself end to end, and every test
// would go green while proving nothing. The fence is what makes that
// independence mechanical rather than a matter of good intentions, and it lands
// with the package core, before any subdirectory exists, so that anything added
// later arrives inside a fence rather than in front of one.
...
// fakeic7100 must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/ic7100 — the dialect this fake is the other witness to", "github.com/gm5dna/open-rig-programmer/core/civ/ic7100", true},
		{"core/driver — the layer under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver", true},
		{"core/driver/ic7100", "github.com/gm5dna/open-rig-programmer/core/driver/ic7100", true},
		...
		{"internal/fakeic9700 — this package's structural exemplar, equally forbidden", "github.com/gm5dna/open-rig-programmer/internal/fakeic9700", true},
		{"internal/fakeic905", "github.com/gm5dna/open-rig-programmer/internal/fakeic905", true},
		{"fakeic7100 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeic7100", true},
		{"stdlib", "io", false},
		...
		{"clone dir name is not the module path either", "codex-icom-ic7100/core/civ", false},
...
// is inside a subdirectory, which is the property the ParseDir version cannot
// have.
//
...
//     may import whatever they need (this package's tests import nothing
//     project-internal, but the driver tests that drive this fake will import
//     both it and half of core/, and that is correct).
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeic7100 must not
// import anything project-internal — not core/civ, not core/civ/ic7100, not
// core/driver, not core/codeplug, not core/spec, not internal/fakeradio and not
// any sibling fake — so that a systematic misreading of the IC-7100 memory
// record cannot agree with itself on both sides of a test and pass invisibly.
// It covers this directory AND EVERY DIRECTORY BENEATH IT.
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic7100 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
func fenceTestTree(t *testing.T) string {
	...
	return writeTree(t, map[string]string{
		"clean.go":           "package p\n\nimport \"io\"\n\nvar _ io.Reader\n",
		"gen/main.go":        "package main\n\nimport _ \"github.com/gm5dna/open-rig-programmer/core/civ/ic7100\"\n",
...
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/civ/ic7100"; got.path != want {
...
// file's scan walks, by demonstrating the miss rather than asserting it in a
// comment: parser.ParseDir over the same tree's root reports no violation at
// all, because it never descends into gen/.
//
// If this test ever fails because ParseDir started recursing, the divergence is
// no longer needed, which is worth knowing, and is why the assertion is stated
// in that direction.
func TestScanForbiddenImports_IsRecursiveWhereParseDirIsNot(t *testing.T) {
	...
	if missed != 0 {
		t.Fatalf("parser.ParseDir found %d forbidden imports in the tree's root — it appears to recurse now, so this package's walking scan is no longer the reason the ParseDir version had to be extended", missed)
	}
