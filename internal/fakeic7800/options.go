...
// (§3.12(i), register 7800-R7). 0x5A was chosen because it is obviously
// synthetic — an alternating bit pattern, and not one of the addresses or
// codes this package names (0x6A, 0xE0, 0x00, 0xFB, 0xFA, 0xFD, 0xFE). A
// plausible-looking default would be worse than an implausible one: a
// consumer whose ID probe happened to expect the right value would pass
// against a fake that guessed, and nobody would learn the guess was a
// guess.
var defaultIDToken = []byte{0x5A}

// defaultQueuedFrames bounds the radio's output queue.
...
// before any answer to it, running BEFORE the address filter.
//
// The IC-7800's document names NO echo setting at all (matrix §3.6,
// MANUAL-EVIDENCED absence — no USB CI-V port exists on this model). This
// option therefore models no named menu item; see doc.go, "Echo", and
// register FAKE-4.
...
// every `every`, addressed to 0x00 — ASSUMED, matrix §3.5(b), register
// 7800-R9. A non-positive interval starts nothing.
...
func WithRecordLength(n int) Option {
	return func(c *config) {
		if n < 1 {
			panic("fakeic7800: WithRecordLength needs at least one byte — a zero-length record would make a set indistinguishable from a read")
		}
		c.recordLen = n
	}
}
...
// (NG) as ASSUMED from a single capture (register 7800-R2a); §3.8(b) grades
// the alternative — that a record which reads back as all-FF means "empty"
// — as equally undocumented ("unresolved either way", register 7800-R2b).
// This package does not pick a winner; it offers both. See doc.go, register
// FAKE-2.
...
// (register 7800-R15: "the document... never says whether a short record is
// accepted, padded or rejected"); it names no pad convention for the
// accepted case, because the matrix's own default reading is that nothing
// but a full record is accepted at all. The zero-tail padding here is this
// package's own choice — doc.go, register FAKE-3 — not a citation. A set
// LONGER than RecordLen is always refused, in both modes.
...
// echoes, not flood frames. No IC-7800 timing has ever been observed by
// this project (matrix §0), so there is no default and nothing here models
// a real delay. The wait is interruptible: Close never has to wait one out.
