...
// internal/fakeic7610 and internal/fakeic7851 both follow, so a bug shared
// between the fake's protocol code and its own tests cannot hide behind
...
// TestGoldenVectorsReadAndSet replays two of core/civ/ic7800/testdata/
// ic7800-vectors.golden's frames — the two this package's protocol covers —
// byte for byte, so this independently-authored fake is checked against the
// same frozen evidence the production codec's own tests are. The golden
// file's other two vectors (read-transceiver-id and manual-example-14) are
// exercised in TestTransceiverIDCommandIsRecognised and
// TestPreamblePaddingIsTolerated below; their reply CONTENT for 19 00 is not
// checked here since this package invents its own ID token (doc.go).
func TestGoldenVectorsReadAndSet(t *testing.T) {
	...
	set := append([]byte{0x00, 0x01,
		0x00,                         // select memory setting: OFF
		0x00, 0x00, 0x25, 0x14, 0x00, // frequency, 5 bytes
		0x01, 0x01, // mode USB, filter FIL1
		0x10,             // tone-type TONE(hi) / data-mode OFF(lo)
		0x00, 0x08, 0x85, // tone_tx 88.5 Hz
		0x00, 0x10, 0x00, // tone_rx 100.0 Hz
	}, name...)
	if len(set) != 2+RecordLen {
		t.Fatalf("built golden set payload is %d bytes, want %d", len(set), 2+RecordLen)
	}

	setFrame := mkFrame(AddrRadio, AddrController, append([]byte{cnMemory, scMemory}, set...)...)
	wantSetFrame := []byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00,
		0x00, 0x01, 0x00, 0x00, 0x00, 0x25, 0x14, 0x00, 0x01, 0x01, 0x10, 0x00, 0x08, 0x85,
		0x00, 0x10, 0x00, 0x48, 0x4F, 0x4D, 0x45, 0x20, 0x51, 0x54, 0x48, 0x30, 0x31, 0xFD}
	if !bytes.Equal(setFrame, wantSetFrame) {
		t.Fatalf("built set frame = % X, want the golden vector % X (core/civ/ic7800/testdata/ic7800-vectors.golden, set-record-name-with-space)", setFrame, wantSetFrame)
	}
	...
	wantReadFrame := []byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00, 0x00, 0x01, 0xFD}
	...
	want := answer(append([]byte{cnMemory, scMemory, 0x00, 0x01}, set[2:]...)...)
...
func TestPreamblePaddingIsTolerated(t *testing.T) {
	r := New()
	defer r.Close()
	padded := []byte{0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0x6A, 0xE0, 0x18, 0x01, 0xFD}
...
func TestEmptyChannelReplyModes(t *testing.T) {
	...
	// than the single capture register 7800-R2a names; see doc.go.
	r := New()
	defer r.Close()
...
// §3.16(f); doc.go, "What this radio answers, and what it refuses".
...
// TestUSBEchoRunsBeforeTheAddressFilter pins doc.go's "Echo" ordering: a
// frame addressed to another radio is echoed verbatim and still not
// answered.
func TestUSBEchoRunsBeforeTheAddressFilter(t *testing.T) {
	r := New(WithUSBEcho())
	defer r.Close()
	own := mkFrame(AddrRadio, AddrController, cnID, scID)
	c := r.Port()
	if got := exchange(t, c, own); !bytes.Equal(got, own) {
		t.Fatalf("echo of own-addressed frame = % X, want % X", got, own)
	}
	// own also carried an ID answer, queued right behind the echo — drain
	// it before the next exchange, or it leaks into the read below.
	drainBuf := make([]byte, 4096)
	_ = c.SetDeadline(time.Now().Add(time.Second))
	if _, err := c.Read(drainBuf); err != nil {
		t.Fatalf("draining the ID answer that follows the echo: %v", err)
	}

	elsewhere := mkFrame(0x00, AddrController, cnID, scID)
	_ = c.SetDeadline(time.Now().Add(time.Second))
	if _, err := c.Write(elsewhere); err != nil {
		t.Fatal(err)
	}
	b := make([]byte, 4096)
	n, err := c.Read(b)
	if err != nil {
		t.Fatalf("no echo arrived: %v", err)
	}
	if got := b[:n]; !bytes.Equal(got, elsewhere) {
		t.Fatalf("echo of elsewhere-addressed frame = % X, want % X", got, elsewhere)
	}
	// And nothing further follows — no answer chases the echo.
	_ = c.SetDeadline(time.Now().Add(100 * time.Millisecond))
	if n2, err2 := c.Read(b); err2 == nil {
		t.Fatalf("an answer followed the echo: % X", b[:n2])
	}
}
