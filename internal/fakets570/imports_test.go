...
// This file is internal/fakets480's imports_test.go, COPIED — copied because
// the rule it enforces forbids importing anything to share (see doc.go, THE
// HARD RULE). THE SCAN IS RECURSIVE: it walks the whole directory tree rather
// than calling parser.ParseDir("."), which reads one directory and stops, so
// that any future subdirectory of this package lands inside the fence rather
// than in front of it. TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory
// is the fence's own red proof, run green, against a temporary tree of its
// own making.
...
// fakets570 must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/kw/ts570 — this row's own layout value", "github.com/gm5dna/open-rig-programmer/core/kw/ts570", true},
		...
		{"core/driver/ts570 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ts570", true},
		{"internal/fakets480 — the other Kenwood fake this package is shaped after, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakets480", true},
		...
		{"fakets570 itself", "github.com/gm5dna/open-rig-programmer/internal/fakets570", true},
...
//     internal, but core/driver/ts570's tests will import both this package
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakets570 must not
// import anything project-internal — not core/kw, not core/kw/ts570, not
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakets570 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/kw"; got.path != want {
...
// file diverges from a bare parser.ParseDir version, by demonstrating the miss
