...
func TestOutOfRangeChannelIsSilence(t *testing.T) {
	...
	// CH=0x65 (101) is one past FT-900's 100-channel range (matrix §1.9).
	if _, err := port.Write(frame(opStatusUpdate, [4]byte{uMemoryRecord, 0, 0, 0x65})); err != nil {
		t.Fatalf("Write: %v", err)
	}
...
func TestChannel33AnswersUnlikeFT890(t *testing.T) {
	// The FT-900's own §1.8 identity probe is REVERSED from the FT-890's:
	// CH=0x21 (33) is IN range here (1..100), so it must answer — this is
	// the exact distinction a real driver's identity probe leans on to
	// tell the two radios apart.
	r := New()
	defer r.Close()
	port := r.Port()
...
func TestStoreOutOfRangeChannelDoesNothing(t *testing.T) {
	...
	if _, err := port.Write(frame(opStore, [4]byte{0x65, 0, 0, 0})); err != nil {
		t.Fatalf("Write: %v", err)
	}
