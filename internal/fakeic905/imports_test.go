...
// This file is internal/fakedx101's imports_test.go, COPIED — copied because
...
// copy is itself internal/fakedx10's copy of internal/fakeradio's, extended:
// fakeradio's version is NON-RECURSIVE, using parser.ParseDir("."), which reads
// one directory and stops.
//
// The recursive version is the one copied here, and it is copied BEFORE this
// package has any subdirectory at all, deliberately — the order both earlier
// packages took. A fence that arrives after the directory it needs to cover is
...
// fakeic905 must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/ic905 — the dialect this fake speaks and must not consult", "github.com/gm5dna/open-rig-programmer/core/civ/ic905", true},
		...
		{"core/driver/ic905 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ic905", true},
		...
		{"fakeic905 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeic905", true},
		{"stdlib", "io", false},
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeic905 must not
// import anything project-internal — not core/civ, not core/civ/ic905, not
// core/codeplug, not core/spec, not internal/fakeradio, not internal/fakedx10
// and not internal/fakedx101 — so that a systematic bug in the production codec
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic905 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
