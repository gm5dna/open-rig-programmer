...
// shared, because the rule it enforces forbids importing anything to share
// (see doc.go, THE HARD RULE). Only the package name and the forbidden-import
// examples differ: the examples name THIS radio's codec and driver, which are
// the two packages this one must never see.
//
// It is a RECURSIVE walk (filepath.WalkDir), not parser.ParseDir("."), which
// reads one directory and stops. That difference is the whole point of copying
// the fakedx101 version rather than internal/fakeradio's: a subdirectory added
// to this package later — a generator, a fixture builder, anything — arrives
// INSIDE the fence rather than in front of one.
//
// THIS FILE WAS WRITTEN FIRST, BEFORE ANY PROTOCOL CODE IN THIS PACKAGE, and
// before any subdirectory of it exists.
// TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory proves the
// scan would bite a violation in a subdirectory that is not there yet.
...
// fakeic7610 must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ — the CI-V codec this fake must not consult", "github.com/gm5dna/open-rig-programmer/core/civ", true},
		{"core/civ/ic7610 — this radio's own codec, the one whose bugs this fake exists to expose", "github.com/gm5dna/open-rig-programmer/core/civ/ic7610", true},
		{"core/driver/ic7610 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ic7610", true},
		...
		{"internal/fakedx10 — another sibling fake", "github.com/gm5dna/open-rig-programmer/internal/fakedx10", true},
		{"fakeic7610 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeic7610", true},
		{"stdlib", "io", false},
...
// is inside a subdirectory, which is the property a parser.ParseDir version
// cannot have.
//
...
//     may import whatever they need. (This package's own tests import nothing
//     project-internal either, but that is a separate discipline — the fence
//     does not enforce it.)
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeic7610 must not
// import anything project-internal — not core/civ, not core/civ/ic7610, not
// core/driver/ic7610, not internal/fakedx101, not internal/fakeradio — so that
// a systematic bug in the production codec cannot pass end-to-end tests
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic7610 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// a generator subdirectory would take), one violating _test.go beside it, and
...
func fenceTestTree(t *testing.T) string {
	...
	return writeTree(t, map[string]string{
		"clean.go":           "package p\n\nimport \"io\"\n\nvar _ io.Reader\n",
		"gen/main.go":        "package main\n\nimport _ \"github.com/gm5dna/open-rig-programmer/core/civ/ic7610\"\n",
...
// by design.
//
// This package has no subdirectory today. That is precisely why the test
// exists and why this file was written before any protocol code: the fence is
// proven effective against a directory that is not there, so that whatever
// arrives later arrives inside it.
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/civ/ic7610"; got.path != want {
...
// file walks rather than calling parser.ParseDir, by demonstrating the miss
// rather than asserting it in a comment: ParseDir over the same tree's root
...
func TestScanForbiddenImports_IsRecursiveWhereParseDirIsNot(t *testing.T) {
	...
	if missed != 0 {
		t.Fatalf("parser.ParseDir found %d forbidden imports in the tree's root — it appears to recurse now, so this package's walking scan is no longer the reason the fence had to be a walk", missed)
	}
