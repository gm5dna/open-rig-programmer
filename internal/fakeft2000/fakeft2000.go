...
// Radio is a simulated FT-2000 or FT-2000D: an in-memory duplex pipe
...
type Radio struct {
	...
	// NO protocol at all — it is PROTOCOL-FREE, so a bug in it cannot make a
	// wrong codec look right (doc.go).
	pipe *fakepipe.Pipe
...
// WithFactoryImage option the slot map defaults to DefaultImage(); without a
// WithModelName option this Radio answers ID as FT-2000 (doc.go).
...
// once. It deliberately leaves the HOST end open, so a pending or subsequent
// read there sees io.EOF — the signal a host should get from "the radio went
// away".
...
//
// ITS ONLY WRITE IS A REPLY: nothing in this loop can originate a frame, so
// this fake never pushes anything unsolicited.
func (r *Radio) serve() {
...
// is silence, and silence is SUCCESS for every accepted Set this radio takes
// (doc.go: "What is NOT in this register" — the accepted-Set/nothing-to-say
// convention).
