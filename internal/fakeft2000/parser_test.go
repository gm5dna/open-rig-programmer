...
func TestValidModeByte(t *testing.T) {
	...
	for _, b := range []byte("0DEF") {
		if validModeByte(b) {
			t.Errorf("validModeByte(%q) = true, want false — this radio's legend has no hole, no placeholder and stops at C", b)
		}
	}
}
...
func TestValidCTCSSByte(t *testing.T) {
	...
	for _, b := range []byte("34") {
		if validCTCSSByte(b) {
			t.Errorf("validCTCSSByte(%q) = true, want false — this radio's P8 is three-valued, not the FT-991A's five", b)
		}
	}
}
...
func TestValidToneDigits(t *testing.T) {
	tests := []struct {
		...
		{"00", true},
		{"49", true},
		{"99", true}, // shape only — CN's separate 0-49 ceiling is not enforced here
		{"9", false},
		{"999", false},
		{"AB", false},
	}
...
func TestHandleMR_OutOfDomainSlotAnswersRejection(t *testing.T) {
	r := New()
	defer r.Close()
	for _, slot := range []string{"000", "118", "999"} {
...
func TestHandleID_DefaultRowIsFT2000(t *testing.T) {
	r := New()
	defer r.Close()
	if reply, want := send(r, "ID;"), []byte("ID0251;"); !bytes.Equal(reply, want) {
		t.Errorf("ID answer = %q, want %q", reply, want)
	}
}

func TestHandleID_FT2000D(t *testing.T) {
	r := New(WithModelName("FT-2000D"))
	defer r.Close()
	if reply, want := send(r, "ID;"), []byte("ID0252;"); !bytes.Equal(reply, want) {
		t.Errorf("ID answer = %q, want %q", reply, want)
	}
}
...
func TestWithModelName_UnknownNamePanics(t *testing.T) {
	defer func() {
		if recover() == nil {
			t.Fatal("WithModelName with an unknown name did not panic")
		}
	}()
	WithModelName("FT-991A")
}
