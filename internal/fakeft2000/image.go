...
// (layout:928-931, state.go's Mode doc).
const (
	modeLSB = '1'
	modeUSB = '2'
)
...
// that would need more than 8 digits (matrix §1.1: this radio's P2 is one
// digit narrower than every other registered Yaesu dialect's).
func encodeFreqDigits(hz uint64) (string, error) {
	if hz > 99_999_999 {
		return "", fmt.Errorf("fakeft2000: frequency %d Hz needs more than 8 digits", hz)
	}
...
func defaultState(freqHz uint64, mode byte) MemState {
	...
	if !validModeByte(mode) {
		panic("fakeft2000: invalid mode byte in a fixture constant")
	}
	return MemState{
		Freq:     freq,
		ClarSign: '+',
...
// (layout:846-847): DIRECT DECIMAL CHANNEL NUMBERS, so pmsSlot(1, 'L') is
...
// nothing else.
//
// MINIMAL BY DESIGN, mirroring internal/fakeft991a's own DefaultImage
...
// least one PMS pair populated so the direct-decimal PMS numbering (this
// radio's own difference from the FT-991A's identical span) is exercised, and
// nothing outside 001-117 because this radio has none.
//
// THE CONTENT IS INVENTED (doc.go's register entry THE DEFAULT IMAGE'S
// CONTENT IS INVENTED). No FT-2000 or FT-2000D's factory memory contents have
...
func DefaultImage() map[string]MemState {
	return map[string]MemState{
		...
		// internal/fakeft991a's own DefaultImage uses for its first channel.
