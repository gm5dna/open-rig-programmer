...
// out-of-domain slot, a malformed frame, an unknown command and an
// overflowed accumulator are indistinguishable to the host, the
// fleet-wide convention every registered Yaesu fake plays (doc.go, "What
// is NOT in this register, and why").
var rejection = []byte("?;")
...
// frame means. Copied in shape from internal/fakeft2000's own.
...
// --- Slot grammar ---
//
// MW and MR both draw the same span, DIRECT DECIMAL CHANNEL NUMBERS,
// "(001 117)" (layout:918, :945): 001-099 plain memory channels, 100-117
// nine PMS pairs (matrix §2.4). MC's own legend prints a wider span
// (000-117) that MW/MR do not share — doc.go's register entry 2 and the
// matrix's own erratum note — so this fake's slot grammar follows MW/MR
// and refuses 000 everywhere, including as an MC-set target.
...
// bytes, "Clarifier Direction +: Plus Shift, --: Minus Shift"
// (layout:945-946).
...
// "0: CTCSS OFF 1: CTCSS ENC/DEC 2: CTCSS ENC" (layout:929, :956) — the
// ordinary legacy domain, no DCS member (matrix §2.9).
func validCTCSSByte(b byte) bool { return b >= '0' && b <= '2' }

// p9WriteFixed is the literal MW must carry on P9 — "P9: 0: (Fixed)"
// (layout:955-957). Unlike internal/fakeft2000's live-both-directions
// tone index, this radio's write side accepts nothing else — doc.go's
// register entry TONE INDEX (P9) (matrix §1.3).
const p9WriteFixed = "00"

// validToneDigits reports whether s is a 2-digit ASCII tone-table index —
// the shape MR's own answer field carries (layout:930-931). Used only to
// validate state placed directly by WithSlot/WithFactoryImage, never a
// Set (which must match p9WriteFixed exactly).
...
// "0: Simplex 1: Plus Shift 2: Minus Shift" (layout:931, :957).
...
// (docs/superpowers/ftdx3000-capability-matrix.md §1.1, layout:917-931 and
// :944-957). Positions are 0-indexed offsets INTO THE BLOCK — i.e. into a
// frame's bytes after the two-byte command name and before the
// terminator:
//
...
// mwSetKindFixed is MW's own Set-direction P7, "P7 0: (Fixed)"
// (layout:955): the write direction carries no channel information at
// all.
const mwSetKindFixed = '0'

// kindMemory is the P7 byte every populated slot answers with, whichever
// of MR's Read legend's two members, "0: VFO 1: Memory" (layout:929), a
...
// FIELD STRICTNESS) and returns the slot it names together with the state
// it encodes. ok is false on the first violation found; the caller
...
func parseMemoryBlock(block []byte) (slot string, s MemState, ok bool) {
	...
	// P9 must be the literal fixed form — the asymmetry (doc.go register
	// entry 3, matrix §1.3). Anything else, shape-valid or not, is
	// refused: this fake never coerces a nonzero tone Set to "00".
	tone := string(block[blkToneStart:blkToneEnd])
	if tone != p9WriteFixed {
		return "", MemState{}, false
	}
...
// --- MW: MEMORY CHANNEL WRITE (block layout:944-957) ---
//
...
// --- MR: MEMORY CHANNEL READ (block layout:917-931) ---
//
// Read/Answer only. Read frame (6 bytes) "MR" + 3-byte slot + ';';
// Answer frame (27 bytes) "MR" + the 24-byte field block + ';'.
...
// --- MC: MEMORY CHANNEL (block layout:875-883) ---
//
...
func (r *Radio) handleMC(body []byte) []byte {
	...
	case slotWireLen:
		slot := string(body)
		if !validSlot(parseSlotForm(slot)) {
			// Includes MC's own "000" hole — doc.go's register entry 2:
			// this fake follows MW/MR's narrower span, not MC's wider one.
			return rejection
		}
		r.mu.Lock()
		defer r.mu.Unlock()
...
// --- ID (block layout:759-762) ---
//
// No Set. Read "ID;", a 7-byte Answer: "ID" + the 4-digit CATID + ';'.
// This package's single row always answers "0462" (doc.go).
...
// --- AI: AUTO INFORMATION (block layout:210-217) ---
//
// Set frame (4 bytes) "AI" + P1 + ';', fire-and-forget; Read "AI;" (3
// bytes) answered by "AI" + P1 + ';' (4 bytes). P1 is "0: Auto Information
// OFF / 1: Auto Information ON" (layout:212-213).
//
// core/transport.Engine.Init opens every CAT session with an
// unconditional "AI0;" (ClassWrite), so this handler's silent-accept path
// is on the critical path of every fake session — an unmodelled AI
// answering "?;" like any other unknown command would make Init's write
// draw a rejection and fail the open outright.
//
...
// accepted, stored and read back faithfully, and then nothing follows
...
// consists of 2 alphabetical characters. You may use either lower or
// upper case characters." (layout:98-99). Field values remain
// case-sensitive: the manual's statement is about the two-character
// command name alone.
