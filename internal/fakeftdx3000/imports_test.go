...
// This file is internal/fakeft2000's imports_test.go, COPIED — copied
// because the rule it enforces forbids importing anything to share (see
// doc.go, THE HARD RULE, and its "sibling, not a refactor" section), and
// because that package's version is already the RECURSIVE one: it walks
// the tree rather than calling parser.ParseDir("."), which reads one
// directory and stops. THE SCAN IS RECURSIVE so that if this package ever
// grows a subdirectory, the next import lands inside the fence rather
// than in front of it.
...
// which fakeftdx3000 must never have, in this directory or any beneath it
...
// It is the single share Stuart permitted on 06/09/2026, and it is safe
// precisely because a bug in it cannot make a wrong codec look right; it
// can only stop bytes moving, which this package's own tests notice at
// once. Everything above the wire stays here, written independently (see
// doc.go).
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/cat/ftdx3000 — the dialect this fake speaks and must not consult", "github.com/gm5dna/open-rig-programmer/core/cat/ftdx3000", true},
		...
		{"core/driver/ftdx3000 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ftdx3000", true},
		{"internal/fakeft2000 — the sibling fake this package is shaped after, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakeft2000", true},
		{"internal/fakeft991a — another sibling fake", "github.com/gm5dna/open-rig-programmer/internal/fakeft991a", true},
		{"internal/fakeradio — the FT-710's", "github.com/gm5dna/open-rig-programmer/internal/fakeradio", true},
		{"fakeftdx3000 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeftdx3000", true},
...
// each non-test .go file's import block, and reports every
...
//     anything and cannot introduce a dependency.
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeftdx3000
// must not import anything project-internal — not core/cat, not
// core/cat/ftdx3000, not core/codeplug, not core/spec, and not any
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeftdx3000 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}

	// Vacuity guards. Without them a filter bug, a rename, or a walk that
	// silently visited nothing would leave this test passing with
	// nothing examined.
	if res.files == 0 {
...
// nowhere else — the _test.go file beside it and the testdata fixture are
