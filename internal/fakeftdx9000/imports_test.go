...
// This file is internal/fakeft991a's imports_test.go, COPIED VERBATIM bar this
// comment and the package name — copied because the rule it enforces forbids
// importing anything to share (see doc.go, THE HARD RULE), and because that
// package's version is already the RECURSIVE one: it walks the tree rather
// than calling parser.ParseDir("."), which reads one directory and stops. This
// package has no generator subdirectory of its own, but the fence stays
// recursive so that the next thing to arrive here lands inside it rather than
// in front of it.
...
// fakeftdx9000 must never have, in this directory or any beneath it (see
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/driver/ftdx9000 — the driver under test against this fake, and forbidden to this package by the milestone's quarantine", "github.com/gm5dna/open-rig-programmer/core/driver/ftdx9000", true},
		{"internal/fakeft991a — the sibling fake this package is shaped after, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakeft991a", true},
		{"internal/fakeft891 — another sibling fake", "github.com/gm5dna/open-rig-programmer/internal/fakeft891", true},
		{"internal/fakeradio — the FT-710's", "github.com/gm5dna/open-rig-programmer/internal/fakeradio", true},
		{"fakeftdx9000 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeftdx9000", true},
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeftdx9000 must not
// import anything project-internal — not core/cat, not core/driver/ftdx9000,
// not core/codeplug, not core/spec, and not any sibling fake — so that a
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeftdx9000 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
