...
// HARD RULE, and its "sibling, not a refactor" section), which makes a shared
// helper package the one thing this test cannot be built out of.
//
// It is the RECURSIVE version: it walks subdirectories, where
// internal/fakeradio's original uses parser.ParseDir("."), which reads one
// directory and stops. This package has no subdirectory today. THE FENCE LANDS
// ANYWAY, AND IT LANDS RECURSIVE, so that anything added beneath this directory
// later — a generator, a fixture builder — arrives inside a fence rather than in
// front of one, and
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/ic705 — the profile whose record layout this fake must re-derive", "github.com/gm5dna/open-rig-programmer/core/civ/ic705", true},
		{"core/driver/ic705 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ic705", true},
...
//     internal, but core/driver/ic705's tests will import both this package and
//     half of core/, and that is correct).
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeic705 must not
// import anything project-internal — not core/civ, not core/civ/ic705, not
// core/driver/ic705, not core/codeplug, not core/spec, not internal/fakeradio
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic705 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// a generator beneath this package would take), one violating _test.go beside
// it, and one violating file under testdata. Only the subdirectory's non-test
// file may be reported.
func fenceTestTree(t *testing.T) string {
	t.Helper()
	return writeTree(t, map[string]string{
		"clean.go":           "package p\n\nimport \"io\"\n\nvar _ io.Reader\n",
		"gen/main.go":        "package main\n\nimport _ \"github.com/gm5dna/open-rig-programmer/core/civ/ic705\"\n",
...
// by design.
//
// It is the whole reason this file lands with the package rather than with the
// first subdirectory: no subdirectory exists yet, and the fence is proven
// effective against one that does not.
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/civ/ic705"; got.path != want {
