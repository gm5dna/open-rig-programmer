...
// This file is internal/fakeic7851's imports_test.go, COPIED — copied
// because the rule it enforces forbids importing anything to share (see
// doc.go, THE HARD RULE), so a fresh copy is how every fake package proves
// it independently. Nothing about the copy is edited except the package
// clause, the prose, and the forbidden-import test-case list.
...
// which fakeic9100 must never have, in this directory or any beneath it
...
// It is safe precisely because a bug in it cannot make a wrong codec look
// right; it can only stop bytes moving, which this package's own tests
// notice at once. Everything above the wire stays here, written
// independently (see doc.go).
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/ic9100 — the dialect this fake is the other witness to", "github.com/gm5dna/open-rig-programmer/core/civ/ic9100", true},
		{"core/driver — the layer under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver", true},
		{"core/driver/ic9100", "github.com/gm5dna/open-rig-programmer/core/driver/ic9100", true},
		...
		{"internal/fakeic7100 — a sibling fake, equally forbidden", "github.com/gm5dna/open-rig-programmer/internal/fakeic7100", true},
		{"internal/fakeic7851", "github.com/gm5dna/open-rig-programmer/internal/fakeic7851", true},
		{"fakeic9100 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeic9100", true},
		...
		{"clone dir name is not the module path either", "codex-icom-ic9100/core/civ", false},
...
//     and cannot introduce a dependency; flagging one would be a false
//     positive on a fixture. Nothing in this package puts Go source there
//     today.
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeic9100 must
// not import anything project-internal — not core/civ, not
// core/civ/ic9100, not core/driver, not core/codeplug, not core/spec and
// not any sibling fake — so that a systematic misreading of the IC-9100
// memory record cannot agree with itself on both sides of a test and pass
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic9100 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
func fenceTestTree(t *testing.T) string {
	...
	return writeTree(t, map[string]string{
		"clean.go":           "package p\n\nimport \"io\"\n\nvar _ io.Reader\n",
		"gen/main.go":        "package main\n\nimport _ \"github.com/gm5dna/open-rig-programmer/core/civ/ic9100\"\n",
...
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/civ/ic9100"; got.path != want {
...
// parser.ParseDir over the same tree's root reports no violation at all,
// because it never descends into gen/.
func TestScanForbiddenImports_IsRecursiveWhereParseDirIsNot(t *testing.T) {
	...
	if missed != 0 {
		t.Fatalf("parser.ParseDir found %d forbidden imports in the tree's root — it appears to recurse now, so this package's walking scan may no longer be needed", missed)
	}
