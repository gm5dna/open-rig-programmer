...
// This file is internal/fakedx101's imports_test.go, COPIED IN MECHANISM —
// copied because the rule it enforces forbids importing anything to share (see
// doc.go, THE HARD RULE). Only the mechanism was taken: the module prefix, the
// predicate, the recursive walking scan and the four tests below. Nothing about
// what fakedx101 knows, answers or contains was read or carried across.
//
...
//
// That hole would be a real one here. This fake's whole reason to exist is that
// its knowledge of the IC-9700 memory record comes from two printed-diagram
// transcriptions and from nothing in this module: not core/civ/ic9700, not its
// profile, not its golden vectors, not its field ledger, not core/driver, and
// not another fake. If any of those could be imported — from this directory or
// any subdirectory anyone later adds — then the fake and the code under test
// would share a reading of the record, and a systematic misreading would agree
// with itself end to end and pass every test invisibly. The fence is what makes
// that independence mechanical rather than a matter of good intentions, and it
// lands with the package core, before any subdirectory exists, so that anything
// added later arrives inside a fence rather than in front of one.
// TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory proves it
// would bite there today, while that directory still does not exist.
...
// fakeic9700 must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/ic9700 — the dialect this fake speaks and must not consult", "github.com/gm5dna/open-rig-programmer/core/civ/ic9700", true},
		{"core/driver — the layer under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver", true},
		{"core/driver/ic9700", "github.com/gm5dna/open-rig-programmer/core/driver/ic9700", true},
		...
		{"internal/fakedx101 — this package's structural exemplar, equally forbidden", "github.com/gm5dna/open-rig-programmer/internal/fakedx101", true},
		{"internal/fakedx10", "github.com/gm5dna/open-rig-programmer/internal/fakedx10", true},
		{"fakeic9700 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeic9700", true},
		{"stdlib", "io", false},
		...
		{"worktree dir name is not the module path either", "icom-ic9700/core/civ", false},
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeic9700 must not
// import anything project-internal — not core/civ, not core/civ/ic9700, not
// core/driver, not core/codeplug, not core/spec, not internal/fakeradio and not
// any sibling fake — so that a systematic misreading of the IC-9700 memory
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic9700 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
func fenceTestTree(t *testing.T) string {
	...
	return writeTree(t, map[string]string{
		"clean.go":           "package p\n\nimport \"io\"\n\nvar _ io.Reader\n",
		"gen/main.go":        "package main\n\nimport _ \"github.com/gm5dna/open-rig-programmer/core/civ/ic9700\"\n",
...
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/civ/ic9700"; got.path != want {
