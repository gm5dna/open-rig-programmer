...
// in for a serial line, and the port the driver opens onto them. Nothing here
// knows a CI-V byte from any other byte.
...
// WithBroadcasts, WithAddressedFlood — would then wedge its own emitter goroutine
// the moment the consumer under test stopped reading, which is precisely the
// condition those options exist to create. A drain cap is not reachable if the
// flood cannot outrun the drain.
type pipe struct {
	mu     sync.Mutex
	cond   *sync.Cond
	buf    []byte
