...
// This file is internal/fakets590's imports_test.go, COPIED — copied because
// the rule it enforces forbids importing anything to share (see doc.go, THE
// HARD RULE), and because that package's version is already the RECURSIVE one:
// it walks the tree rather than calling parser.ParseDir("."), which reads one
// directory and stops.
//
// THE SCAN IS RECURSIVE, and that is not a stylistic preference. This package
// has no subdirectory today and the plan gives it none — there is no generator
// here, because the widths table is projected at init from the embedded CSV
// (exinventory.go, the plan's P19) — but the piece most likely to arrive one
// day is exactly the piece that would reach for internal/extable, the A-side
// machinery this projection must never share. One parser on both sides of the
// EX cross-check would reproduce a shared parsing bug into both inventories
// invisibly. So the fence is recursive from the start, and the next
// subdirectory to arrive lands inside it rather than in front of it.
...
// fakets990 must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/kw — the envelope this fake must be able to disagree with", "github.com/gm5dna/open-rig-programmer/core/kw", true},
		{"core/kw/ma — the codec this fake must be able to disagree with", "github.com/gm5dna/open-rig-programmer/core/kw/ma", true},
		...
		{"core/driver/ts990 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ts990", true},
		{"internal/fakets590 — the sibling fake this package is shaped after, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakets590", true},
		{"internal/fakets890 — this milestone's other new fake, whose grid, legends and lockout encoding differ", "github.com/gm5dna/open-rig-programmer/internal/fakets890", true},
		{"internal/extable — the A-side machinery this projection must never share", "github.com/gm5dna/open-rig-programmer/internal/extable", true},
		{"fakets990 itself", "github.com/gm5dna/open-rig-programmer/internal/fakets990", true},
...
//     internal, but core/driver/ts890's tests will import both this package
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakets990 must not
// import anything project-internal — not core/kw, not core/kw/ma, not
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakets990 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// in the root, one VIOLATING file in a subdirectory (gen/, by name — the
// subdirectory this package would grow if anyone reintroduced the generator
// P19 declines), one violating _test.go beside it, and
...
func TestScanForbiddenImports_IsRecursiveWhereParseDirIsNot(t *testing.T) {
	...
	if missed != 0 {
		t.Fatalf("parser.ParseDir found %d forbidden imports in the tree's root — it appears to recurse now, so this package's walking scan is no longer the reason the copied fence had to be a walk", missed)
	}
