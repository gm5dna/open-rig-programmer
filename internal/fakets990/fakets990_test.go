...
// the failure vocabulary for a command outcome (990:108-113), so a passing
...
// TestID_AnswersThisRowsPrintedNumber. "022" (990:2612) is the only value this
// book prints, and the answer's shape is the six-cell grid "I D P1 P1 P1 ;"
// (990:2617). It is what makes a probe against this fake succeed for this
// registry row and produce a wrong-radio refusal for any other — the TS-890S's
// own number is 024, printed in that radio's book and nowhere in this one.
func TestID_AnswersThisRowsPrintedNumber(t *testing.T) {
	_, conn := newTestRadio(t)
	if got, want := exchange(t, conn, "ID;"), "ID022;"; got != want {
		t.Errorf("ID; -> %q, want %q (990:2612)", got, want)
	}
}

// TestID_HasNoSetDirection. The ID block prints a Read and an Answer and no
// Set at all (990:2609-2617), so anything between "ID" and ';' is simply
// unknown.
func TestID_HasNoSetDirection(t *testing.T) {
	_, conn := newTestRadio(t)
	for _, send := range []string{"ID022;", "ID0;"} {
...
// only a four-cell width (990:2536) and one worked example, "for firmware
// version 1.00, it reads 'FV1.00;'" (990:2533) — doc.go's register entry THE
// DEFAULT FIRMWARE STRING.
func TestFV_AnswersTheBooksWorkedExample(t *testing.T) {
	_, conn := newTestRadio(t)
	if got, want := exchange(t, conn, "FV;"), "FV1.00;"; got != want {
		t.Errorf("FV; -> %q, want %q (990:2533)", got, want)
	}
	if len(strings.TrimSuffix(strings.TrimPrefix("FV1.00;", "FV"), ";")) != firmwareFieldLen {
		t.Errorf("the worked example's P1 field is not %d cells wide (990:2536)", firmwareFieldLen)
	}
}
...
func TestAI_ReadAnswersOffAtConstruction(t *testing.T) {
	...
	if got, want := exchange(t, conn, "AI;"), "AI0;"; got != want {
		t.Errorf("AI; -> %q, want %q (990:173)", got, want)
	}
}

// TestAI_SetIsFireAndForgetAndMovesTheState over the three values this book
// prints WITH A MEANING: "0: AI OFF" (990:173), "2: AI ON (Not back up the ON
// state)" (990:176) and "4: AI ON (Back up the ON state)" (990:178).
...
// TestAI_RefusesTheTwoValuesPrintedNotUsed. "1: Not used" (990:174) and
// "3: Not used" (990:177) are printed in the legend and given no meaning, and
...
// TestAI_NothingIsEverPushedUnsolicited. The book says an AI-ON radio sends
// the respective response command when a parameter is changed (990:180-182),
// and no TS-990S has been observed by this project; modelling silence is the
...
// lower or upper case characters." (990:77-83). The mixed-case form is a
...
func TestCommandNamesAreAcceptedInEitherCase(t *testing.T) {
	...
	for _, send := range []string{"id;", "Id;", "iD;"} {
		if got, want := exchange(t, conn, send), "ID022;"; got != want {
			t.Errorf("%q -> %q, want %q", send, got, want)
		}
	}
	// The three-character name folds the same way, and the digit is
	// untouched.
	if got := exchange(t, conn, "ma0050;"); len(got) != recLen {
		t.Errorf("ma0050; -> %q (%d bytes), want a %d-byte blank answer", got, len(got), recLen)
	}
}

// TestFieldValuesRemainCaseSensitive. The sentence at 990:77-83 is about the
// command NAME; extending it to parameters would be an invented leniency. The
// mode letters A-N (990:3717-3730) are the field this can be shown on.
...
// TS-590's MR read and MC selector, and the MA-family members this milestone
// does not model.
func TestUnknownCommandsAreRejected(t *testing.T) {
	...
	for _, send := range []string{
		...
		"MA1" + strings.Repeat("0", 14) + ";", // MA1 Memory Channel Direct Entry, off the roster
		"MA5000;",                             // MA5 Channel Deletion, off the roster
		"MA;",                                 // no such command: the family is MA0..MA6
		"MN007;",                              // Memory Channel Number Set, off the roster (doc.go)
		"MN;",                                 // Memory Channel Number Read, off the roster (doc.go)
...
// that a receive-buffer overrun produces "O;" (990:121), a different event
...
func TestAccumulatorOverflowRejectsOnceAndResyncs(t *testing.T) {
	...
	if got, want := mustReadFrame(t, conn), "ID022;"; got != want {
...
// TestStreamErrorsReplaceTheExchangesReply. The two SERIAL-LINE tokens are not
// command outcomes: "E;" reports a communication error such as an overrun or
// framing error (990:118-120) and "O;" a receive buffer overrun (990:121).
func TestStreamErrorsReplaceTheExchangesReply(t *testing.T) {
	for _, tt := range []struct {
		kind StreamError
		want string
	}{{StreamErrorE, "E;"}, {StreamErrorO, "O;"}} {
		t.Run(tt.want, func(t *testing.T) {
			_, conn := newTestRadio(t, WithStreamError(tt.kind, 1))
			if got := exchange(t, conn, "ID;"); got != tt.want {
				t.Errorf("exchange 1 -> %q, want %q", got, tt.want)
			}
			if got, want := exchange(t, conn, "ID;"), "ID022;"; got != want {
				t.Errorf("exchange 2 -> %q, want %q — the script covers one exchange", got, want)
			}
		})
	}
}

// TestWithTransientNAKSuppressed_DropsTheRejection. The error table prints,
// under the "?;" row itself: "Occasionally, this message may not appear due to
// microprocessor transients in the transceiver." (990:114-116). So a Kenwood
// host cannot treat silence as "the radio did not refuse".
func TestWithTransientNAKSuppressed_DropsTheRejection(t *testing.T) {
	_, conn := newTestRadio(t, WithTransientNAKSuppressed())
	writeFrame(t, conn, "XY;")
	assertNoReply(t, conn)
	// An accepted answer is untouched.
	if got, want := exchange(t, conn, "ID;"), "ID022;"; got != want {
		t.Errorf("ID; -> %q, want %q", got, want)
	}
}

// TestWithMemoryReadUnsupported_RefusesEveryMA0Read. It plays the SECOND cause
// the error table itself prints (990:109-113), which is a state and not a
// defect.
func TestWithMemoryReadUnsupported_RefusesEveryMA0Read(t *testing.T) {
	_, conn := newTestRadio(t, WithMemoryReadUnsupported())
	for _, slot := range []string{"000", "099"} {
