...
// TestTheInboundAllFFRecordReadsAsEmpty pins the modelled assumption
// ic7760-empty-reply-ff: that a stored record of FF bytes read back means
// "empty". The guide's only FF in the memory context is a value the CONTROLLER
// sends to erase, and nothing licenses reading it backwards, so the
// interpretation is ASSUMED — and it is what this fake does, always.
func TestTheInboundAllFFRecordReadsAsEmpty(t *testing.T) {
...
// this fake makes always, and it is not the same rule as the inbound all-FF
// reading (ic7760-empty-reply-ff) or the full-record insistence
...
// is unprinted, so this fake insists.
...
// TestTheBroadcastFormIsTheRegisterEntry pins ic7760-broadcast-form: the guide
// prints no to=00 broadcast frame, so the address unsolicited frames carry is
// the tier's assumption.
...
func TestTheBroadcastFormIsTheRegisterEntry(t *testing.T) {
...
func TestEveryModelledAssumptionIsReachableUnderItsRegisterName(t *testing.T) {
	...
	d := defaultConfig()
	if d.emptyReply != CodeNG || d.recordLen != RecordLen || d.addr != AddrRadio {
