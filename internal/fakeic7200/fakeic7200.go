...
// follows a 1A 00 channel selector. See doc.go, "The record: 17 bytes,
// not 9". WithRecordLength overrides it.
const RecordLen = 17

// controllerAddr is the printed controller address (matrix §3.4);
// radioAddrDefault is the IC-7200's own printed default, 76h. scanEdgeP1
// and scanEdgeP2 are the slot keys parseChannel gives "P1" and "P2":
// negative, so that they cannot collide with memory channels 1 to 199.
const (
	controllerAddr   byte = 0xe0
	radioAddrDefault byte = 0x76
	scanEdgeP1            = -1
	scanEdgeP2            = -2
)
...
// over the wire. addr is "001".."199", "P1" or "P2" (parseChannel). It
...
func (r *Radio) handle(f wireFrame) []byte {
	...
	// 0x0b (Memory clear), the live 0F split command, the live 1A 02
	// filter command and anything else this package does not model: all
	// refused, per doc.go.
	return r.answer(f, 0xfa)
}

// selector decodes the two packed-BCD channel bytes ①,② (matrix §1 row 4,
// §1b): 0001-0199 are memory channels 1 to 199, 0200 is programmed scan
// edge P1 and 0201 is P2. It returns the slot key parseChannel produces
...
func (r *Radio) memory(f wireFrame, p []byte) []byte {
	...
	// A short (or long) set is refused — ASSUMED, matrix §3.10, register
	// entry ic7200-full-record-mandatory: the manual prints one full
	// record form and says nothing about a partial one. Unlike IC-7610,
	// this manual prints no separate "1A 00 <hi> <lo> FF" clear form to
	// special-case (matrix §3.13); a length mismatch alone is enough.
