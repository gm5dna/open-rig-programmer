...
// content — a memory channel (001-500) or a PMS band-edge (501-504), the
...
type MemState struct {
	...
	Freq string
	// ClarSign is P3's sign byte, '+' or '-' (layout:791-792).
	ClarSign byte
	// ClarMag is P3's 4-digit ASCII magnitude, "Clarifier Offset: 0000 -
	// 9999 (Hz)" (layout:791-792).
	ClarMag string
	// RXClar is P4, "0: RX CLAR OFF 1: RX CLAR ON" (layout:793).
	RXClar bool
	// TXClar is P5, "0: TX CLAR OFF 1: TX CLAR ON" (layout:794).
	TXClar bool
	// Mode is P6, one ASCII byte from the eleven-member legend
	// (layout:794-797/798): 1 LSB, 2 USB, 3 CW, 4 FM, 5 AM,
	// 6 DATA(RTTY-LSB), 7 CW-R, 8 USER-L, 9 DATA(RTTY-USB), B FM-N,
	// C USER-U. A CLEAN HOLE at 'A', no 'D'/'E'/'F' member at all (matrix
	// §1.2, doc.go).
	Mode byte
	// Kind is P7 — the ANSWER byte, "0: VFO 1: Memory" (layout:796, MR's
	...
	// Fixed" (layout:798) — the Set carries no channel information at
	// all, so there is nothing to store from it; this field is what a
	// read answers with, not what a Set delivered.
	Kind byte
	// CTCSS is P8: '0' CTCSS OFF, '1' CTCSS ENC/DEC, '2' CTCSS ENC — THREE
	// values (layout:799), the ordinary legacy domain, no DCS member
	// (matrix §2.14).
	CTCSS byte
	// Tone is P9: the 2-digit ASCII tone-table index, "Tone Number (See
	// Table 1)" (layout:800). A LIVE VALUE, stored at its printed WIDTH
	// and not range-checked against CN's separate 00-49 ceiling (doc.go's
	// register entry TONE INDEX (P9) IS STORED AT ITS PRINTED WIDTH).
	Tone string
