...
func TestNew_DefaultsToFT450D(t *testing.T) {
	r := New()
	defer r.Close()
...
func TestPort_RoundTrip(t *testing.T) {
	...
	if got, want := buf[:n], []byte("ID0244;"); !bytes.Equal(got, want) {
...
func TestWithSlot_IsTheOnlyRouteToNonDefaultPMSState(t *testing.T) {
	// doc.go's register entry PMS SLOTS (501-504) REFUSE EVERY MW SET:
	// WithSlot/WithFactoryImage are the sole way a PMS slot ever carries
	// scripted state.
	r := New(WithSlot("503", MemState{
		Freq: "05000000", ClarSign: '+', ClarMag: "0000",
		Mode: '4', Kind: kindMemory, CTCSS: '0', Tone: "00", Shift: '0',
	}))
	defer r.Close()
	s, ok := r.SlotState("503")
	if !ok || s.Freq != "05000000" {
		t.Errorf("WithSlot did not overlay PMS channel 503: %+v, ok=%v", s, ok)
	}
}
