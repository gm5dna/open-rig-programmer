...
// out-of-legend Mode or CTCSS byte, a Tone outside two digits) and drive a
// real driver's parse-error path through a real fake rather than a
// scripted transcript. This is also the ONLY way a PMS slot (501-504) can
// carry non-default state: MW refuses every PMS Set (doc.go's register
// entry 6), so WithSlot and WithFactoryImage are the sole routes in.
