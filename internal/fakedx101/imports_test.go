...
// This file is internal/fakedx10's imports_test.go, COPIED — copied because the
// rule it enforces forbids importing anything to share (see doc.go, THE HARD
...
// fakedx101 must never have, in this directory or any beneath it (see doc.go,
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/cat/ftdx101 — the dialect this fake speaks and must not consult", "github.com/gm5dna/open-rig-programmer/core/cat/ftdx101", true},
		...
		{"core/driver/ftdx101 — the driver under test against this fake", "github.com/gm5dna/open-rig-programmer/core/driver/ftdx101", true},
		{"internal/fakeradio — a sibling fake, deliberately not shared", "github.com/gm5dna/open-rig-programmer/internal/fakeradio", true},
		{"internal/fakedx10 — this package's structural exemplar, equally forbidden", "github.com/gm5dna/open-rig-programmer/internal/fakedx10", true},
		{"internal/extable — gen/ must not reach for it", "github.com/gm5dna/open-rig-programmer/internal/extable", true},
		{"fakedx101 itself", "github.com/gm5dna/open-rig-programmer/internal/fakedx101", true},
		{"stdlib", "io", false},
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakedx101 must not
// import anything project-internal — not core/cat, not core/cat/ftdx101, not
// core/codeplug, not core/spec, not internal/fakeradio and not
// internal/fakedx10 — so that a systematic bug in the production codec cannot
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakedx101 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// this package's own generator will take), one violating _test.go beside it,
// and one violating file under testdata. Only the subdirectory's non-test file
// may be reported.
