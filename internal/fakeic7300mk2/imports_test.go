...
// HARD RULE, and its "A SIBLING, not a refactor" section).
//
// It is the RECURSIVE version, and that matters here for the same reason it
// mattered there: internal/fakeradio's original uses parser.ParseDir("."),
// which reads one directory and stops, so anything in a subdirectory sits
// outside the fence entirely. This package has no subdirectory today. THE
// FENCE LANDS WITH THE PACKAGE ANYWAY, so that whatever arrives later — a
// generator, a fixture builder — arrives inside a fence rather than in front
// of one, and TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory
// proves the scan would bite there before the directory exists.
...
// fakeic7300mk2 must never have, in this directory or any beneath it (see
// doc.go, THE HARD RULE): it may depend only on the standard library.
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/civ/ic7300mk2 — the codec this fake must not consult", "github.com/gm5dna/open-rig-programmer/core/civ/ic7300mk2", true},
		...
		{"internal/fakeic7300 — the MK1's fake, a sibling and equally forbidden", "github.com/gm5dna/open-rig-programmer/internal/fakeic7300", true},
		{"internal/fakedx101 — this package's structural exemplar", "github.com/gm5dna/open-rig-programmer/internal/fakedx101", true},
		{"internal/fakeradio", "github.com/gm5dna/open-rig-programmer/internal/fakeradio", true},
		{"internal/extable", "github.com/gm5dna/open-rig-programmer/internal/extable", true},
		{"fakeic7300mk2 itself", "github.com/gm5dna/open-rig-programmer/internal/fakeic7300mk2", true},
		{"stdlib", "io", false},
...
//     project-internal either, by the task's instruction, but a consumer's
//     tests will import both this package and half of core/, and that is
//     correct).
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakeic7300mk2 must not
// import anything project-internal — not core/civ, not core/civ/ic7300mk2, not
// core/driver, not core/codeplug, not core/spec, and not any sibling fake — so
// that a systematic bug in the production codec cannot pass end-to-end tests
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakeic7300mk2 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
...
// such a subdirectory usually takes), one violating _test.go beside it, and one
...
func fenceTestTree(t *testing.T) string {
	...
	return writeTree(t, map[string]string{
		"clean.go":           "package p\n\nimport \"io\"\n\nvar _ io.Reader\n",
		"gen/main.go":        "package main\n\nimport _ \"github.com/gm5dna/open-rig-programmer/core/civ/ic7300mk2\"\n",
...
// placed in a subdirectory this package does not yet have, and that it bites
// there and nowhere else — the _test.go file beside it and the testdata fixture
// are both skipped by design.
//
// The fixture tree is synthetic and independent of anything real, which is what
// keeps this a check on the SCAN rather than on whatever some directory happens
// to import today.
func TestScanForbiddenImports_CatchesAForbiddenImportInASubdirectory(t *testing.T) {
	...
	if want := "github.com/gm5dna/open-rig-programmer/core/civ/ic7300mk2"; got.path != want {
