...
// accident, and a fake that shipped fewer would make core/driver/ftdx10's
...
func TestTwoRadiosFromOneImageDoNotAlias(t *testing.T) {
	...
	_, connA := newTestRadio(t, WithFactoryImage(img))
	_, connB := newTestRadio(t, WithFactoryImage(img))
...
// core/driver/ftdx10's no-contiguity, no-early-termination discovery walk needs
...
func TestWithEMG_PopulatesTheEmergencyChannelOnly(t *testing.T) {
	...
	// 5.1675 MHz USB, the invented placeholder (doc.go register entry 15),
...
func TestFiveMHzSlotSpelling(t *testing.T) {
	...
	// DIALECT's ASSUMED register entry 5 and is not re-derived here.
	tests := []struct {
		n    int
		want string
