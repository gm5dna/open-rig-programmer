...
// legend "Read: 0: VFO 1: Memory" (manual line ~1233).
const (
...
// identically beside four commands (manual lines 1146-1149, 1192-1194,
// 1227-1229, 1267-1269).
const (
	modeLSB = '1'
	modeUSB = '2'
...
// and mode, no clarifier offset, clarifier flags off, CTCSS off, simplex, the
...
func defaultState(freqHz uint64, mode byte) MemState {
	...
	if !validModeBuildByte(mode) {
		panic("fakedx10: invalid mode byte in a fixture constant")
	}
	return MemState{
		Freq:     freq,
		ClarSign: '+',
...
// M-01 populated, the nine PMS pairs P1L-P9U populated, and NOTHING ELSE — no
// 5 MHz bank and no EMG channel (With5xx and WithEMG add those), and no region
// concept anywhere (doc.go's divergence list).
//
// MINIMAL BY DESIGN. Every FTdx10 test that needs a populated channel names
...
// core/driver/ftdx10's discovery walk in its ordinary case: ~100 probes, every
// one answered "?;", no discovered banks.
//
// THE CONTENT IS INVENTED — doc.go register entry 15. No FTdx10's factory
// memory contents have been read by this project; these are placeholders
// carried over from fakeradio's own invented values, and what they exist to
// provide is SHAPE (a populated memory channel, a populated PMS pair), not
// plausible data.
func DefaultImage() map[string]MemState {
	slots := map[string]MemState{
		...
		// from the FT-710's operation manual; nothing establishes it for the
		// FTdx10, and it is a placeholder here.
	...
	// HF/6m band — placeholders, not sourced from any programmed radio.
	bands := []band{
...
// numbering is the DIALECT's ASSUMED register entry 5 (cited), and taking wire
// numbers here keeps this package from re-deriving it — a fixture names the
// codes it wants and this function only spells them.
...
//
// That shape is the point. core/driver/ftdx10's discovery walks the WHOLE
...
// declared range") — precisely because the FT-710's termination rules are that
// radio's hardware facts. A contiguous fixture would let a walk that stopped
// at the first "?;" pass anyway; this one fails it at 503, and a walk that
// missed the ceiling fails it at 599.
//
// It is NOT a claim about any radio's inventory (doc.go register entry 15): no
// FTdx10 this project has touched has a 5 MHz bank at all.
...
// ImageUK/ImageUS pair encodes an FT-710 HARDWARE finding about which
// variants have a 5xx bank; this project has no such evidence about any
// FTdx10, so there is nothing to model a region on, and core/driver/ftdx10
// deliberately implements no RegionReporter.
...
// and a placeholder here too (doc.go register entry 15).
//
...
// placeholder only, exactly as fakeradio uses it. Nothing establishes what a
// real FTdx10's EMG channel holds, or which variants have one (doc.go register
// entry 15).
//
