...
//
// This is core/driver/ftdx10's respondingport_test.go's mtAnswerFields, in the
// fake's own test package for the same reason it exists there: a fixture built
// by the code under test pins the parser against the builder, and the two
// would agree about a wrong offset exactly as happily as about a right one.
...
// (manual lines 1183-1202): "MR" + the same shared field block + ';'. Only the
// fields the block carries are read; tag and P11 are ignored.
...
// above against a frame written out character by character, so every test
// using it rests on something checked rather than on a helper checked by
// nothing.
//
...
// the chart's fixed '0' (and is byte-identical to core/driver/ftdx10's own
// hand-derived wantSetFrame, written independently in that package); the
// answer's is '1', Memory — doc.go register entry 2.
...
// TestMTRead_TagFieldIsSpacePaddedAndAllFillIsNoTag covers the two tag edges
// the combined form has (doc.go register entry 13): a short tag comes back
// padded to the full field, and a slot with no tag comes back all-fill.
func TestMTRead_TagFieldIsSpacePaddedAndAllFillIsNoTag(t *testing.T) {
	...
	if field := got[28:40]; field != strings.Repeat(" ", 12) {
		t.Errorf("an untagged channel's P12 field = %q, want twelve spaces (the DIALECT's ASSUMED TagFill, its register entry 1)", field)
	}
}
...
func TestMTRead_EmptyAndMalformedSlots(t *testing.T) {
	...
	tests := []struct {
		...
		// exchange core/driver/ftdx10 reads as an empty channel, and as
...
func TestMTSet_CreatesAnAbsentChannel(t *testing.T) {
	...
	// core/driver/ftdx10's entry 9: one combined Set creates the channel, with
...
// TestMTSet_RoundTripsByteFaithfully is the round-trip the plan names: a
// 41-byte Set, then a read of the same slot, and the answer's field bytes —
// INCLUDING THE CLARIFIER BYTES — equal the Set's.
//
// The clarifier is the point. fakeradio stores zeros and ignores what was sent,
// on an FT-710 HARDWARE finding about ITS MW command (its register item 20,
// and the spec.Inert write policy resting on it); that finding is not this
// radio's, this fake stores what arrives, and the FTdx10's Simulated profile
// writes the clarifier as Supported because of it (doc.go register entry 5).
// A non-zero, negative, on-step offset with the RX flag set and the TX flag
// clear is the value that would be destroyed by any of the plausible wrong
// behaviours: zeroing, sign-dropping, flag-dropping or rounding.
//
// P7 IS THE ONE EXCEPTION, and it is not a slip: the chart's legend reads
// "Set: 0: (Fixed) / Read: 0: VFO 1: Memory". The Set direction's P7 is a fixed
...
func TestMTSet_AcceptedOnFiveMHzAndEMG(t *testing.T) {
	...
	// and reachable only by writing bytes to Port() as this test does.
...
func TestSetsDoNotMoveTheSelection(t *testing.T) {
	...
	// effect for a radio nobody has written to is the class of borrowed fact
...
func TestMRRead_PopulatedSlot(t *testing.T) {
	...
	// MR is answered even though core/driver/ftdx10 NEVER SENDS IT: the command
	// is on the radio, so it is on the fake (parser.go's MR section). The tag
	// is absent from this frame by the chart, so a tagged channel's MR answer
	// carries the field block alone.
...
func TestMR_HasNoSetDirection(t *testing.T) {
	...
	// This radio's availability table gives MR no Set (X O O X) — a MANUAL
	// FACT, not an assumption. An MR frame in the 28-byte Set shape is simply
	// not a command.
...
// (manual lines 1258-1272): "MW" + the same shared field block + ';', with P7
// the chart's fixed '0'. Everything but the slot and the frequency is the
...
// exists on this radio (availability O X X X) even though this project's driver
// never sends one, and a fake that rejected it would be faking a different
// radio — doc.go register entry 9.
...
func TestMWSet_Rejections(t *testing.T) {
	tests := []struct {
		...
		// MW's own restricted P1 legend (manual line 1259) — a MANUAL FACT of
		// this radio, unlike MT's leniency above.
