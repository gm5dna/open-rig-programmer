...
// This file is internal/fakeradio's imports_test.go, COPIED AND EXTENDED —
...
func TestIsForbiddenImport(t *testing.T) {
	tests := []struct {
		...
		{"core/cat/ftdx10 — the dialect this fake speaks and must not consult", "github.com/gm5dna/open-rig-programmer/core/cat/ftdx10", true},
...
//     internal, but core/driver/ftdx10's tests import both this package's
//     sibling and half of core/, and that is correct).
...
// TestNoCoreImports enforces THE HARD RULE (see doc.go): fakedx10 must not
// import anything project-internal — not core/cat, not core/cat/ftdx10, not
// core/codeplug, not core/spec, and not internal/fakeradio — so that a
...
func TestNoCoreImports(t *testing.T) {
	...
	for _, v := range res.violations {
		t.Errorf("%s imports %q — fakedx10 MUST NOT import any project-internal package, in this directory or any beneath it (see doc.go, THE HARD RULE)", v.file, v.path)
	}
