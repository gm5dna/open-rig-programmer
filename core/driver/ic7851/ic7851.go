...
// New7851 builds the IC-7851 driver. The IC-7851 and IC-7850 share one
...
func (d *ic7851Driver) Model() string { return d.model }
...
var ErrAnswerMismatch = errors.New("ic7851: a memory answer named a channel other than the one requested")
...
func (e *AnswerMismatchError) Error() string {
	return fmt.Sprintf("ic7851: asked about %s and was answered about %s — the memory-answer matcher is envelope-only, so this is the driver's check (tier ruling T2)", e.Want, e.Got)
}
...
// error.
func (d *ic7851Driver) Open(ctx context.Context, port transport.Port, id driver.Identity) (driver.Session, error) {
	...
	// sessions.
	framing, err := civ.NewFraming(civic7851.Profile())
	if err != nil {
		_ = port.Close()
		return nil, fmt.Errorf("ic7851: framing: %w", err)
	}
	...
	if err != nil {
		...
		_ = port.Close()
		return nil, fmt.Errorf("ic7851: Open: %w", err)
	}
...
// one place.
func (d *ic7851Driver) open(ctx context.Context, eng *transport.Engine, stats civ.AccumulatorStatsReporter, id driver.Identity) (*Session, error) {
	p := civic7851.Profile()
	...
	if err != nil {
		return nil, fmt.Errorf("ic7851: Open: building the 19 00 read: %w", err)
	}
...
func probeSlot(ctx context.Context, eng *transport.Engine, p civ.Profile, a civ.ChannelAddress) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("ic7851: Open: building the 1A 00 read for %s: %w", a, err)
	}
	...
	if err != nil {
		return nil, false, fmt.Errorf("ic7851: Open: probing %s: %w", a, err)
	}
	got, raw, err := p.MemoryAnswerRecord(frame)
	if err != nil {
		var lengthErr *civ.RecordLengthError
		if errors.As(err, &lengthErr) {
			return nil, false, &RecordLengthMismatchError{Err: lengthErr, Got: lengthErr.Got, Want: civic7851.RecordOnlyLength, Slot: a}
		}
		return nil, false, fmt.Errorf("ic7851: Open: probing %s: %w", a, err)
	}
	if got != a {
...
var (
	_ driver.Driver              = (*ic7851Driver)(nil)
