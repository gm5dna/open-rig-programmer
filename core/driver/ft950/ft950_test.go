...
// phase" (matrix §4: no 60m/EMG bank to probe for): AI0 + ID and nothing
// else.
func TestOpen_SendsExactlyTwoFrames(t *testing.T) {
	...
	if got, want := p.Transcript(), []string{"AI0;", "ID;"}; !reflect.DeepEqual(got, want) {
		t.Errorf("Open's transcript = %v, want exactly %v — this radio has nothing to discover (matrix §4), so Open sends the AI0 preamble and the identity probe and NOTHING more", got, want)
	}
}
