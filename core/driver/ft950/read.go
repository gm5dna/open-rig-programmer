...
func (s *Session) ReadChannel(ctx context.Context, slot string) (codeplug.Channel, error) {
	...
	if err != nil {
		return codeplug.Channel{}, fmt.Errorf("ft950: ReadChannel %s: %w: %w", sl.Wire(), driver.ErrRecordDecode, err)
	}
	...
	if !kindAccepted(m.Kind) {
		return codeplug.Channel{}, &KindMismatchError{Model: "ft950", Slot: sl.Wire(), Got: m.Kind, Want: acceptedKinds}
	}
