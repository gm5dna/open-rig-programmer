...
func TestWriteChannel_Succeeds(t *testing.T) {
	...
	if got := mw[21:23]; got != "00" {
		t.Errorf("MW P9 = %q, want \"00\" — this radio's P9 is printed-fixed on write regardless of profile", got)
	}
}

func TestWriteChannel_KnownToneIsRefusedBeforeAnyWireTraffic(t *testing.T) {
	p, s := openSession(t, Simulated, slotImage{})
	ch := populatedChannel("001")
	ch.Data.CTCSSTone = codeplug.ToneField{State: codeplug.Known, Value: s.caps.CTCSSTones[10]}

	if _, err := s.WriteChannel(testCtx(t), ch); err == nil {
		t.Fatal("WriteChannel with a Known CTCSSTone succeeded, want a refusal — matrix §1.3: FieldCTCSSTone's write side is Unsupported on this radio")
	}
	transcript := p.Transcript()
	if len(transcript) != 2 { // AI0, ID only — no MW sent
		t.Errorf("transcript = %v, want [AI0;, ID;] only — the refusal must precede any MW frame", transcript)
	}
}
