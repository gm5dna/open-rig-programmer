...
func TestReadChannel_Populated(t *testing.T) {
	...
	if ch.Data.CTCSSTone.State != codeplug.Known {
		t.Errorf("CTCSSTone.State = %v, want Known — matrix §1.3: P9 is live on read", ch.Data.CTCSSTone.State)
	}
...
func TestReadChannel_LiveToneIndexDecodes(t *testing.T) {
	f := mrAnswerFields{
		slot: "003", freq: "14250000", clarSign: '+', clarMag: "0000",
		rxClar: '0', txClar: '0', mode: '2', kind: '1', ctcss: '0',
		tone: "26", shift: '0',
	}.frame()
	_, s := openSession(t, Simulated, slotImage{mrAnswers: map[string]string{"003": f}})

	ch, err := s.ReadChannel(testCtx(t), "003")
	if err != nil {
		t.Fatalf("ReadChannel: %v", err)
	}
	if ch.Data.CTCSSTone.Value != s.caps.CTCSSTones[26] {
		t.Errorf("CTCSSTone.Value = %v, want chart entry 26 (%v)", ch.Data.CTCSSTone.Value, s.caps.CTCSSTones[26])
	}
}

func TestReadChannel_AnswerMismatchRefuses(t *testing.T) {
	// The radio answers slot 001's read with a frame naming a different
	// slot — the corruption this project always refuses.
	f := mrAnswerFields{
