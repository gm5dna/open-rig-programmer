...
// testCtxTimeout is small: this radio's Open is AI0 + one ID probe and
// nothing else (params.Probe is the zero value — no 5xx/EMG bank), so
// there is no discovery walk to budget wall clock for.
const testCtxTimeout = 5 * time.Second

// Compile-time proof of the seams this package satisfies.
var (
	_ driver.Driver              = (*ftdx3000Driver)(nil)
...
var testIdentity = driver.Identity{Port: "scripted-pipe", USBSerial: "SIM0462"}
...
func openSession(t *testing.T, profile Profile, img slotImage, opts ...Option) (*respondingPort, *Session) {
	...
	s, ok := sess.(*Session)
	if !ok {
		t.Fatalf("Open returned a %T, want *ftdx3000.Session", sess)
	}
	return p, s
}
...
func TestDriver_ModelAndCapabilities(t *testing.T) {
	...
	if caps.CATID != catID {
		t.Errorf("Capabilities().CATID = %q, want %q", caps.CATID, catID)
	}
...
func TestOpen_Succeeds(t *testing.T) {
	...
	if got := s.Identity().CATID; got != catID {
		t.Errorf("Identity().CATID = %q, want %q", got, catID)
	}
	// AI0 + ID only — no discovery sweep.
...
func TestOpen_ForeignCATIDRefuses(t *testing.T) {
	...
	if wrong.WantModel != modelName {
		t.Errorf("WantModel = %q, want %q", wrong.WantModel, modelName)
	}
	if wrong.GotModel != "" {
		t.Errorf("GotModel = %q, want \"\" — this package names only its own radio", wrong.GotModel)
	}
}

func TestOpen_UnconfiguredDialectRefusesToOpen(t *testing.T) {
	p := newRespondingPort(t, slotImage{catID: catID})
	d := &ftdx3000Driver{Base: driver.Base{Profile: Simulated}} // zero dialect, deliberately not via New
