...
var ErrUnmappedRegion = errors.New("ic7800: the slot's unmapped record regions differ from this profile's Fixed template")
...
var ErrOutOfDomain = errors.New("ic7800: a Known value lies outside what this radio's record can encode")
...
func (e *OutOfDomainError) Error() string {
	return fmt.Sprintf(
		"ic7800: %s = %d is outside what this radio's record can encode (maximum %d) — refused by the driver. This is a DRIVER-level refusal, not the outbound gate: civ.FieldSpan carries no numeric domain, so the gate offers no such check (see doc.go, the deferred gate-domain gap)",
...
func (s *Session) WriteChannel(ctx context.Context, ch codeplug.Channel) (driver.WriteResult, error) {
	...
	if ch.Data == nil {
		return refused(ch.Slot, []spec.Field{spec.FieldErase},
			"this radio's memory-clear forms are printed in its document but no IC-7800 has ever been asked to use one, so FieldErase carries the zero FieldSupport and consent structurally never reaches it (spec D4 \"Erase\"; matrix §3.13)")
	}
	d := *ch.Data

	a, bank, err := slotToAddress(ch.Slot)
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}}, fmt.Errorf("ic7800: WriteChannel: %w", err)
	}
	...
	if len(unwritable) > 0 {
		return refused(ch.Slot, unwritable,
			"this session cannot write these fields on bank "+string(bank)+": either no IC-7800 has ever been written to by this project and no consent was recorded, or the field is one ruling E6 leaves UNMAPPED (scan_skip, data_mode) and therefore one this driver refuses rather than collapses 4->2")
	}
	...
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}},
			fmt.Errorf("ic7800: WriteChannel %s: the pre-write preservation read: %w", ch.Slot, err)
	}
	...
	// (register entry ic7800-full-record-mandatory, matrix lift R15: the
	...
	cmd, err := civic7800.Profile().BuildMemorySet(rec)
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}},
			fmt.Errorf("ic7800: WriteChannel %s: building the 1A 00 set: %w", ch.Slot, err)
	}
	...
	switch {
	case err == nil:
		// FB arrived. Register entry ic7800-1a00-set-ack, matrix lift
	...
	case errors.Is(err, transport.ErrRejected):
		...
		return driver.WriteResult{Steps: steps},
			fmt.Errorf("ic7800: WriteChannel %s: the radio rejected the %s set: %w", ch.Slot, memorySetStep, err)
	...
	default:
		...
		return driver.WriteResult{Steps: steps},
			fmt.Errorf("ic7800: WriteChannel %s: the %s set was written and never acknowledged, so its outcome is unknown: %w", ch.Slot, memorySetStep, err)
	}
}
...
func missingMandatory(d codeplug.ChannelData) (spec.Field, string) {
	...
	if len(d.Tag) > civic7800.Profile().NameLength() {
		return spec.FieldTag, fmt.Sprintf("the tag is %d characters and this radio's name span ⑱~㉗ holds %d", len(d.Tag), civic7800.Profile().NameLength())
	}
	return "", ""
}
...
// two nibbles are SWAPPED relative to IC-7610's own byte (11)), and IS
// mapped.
//
// The template is asked for by name (civic7800.FixedTemplate) rather than
...
func unmappedRegionsDiffer(raw []byte) error {
	...
	if len(raw) < len(tmpl) {
		...
		return fmt.Errorf("ic7800: internal: the E6 comparison was handed a %d-byte record where the profile declares %d — the length fingerprint should have refused this answer before it reached here", len(raw), len(tmpl))
	}
	for _, chk := range []struct {
		...
		{civic7800.SelectNibbleOffset, "high", func(b byte) byte { return b >> 4 }},
		{civic7800.SelectNibbleOffset, "low", func(b byte) byte { return b & 0x0F }},
		{civic7800.DataModeNibbleOffset, "low", func(b byte) byte { return b & 0x0F }},
	} {
...
// because core/civ/ic7800's enum tables are unexported, and
