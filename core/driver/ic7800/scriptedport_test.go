...
// respondingPort makes the same argument). This package has no
// internal/fakeic7800 radio-state simulator (brief: driver tests use an
// in-package scripted port, not internal/fake<model>) — every scenario
// below, including full round-trip and end-to-end behaviour, is built on
// this table instead. A table can serve deliberately WRONG answers — a
// reply from another address, a broadcast, a record at a length this radio
// does not use, an answer naming a channel nobody asked for — which is
// exactly what the error paths need and what a self-consistent fake would
// never produce.
//
...
// OBSERVED RADIO TRANSCRIBED. No IC-7800 has ever been connected to this
...
type radioImage struct {
	...
	// source-address check). Zero means 0x6A.
	setFrom byte
}
...
func (p *scriptedPort) startFlood(to byte) {
	...
	go func() {
		...
		// fate one layer down.
		frame := []byte{civ.PreambleByte, civ.PreambleByte, to, 0x6A, 0x19, 0x00, 0x6A, civ.EndByte}
...
//
//	1A 00 read : FE FE 6A E0 1A 00 <ch-hi> <ch-lo> FD          — 9 bytes
//	1A 00 set  : FE FE 6A E0 1A 00 <ch-hi> <ch-lo> <25> FD     — 34 bytes
//	19 00 read : FE FE 6A E0 19 00 FD                          — 7 bytes
const (
	memReadFrameLen = 9
	memSetFrameLen  = 34
	idReadFrameLen  = 7
...
func memAnswerFrame(ch int, rec []byte) []byte {
	...
	out := []byte{civ.PreambleByte, civ.PreambleByte, civ.ControllerAddressDefault, 0x6A, civ.CmdMemory, civ.SubMemoryContents, hi, lo}
	out = append(out, rec...)
