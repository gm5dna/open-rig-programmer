...
func New(profile Profile, opts ...Option) driver.Driver {
	d := &ic7800Driver{Base: driver.Base{Profile: profile}}
...
type Option func(*ic7800Driver)

// WithConsentedUnverifiedWrites records the user's consent to writes this
// project has never verified against an IC-7800.
//
...
func WithConsentedUnverifiedWrites() Option {
	return func(d *ic7800Driver) { d.Consented = true }
}

// ic7800Driver implements driver.Driver for the Icom IC-7800.
type ic7800Driver struct {
	driver.Base
}

// Model implements driver.Driver. It must equal Capabilities().Model and
// the Wave-4 registry key.
func (d *ic7800Driver) Model() string { return "IC-7800" }

// Capabilities implements driver.Driver: the STATIC baseline for this
// driver's profile, before any radio has been probed.
func (d *ic7800Driver) Capabilities() spec.Capabilities {
	switch d.Profile {
	case Simulated:
...
// OpenReport is what Open observed while probing, kept on the ic7800
...
func (e *RecordLengthMismatchError) Error() string {
	return fmt.Sprintf(
		"ic7800: %s answered a %d-byte memory record, want %d — the expected length is itself an ASSUMED derivation from one document (D5 entry 6, matrix lift R6), and this refusal names no other model because cross-model record-length distinctness is a Wave-4 tier check",
		e.Slot, e.Got, e.Want)
}
...
func probeSlot(ctx context.Context, eng *transport.Engine, p civ.Profile, a civ.ChannelAddress) ([]byte, bool, error) {
	...
	if err != nil {
		...
		if errors.As(err, &lengthErr) {
			return nil, false, &RecordLengthMismatchError{Err: lengthErr, Got: lengthErr.Got, Want: civic7800.RecordOnlyLength, Slot: a}
		}
		return nil, false, fmt.Errorf("ic7800: Open: probing %s: %w", a, err)
	}
	if got != a {
...
// seams it claims.
var (
	_ driver.Driver                = (*ic7800Driver)(nil)
	_ driver.SerialFramingReporter = (*ic7800Driver)(nil)
...
//
// ASSUMED, ON NO EVIDENCE FROM THIS RADIO'S DOCUMENT. The IC-7800 CI-V
...
//
// LIFT R8 — Stage R capture ic7800-framing-8n1: with an IC-7800 at its
// factory CI-V settings, open its USB CI-V endpoint at 8-N-1 and then at
// 8-N-2, send FE FE 6A E0 19 00 FD at each, and record which framing
...
// 8-N-2 either.
func (d *ic7800Driver) StopBits() int { return 1 }
