...
// goldenRecord is the 25-byte record derived by hand from the matrix and
// committed as core/civ/ic7800/testdata/ic7800-vectors.golden's
...
//	offset  6      01            USB
//	offset  7      01            FIL1
//	offset  8      10            hi = TONE, lo = data mode OFF (E6) —
//	                             UNLIKE the IC-7610 family, this byte's
//	                             nibbles are SWAPPED (matrix §1b)
...
var (
	idReadFrame = []byte{0xFE, 0xFE, 0x6A, 0xE0, 0x19, 0x00, 0xFD}
)

func memReadFrame(ch int) []byte {
	hi, lo := encodeChannel(ch)
	return []byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00, hi, lo, 0xFD}
}
	...
	for _, tt := range []struct {
		...
		t.Run(tt.want, func(t *testing.T) {
			...
			if got := s.Identity().CATID; got != tt.want {
				t.Errorf("Identity().CATID = %q, want %q (the static address 6a followed by the recorded token)", got, tt.want)
			}
...
func TestOpen_FingerprintConfirmsOn25(t *testing.T) {
	for _, occupied := range []int{1, 4, 10} {
		t.Run("occupied slot", func(t *testing.T) {
			img := radioImage{idToken: []byte{0x6A}, records: map[int][]byte{occupied: goldenRecord}}
			p := newScriptedPort(t, img)
			s := openWith(t, p)
			length, confirmed := s.Fingerprint()
			if !confirmed || length != civic7800.RecordOnlyLength {
				t.Errorf("Fingerprint() = (%d, %v), want (%d, true)", length, confirmed, civic7800.RecordOnlyLength)
			}
			r := s.OpenDiagnostics()
...
func TestOpen_EmptyRadioOpensUnfingerprinted(t *testing.T) {
	p := newScriptedPort(t, radioImage{idToken: []byte{0x6A}})
	s := openWith(t, p)
...
func TestOpen_WrongLengthRefusesWithoutAttribution(t *testing.T) {
	for _, n := range []int{24, 26, 27, 39} {
		t.Run("record length", func(t *testing.T) {
			...
			img := radioImage{idToken: []byte{0x6A}, records: map[int][]byte{1: rec}}
			p := newScriptedPort(t, img)
			d := New(RealHardware)
			sess, err := d.Open(t.Context(), p.Port(), driver.Identity{})
			if err == nil {
			...
			if mismatch.Want != civic7800.RecordOnlyLength {
				t.Errorf("*RecordLengthMismatchError.Want = %d, want %d", mismatch.Want, civic7800.RecordOnlyLength)
			}
			if n == 24 {
				drivertest.AssertRecordLengthMismatch(t, mismatch, mismatch.Got, mismatch.Want,
					"ic7800: g0/ch1 answered a 24-byte memory record, want 25 — the expected length is itself an ASSUMED derivation from one document (D5 entry 6, matrix lift R6), and this refusal names no other model because cross-model record-length distinctness is a Wave-4 tier check")
			}
...
// plainly (spec D3.3). This driver builds every frame for 0x6A and the
// codec refuses any answer not from 0x6A, so a radio at a different CI-V
...
func TestOpen_WrongModelAtAnotherAddressJustTimesOut(t *testing.T) {
	...
	if !errors.Is(err, transport.ErrTimeout) {
		t.Errorf("err = %v, want a timeout - a radio off 0x6A is indistinguishable from silence", err)
	}
	if errors.Is(err, driver.ErrWrongRadio) {
...
func TestCapabilitiesSwitch_FailsSafeOnAnUnrecognisedProfile(t *testing.T) {
	...
	if got := New(RealHardware).Model(); got != "IC-7800" {
		t.Errorf("Model() = %q, want IC-7800", got)
	}
}
