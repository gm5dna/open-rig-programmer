...
// contiguous space with three printed forms — and core/civ/ic7800's
...
func slotToAddress(slot string) (civ.ChannelAddress, spec.BankID, error) {
	...
	if len(slot) != 3 {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7800: %q is not a slot on this radio: a memory is three digits (\"001\"..\"099\") and a scan edge is \"P1\" or \"P2\"", slot)
	}
	n, err := strconv.Atoi(slot)
	if err != nil {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7800: %q is not a slot on this radio: %w", slot, err)
	}
	if n < 1 || n > lastMemoryChannel {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7800: %q is outside this radio's memory range \"001\"..\"099\"", slot)
	}
...
func (s *Session) readRaw(ctx context.Context, a civ.ChannelAddress) (civ.MemoryRecord, []byte, bool, error) {
	...
	if err != nil {
		return civ.MemoryRecord{}, nil, false, fmt.Errorf("ic7800: building the 1A 00 read for %s: %w", a, err)
	}
	...
	if got != a { // T2, BEFORE any use of raw
		...
		return civ.MemoryRecord{}, nil, false, &AnswerMismatchError{Model: "ic7800", Requested: a, Answered: got}
	}
...
func (s *Session) ReadChannel(ctx context.Context, slot string) (codeplug.Channel, error) {
	...
	if err != nil {
		return codeplug.Channel{}, fmt.Errorf("ic7800: ReadChannel: %w", err)
	}
	rec, _, empty, err := s.readRaw(ctx, a)
	if err != nil {
		return codeplug.Channel{}, fmt.Errorf("ic7800: ReadChannel %s: %w", slot, err)
	}
	if empty {
	...
	if !ok {
		...
		return codeplug.Channel{}, fmt.Errorf("ic7800: ReadChannel %s: the record carries no frequency", slot)
	}
	...
	if !ok {
		return codeplug.Channel{}, fmt.Errorf("ic7800: ReadChannel %s: the record carries no mode", slot)
	}
	name, _ := rec.Name.Get()
	...
	data := &codeplug.ChannelData{
		...
		// decode them: byte 0's low nibble is a four-valued SELECT-group
		// marker and byte 8's LOW nibble a four-valued data mode — the
		// OPPOSITE nibble to the IC-7610 family's equivalent byte, matrix
		// §1b — and both neutral homes are BoolField. E6 unmaps them; it
		// does NOT make the channel unreadable — WriteChannel is where
		// such a channel becomes unwritable.
