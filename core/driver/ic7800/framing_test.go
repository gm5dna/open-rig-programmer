...
// reporter an IC-7800 would open at 8-N-2 against the tier's assumed
...
func TestDriverImplementsSerialFramingReporter(t *testing.T) {
	...
	if !ok {
		t.Fatal("the IC-7800 driver does not implement driver.SerialFramingReporter - spec D3.1 requires every Icom driver to report its framing")
	}
...
//
// The IC-7800's CI-V Reference Guide says NOTHING about serial framing —
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			wants: []string{
				...
				"say nothing about serial framing",
				// The trap row that must never be read as CI-V evidence.
				"Decode Baud Rate",
				"must never be read as a framing",
				// The register entry and the tier convention.
				"ic7800-framing-8n1",
				"ASSUMED 8-N-1",
			},
		},
