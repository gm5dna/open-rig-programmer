...
// the same seven core/civ/ic7800's layout() carries spans for, in that
...
// field class by field class from an IC-7800's OWN trial evidence, AND the
...
//
// THE IC-7800 HAS NO REGISTERED SIBLING (matrix S4), so there is one
// constant and one row, not a pair.
func TestWriteTrialsComplete_PinnedFalse(t *testing.T) {
	if writeTrialsComplete {
		t.Fatal("writeTrialsComplete is true - no IC-7800 has ever answered a frame; see doc.go")
	}
}
...
func TestBaseline_Shape(t *testing.T) {
	...
	if caps.Model != "IC-7800" {
		t.Errorf("Model = %q, want %q", caps.Model, "IC-7800")
	}
	...
	if caps.TagLen != civic7800.Profile().NameLength() {
		t.Errorf("TagLen %d and the codec's NameLength %d disagree", caps.TagLen, civic7800.Profile().NameLength())
	}
	if caps.TagCharset != civic7800.NameCharset {
	...
	for _, b := range caps.Banks {
		if b.NoBlank {
			t.Errorf("bank %s has NoBlank true - nothing in this document says an IC-7800 memory or scan edge must stay populated", b.ID)
		}
	...
	if len(caps.RequiredSlots) != 0 {
		t.Errorf("RequiredSlots = %v, want empty - no IC-7800 slot is documented as un-erasable", caps.RequiredSlots)
	}
	...
	// OQ2 - ASSUMED, register home ic7800-default-baud, matrix lift R11.
...
// codecEnum returns the enum map core/civ/ic7800's layout carries for f.
//
...
func codecEnum(t *testing.T, f civ.FieldID) map[byte]string {
	t.Helper()
	layouts := civic7800.Profile().Layouts()
	if len(layouts) != 1 {
...
//
// caps.go writes the ten names out because core/civ/ic7800's modeEnum is
...
func TestBaseline_WrittenDownZeroes(t *testing.T) {
	zeroes := []struct {
		...
		{spec.FieldErase, "spec D4 Erase: the wire form EXISTS on this radio, in two shapes, and NO IC-7800 has ever been asked to use one; spec.ConsentUnverifiedWrites structurally never consents this field"},
		...
		{spec.FieldDTCSCode, "DTCS is printed nowhere in this document's 230 pages"},
		{spec.FieldDTCSPolarity, "DTCS is printed nowhere in this document's 230 pages"},
		{spec.FieldScanSkip, "RULING E6: byte 0's LOW nibble is a four-valued SELECT-group marker (0=OFF/1=*1/2=*2/3=*3) and codeplug.ChannelData.ScanSkip is a BoolField; a 4->2 collapse would rewrite a user's SELECT group, so the nibble is UNMAPPED and the field is Unsupported both ways. On Icom, scan_skip is SELECT-group membership, never a skip"},
		{spec.FieldDataMode, "RULING E6: byte 8's LOW nibble is a four-valued data mode (0=OFF/1=DATA 1/2=DATA 2/3=DATA 3) and codeplug.ChannelData.DataMode is a BoolField - the OPPOSITE nibble to the IC-7610 family's equivalent byte (matrix §1b); same collapse, same ruling, same UNMAPPED outcome"},
	}
	for name, caps := range bothProfiles() {
...
// lift R18.
//
// UNLIKE THE IC-7610'S OWN SCAN GRID, this document prints NO "set 0 for
// P1 and P2" note anywhere near byte 0 or the 0100/0101 value list
// (matrix §2): the matrix therefore grades scan_skip write-SUPPORTED on
// SCAN for this model, a genuine per-model difference from the IC-7610.
// It makes no difference here: ruling E6 unmaps the SELECT-group nibble
// on structural grounds alone (a four-valued wire field has no faithful
// BoolField home), independent of whatever note is or is not printed
// beside it, so the field stays Unsupported on BOTH banks regardless.
func TestScanBank_MatchesTheMatrixPerCell(t *testing.T) {
	for name, caps := range bothProfiles() {
		...
		if caps.FieldSupport(spec.BankScan, spec.FieldScanSkip) != (spec.FieldSupport{}) {
			t.Errorf("%s: SCAN/scan_skip is not the zero FieldSupport - E6 unmaps it on both banks regardless of what the matrix's own per-cell grid says (this document prints no \"set 0 for P1/P2\" note, unlike the IC-7610's)", name)
		}
	}
}

// TestUnverified_NothingIsWritable: under capabilitiesUnverified, NOTHING
// this radio has is writable. No IC-7800 has ever answered a frame, so a
