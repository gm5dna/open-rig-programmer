...
func recordAtFrequency(t *testing.T, hz uint64) []byte {
	t.Helper()
	cmd, err := civic7800.Profile().BuildMemorySet(civ.MemoryRecord{
...
// one contiguous space that core/civ/ic7800's profile declares as
...
//
// Register entries ic7800-mode-code-completeness (matrix lift R19: that
// the ten printed codes are the complete set, so 06 and 09-11 name
// nothing) and ic7800-filter-value-set (that byte ⑩ carries only 01, 02
...
// byte 0 is 0x02 (SELECT ★2) or whose byte 8 is 0x12 (TONE + DATA 2) still
...
func TestReadChannel_UnmappedNibblesAreNotSmuggledThrough(t *testing.T) {
	for _, tt := range []struct {
		...
		{"byte 8 = 0x12, TONE + DATA 2", withRecord(8, 0x12)},
		{"byte 8 = 0x13, TONE + DATA 3", withRecord(8, 0x13)},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			if ch.Data.ToneMode != (codeplug.StringField{State: codeplug.Known, Value: "TONE"}) {
				t.Errorf("ToneMode = %+v, want Known TONE - byte 8's HIGH nibble IS mapped, whatever its low nibble carries", ch.Data.ToneMode)
			}
		})
	}
}
