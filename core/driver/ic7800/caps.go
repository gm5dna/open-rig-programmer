...
// FALSE.
//
// FALSE BECAUSE NO IC-7800 HAS EVER BEEN ASKED ANYTHING BY THIS PROJECT
...
// IC-7800 CI-V Reference Guide rev 4 and from nothing else.
//
...
//     from an IC-7800's OWN trial evidence — not from this model's manual,
//     and never from another model's capture, AND
//  3. the Capabilities switch in ic7800.go rewritten to select it,
//
...
// capture from either is never evidence about the other; the IC-7800 has
...
// simulator's, whose Supported writes are a claim about internal/fakeic7800
...
const (
	...
	//
	// The STORABLE ceiling — what an IC-7800 will actually keep in a
	...
	// §1 row 8 / PDF p.209's six rotated nibble labels: 100 Hz: 0–2,
...
var deliberatelyZero = map[string]string{
	...
	"RequiredSlots":          "nothing in this document says any IC-7800 memory or scan edge must stay populated. RequiredSlots is the per-SLOT mechanism and this radio uses none of it",
	...
	"TuningSteps":            "additions design D8 — the IC-7800 record carries no receiver tuning-step field",
	"ProgramTuningStepRange": "additions design D8 — the IC-7800 record carries no programmable tuning-step field",
	"AttenuatorDB":           "additions design D8 — the IC-7800 record carries no attenuator field",
	"PreampOptions":          "additions design D8 — the IC-7800 record carries no preamp field",
	"AntennaOptions":         "additions design D8 — the IC-7800 record carries no antenna field",
	"SimplexTx":              "this row does not grade FieldTxFrequency, so the blank arm leaves nothing to state: core/csvio's CHIRP importer leaves TxFreqHz Unknown on a blank Duplex row exactly as it did before the datum existed",
	...
	"NoTag":                  "the IC-7800 supports channel names via the (18)~(27) ten-byte tag field; NoTag is false",
}
...
func bankFields(rw spec.FieldSupport) map[spec.Field]spec.FieldSupport {
	return map[spec.Field]spec.FieldSupport{
		// The seven the record maps — core/civ/ic7800's layout() carries a
		// FieldSpan for each and nothing else.
		spec.FieldFrequency: rw, // bytes 1-5 (printed r~i), five bytes, little-endian BCD
		spec.FieldMode:      rw, // byte 6 (printed o), the ten printed mode codes
		spec.FieldFilter:    rw, // byte 7 (printed !0), FIL1/FIL2/FIL3
		spec.FieldToneMode:  rw, // byte 8 (printed !1) HIGH nibble, OFF/TONE/TSQL
		spec.FieldToneTx:    rw, // bytes 9-11 (printed !2~!4), three bytes, big-endian BCD
		spec.FieldToneRx:    rw, // bytes 12-14 (printed !5~!7), three bytes, big-endian BCD
		spec.FieldTag:       rw, // bytes 15-24 (printed !8~@7), ten name bytes

		// RULING E6 — THE TWO UNMAPPED NIBBLES. Byte 0 (printed 'e')'s LOW
		// nibble is a four-valued SELECT-group marker (0=OFF, 1=★1, 2=★2,
		// 3=★3; matrix §2, scan_skip row) and byte 8 (printed '!1')'s LOW
		// nibble is a four-valued data mode (0=OFF, 1=DATA 1, 2=DATA 2,
		// 3=DATA 3; matrix §1b) — THE OPPOSITE nibble to the IC-7610
		// family's equivalent byte, where data mode is the HIGH nibble.
		// Their neutral homes, codeplug.ChannelData.ScanSkip and
		...
		// collapsed. This grade DIFFERS from matrix §2, which grades
		// scan_skip and data_mode Sup/Sup — the same divergence the
		// IC-7610's own caps.go records against its own matrix, for the
		// same E6 reason.
		//
		...
		// shapes (1A 00 <ch> FF, and command 0B) — and NO IC-7800 has ever
...
func baseCapabilities(memFields, scanFields map[spec.Field]spec.FieldSupport) spec.Capabilities {
	return spec.Capabilities{
		...
		// "IC-7800" throughout.
		Model: "IC-7800",
		// §3.4 — the CI-V address 0x6A, MANUAL-EVIDENCED, rendered as the
		// document prints it.
		//
		...
		Banks: []spec.Bank{
			{
				...
				// nothing in this document says an IC-7800 memory channel
				// must stay populated.
				NoBlank: false,
				Fields:  memFields,
			},
		...
		// sub-column then right): PDF p.208's "Operating mode" table,
		// corroborated at PDF p.209's "Command: 26".
		//
		// THESE ARE THE TEN VALUES OF core/civ/ic7800's modeEnum, and
		...
		// unexported and core/civ/ic7800 is frozen evidence-bound code
		...
		// ic7800-mode-code-completeness, matrix lift R19).
		//
		...
		// §1 row 5 — PDF p.208's "Filter setting" column, whose fourth and
		// fifth rows are printed "-" (matrix Errata (rev 1) erratum 6).
		// The three values of core/civ/ic7800's filterEnum, pinned against
		...
		// §1 row 8 / PDF p.209's six rotated nibble labels — and tier
		// ruling T1(2).
		//
		...
		// §3.3 / OQ2 — ASSUMED, register home ic7800-default-baud, matrix
		...
		TagLen: 10,
		// PDF p.209's two character tables, transcribed ONCE in
		// core/civ/ic7800 and referenced here. The Icom charset omits
		...
		TagCharset: civic7800.NameCharset,
	}
}
...
// manual and (from Task 14) a fake, and NO IC-7800 HAS EVER ANSWERED A
// FRAME.
//
...
// capabilitiesSimulated is the internal/fakeic7800-backed profile (CLI
...
// is a claim about internal/fakeic7800 and about nothing else.
//
