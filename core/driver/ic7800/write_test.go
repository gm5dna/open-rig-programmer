...
func TestMemorySetSpec_IsWriteWithAckAndNeverRetries(t *testing.T) {
	sp := civ.CIVWriteWithAckSpec(civic7800.Profile().AcknowledgementMatcher())
...
//
// Register entry ic7800-1a00-set-ack, matrix lift R14: THE CODES ARE
...
func TestWriteChannel_FARejectionIsReported(t *testing.T) {
	t.Run("FA from 0x6A is a rejection", func(t *testing.T) {
		img := ackingRadio()
		img.ackSets = false
		img.rejectSets = true
...
func TestWriteChannel_RefusesASlotWhoseUnmappedRegionsDiffer(t *testing.T) {
	for _, tt := range []struct {
		...
		{"byte 0 low = 2, SELECT ★2", withRecord(0, 0x02), civic7800.SelectNibbleOffset, "low", 0x02},
		{"byte 0 low = 1, SELECT ★1", withRecord(0, 0x01), civic7800.SelectNibbleOffset, "low", 0x01},
		{"byte 0 high = 1, not the printed Fixed 0", withRecord(0, 0x10), civic7800.SelectNibbleOffset, "high", 0x01},
		{"byte 8 low = 2, DATA 2", withRecord(8, 0x02), civic7800.DataModeNibbleOffset, "low", 0x02},
		{"byte 8 low = 1, DATA 1", withRecord(8, 0x01), civic7800.DataModeNibbleOffset, "low", 0x01},
	} {
		t.Run(tt.name, func(t *testing.T) {
			img := ackingRadio()
...
func TestWriteChannel_EmptySlotWritesAgainstTheTemplate(t *testing.T) {
	...
	if rec[civic7800.SelectNibbleOffset] != tmpl[civic7800.SelectNibbleOffset] {
		t.Errorf("the set's byte %d is %#02x, want the template's %#02x", civic7800.SelectNibbleOffset, rec[civic7800.SelectNibbleOffset], tmpl[civic7800.SelectNibbleOffset])
	}
	if rec[civic7800.DataModeNibbleOffset]&0x0F != tmpl[civic7800.DataModeNibbleOffset]&0x0F {
		t.Errorf("the set's byte %d low nibble is %#x, want the template's %#x", civic7800.DataModeNibbleOffset, rec[civic7800.DataModeNibbleOffset]&0x0F, tmpl[civic7800.DataModeNibbleOffset]&0x0F)
	}
}
...
func TestWriteChannel_ConsentedWritesTheFullRecord(t *testing.T) {
	...
	want := append([]byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00, 0x00, 0x42}, goldenRecord...)
...
func TestWriteChannel_NoClearFrameIsReachable(t *testing.T) {
	// (a) FE FE 6A E0 1A 00 <ch-hi> <ch-lo> FF FD, and
	// (b) FE FE 6A E0 0B FD.
...
func TestNumericRefusalIsDefenceInDepthNotTheGate(t *testing.T) {
	...
	frame := append([]byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00, 0x00, 0x42}, overRange...)
	frame = append(frame, 0xFD)

	if !civic7800.Profile().AllowedCommand(frame) {
...
// Register entry ic7800-default-tone-undocumented.
func TestWriteChannel_CreateWithoutToneIsRefused(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(refusal.Reason, "ic7800-default-tone-undocumented") {
...
func TestUnreachableRefusalsTellTheTruth(t *testing.T) {
	...
	t.Run("the tone refusal names the situation it is in", func(t *testing.T) {
		...
		if !strings.Contains(create, "no prior record") || !strings.Contains(create, "ic7800-default-tone-undocumented") {
