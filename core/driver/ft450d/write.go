...
// CTCSSTone when — and only when — its FieldState is Known (a non-Known
// tone defaults to wire index 0 rather than refusing the write, the
// ft2000/ft950 precedent this driver follows), plus the Icom tier's
...
//
// THERE IS NO FieldTag/FieldTagDisplay HERE: this radio has no CAT route
// that carries tag text (matrix §0/§2.7), and both fields carry the zero
// FieldSupport on every bank (caps.go).
//
// A caller writing a PMS slot (501-504) requests the SAME fields a MEM
// write does — the capability gate below, not this list, is what refuses
// the six shared fields on PMS (caps.go's pmsFields forces their Write to
// Unsupported there).
...
// NO MT COMMAND (matrix §0), so there is no second frame to sequence
// beside it, and MW's own P7 is write-fixed (dialect.go's MWWriteKind)
// rather than a session-supplied kind.
//
// A write to a PMS slot (501-504) is refused HERE, by the ordinary
// capability gate below — not by a bank-specific branch in this method:
// s.caps.FieldSupport(spec.BankPMS, f) reports Write: Unsupported for
// every one of the six requested fields (caps.go's pmsFields), so
// "unwritable" below is non-empty for any populated PMS write, on both
// profiles, unconditionally (the SAFE SHAPE ruling's caution, not a
...
func (s *Session) WriteChannel(ctx context.Context, ch codeplug.Channel) (driver.WriteResult, error) {
	...
	if len(unwritable) > 0 {
		return res, &driver.WriteRefusedError{
			Slot:   ch.Slot,
			Fields: unwritable,
			Reason: "not write-Supported for this session (the CAT codec cannot express the field, or this session's capability profile does not support writing it — the PMS bank, 501-504, is read-only per the SAFE SHAPE ruling until an owner probe promotes it)",
		}
	}
	...
	if _, err := s.eng.Do(ctx, cmd, fnfSpec()); err != nil {
		if errors.Is(err, cat.ErrRejected) {
			...
			return res, fmt.Errorf("ft450d: WriteChannel %s: MW rejected by radio: %w", ch.Slot, err)
		}
		return res, fmt.Errorf("ft450d: WriteChannel %s: MW: %w", ch.Slot, err)
	}
...
func (s *Session) buildMWCommand(ch codeplug.Channel) (cat.Command, error) {
	...
	// gate's driver.CheckFieldStates rung), so the lookup below cannot fail
	// on a well-formed write — but it is not re-derived here on faith: a
	// caller bypassing that gate gets a refusal naming the field, not a
	// panic or a wrong index. Non-Known leaves ToneIndex at its zero value,
	// which dialect.BuildMWSet then round-trips as tone number "00" — a
	// real, representable channel state (chart index 0), not a
	...
	// field (matrix §1.1). A value between caps.MaxFreqHz and the 9-digit
	// ceiling would pass it and then overflow encodeMemoryFields's
	// fixed-width rendering, corrupting every byte after P2 — the same gap
	// every sibling in this wave closes in its own write.go.
