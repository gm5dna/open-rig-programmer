...
// Package ic7760 is the Icom IC-7760's driver: the capability profiles,
...
//
// The wire codec lives next door in core/civ/ic7760 and is imported here
// as civic7760, exactly as core/driver/ftdx101 imports core/cat/ftdx101 as
...
// # Provenance
//
// Everything protocol-shaped here comes from the IC-7760 CI-V Reference
// Guide rev 4, through core/civ/ic7760's profile and through the
// IC-7760 capability matrix (docs/superpowers/icom-matrices/,
// ic7760-capability-matrix.md rev 1 plus its Errata). NO IC-7760 HAS EVER
...
//     These settings are set in Set mode (Refer to the IC-7760 instruction
...
// IC-7760 and the PC (controller)", with the callout "To the [USB 1] port"
// on a Type-B socket.
//
...
//
//   - THE IDENTITY STEP. A 19 00 read to B2h, with an ADDRESS-MATCHED
...
//     address other than B2h times out — this driver builds every frame
//     for B2h and the codec refuses any answer not from B2h, and there is
...
// `ic7760-default-baud` (civ PROFILE register, core/civ/ic7760/doc.go);
// matrix lift R11.
//
...
// IC-7760 instruction manual, which this project does not hold. The choice
...
// one either.
//
// WHAT A FUTURE WRITE-TRIAL MILESTONE ON THE IC-7760 WOULD NEED (matrix
// §3.13, reproduced verbatim): all of the following, on an IC-7760 Stuart
...
// `ic7760-mode-code-radix` in core/civ/ic7760/doc.go. Reversing the ruling
// is the orchestrator's to do, not an editor's, and
// core/civ/ic7760/crosscheck_test.go says so at the failure site.
//
// # The ic7760 DRIVER register
//
// Six entries, homed HERE, each with its capture block reproduced from the
// matrix. The civ-homed entries live in core/civ/ic7760/doc.go and are
// LISTED at the foot of this register so a reader knows the set is not
// complete without them.
//
//   - ic7760-control-lines-inert (R16) - that RTS and DTR left DEASSERTED
...
//     never toggle them.
//     STAGE R LIFTS IT WITH: capture ic7760-control-lines-inert - with
//     USB SEND, USB Keying (CW) and USB Keying (RTTY) all set to 00 (OFF)
//     from the front panel, open the IC-7760's USB CI-V endpoint with RTS
...
//
//   - ic7760-storable-frequency-ceiling (R17a) - what the radio will
...
//     STAGE W LIFTS IT WITH: capture ic7760-storable-ceiling-ch03 - on one
//     IC-7760, write memory channel 03 with 1A 00 at successively higher
...
//
//   - ic7760-storable-frequency-floor (R17b) - the same question at the
//     other end.
...
//     STAGE W LIFTS IT WITH: capture ic7760-storable-floor-ch04 - on the
...
//
//   - ic7760-scan-edge-record-fields (R18) - that a scan edge's record
...
//     STAGE W LIFTS IT WITH: capture ic7760-scan-edge-p1 - on one IC-7760
...
//
//   - ic7760-mode-code-completeness (R19) - that the ten printed mode
...
//     STAGE R LIFTS IT WITH: capture ic7760-mode-code-sweep - on one
//     IC-7760, step the Main band through every mode position the front
...
//     nothing about any other model.
//
//   - ic7760-select-marker-semantics (R20) - what byte ③'s low-nibble
...
//     STAGE W LIFTS IT WITH: capture ic7760-select-marker-ch05 - write
...
//
// These entries live in core/civ/ic7760/doc.go with their capture blocks;
// they are named here so a reader of this register knows the set is not
// complete without them: ic7760-read-request-form (R1),
// ic7760-empty-channel-fa (R2a), ic7760-empty-channel-ff (R2b),
// ic7760-name-space-character (R3), ic7760-name-pad-byte (R4),
// ic7760-wire-order (R5), ic7760-record-length (R6),
// ic7760-id-token (R7), ic7760-transceive-broadcast-address (R9),
// ic7760-transceive-factory-default (R10), ic7760-default-baud (R11),
// ic7760-civ-rate-list (R12), ic7760-usb-echo-default (R13),
// ic7760-1a00-set-ack (R14), ic7760-full-record-mandatory (R15),
// ic7760-mode-code-radix (RULING OQ1), ic7760-filter-value-set,
// ic7760-default-tone-undocumented and ic7760-e6-unmapped-regions.
//
