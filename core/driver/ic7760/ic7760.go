...
func WithTransportLogger(l transport.Logger) Option {
	return func(d *ic7760Driver) { d.transportOpts = append(d.transportOpts, transport.WithLogger(l)) }
}
...
var ErrAnswerMismatch = errors.New("ic7760: a memory answer named a channel other than the one requested")
...
func (e *AnswerMismatchError) Error() string {
	return fmt.Sprintf("ic7760: asked about %s and was answered about %s — the memory-answer matcher is envelope-only, so this is the driver's check (tier ruling T2)", e.Want, e.Got)
}
...
func (d *ic7760Driver) Open(ctx context.Context, port transport.Port, id driver.Identity) (driver.Session, error) {
	...
	// sessions.
	framing, err := civ.NewFraming(civic7760.Profile())
	if err != nil {
		_ = port.Close()
		return nil, fmt.Errorf("ic7760: framing: %w", err)
	}
	...
	if !ok {
		_ = port.Close()
		return nil, fmt.Errorf("ic7760: the CI-V framing does not report accumulator stats — this driver's diagnostics require civ.AccumulatorStatsReporter")
	}
	...
	if err != nil {
		...
		_ = port.Close()
		return nil, fmt.Errorf("ic7760: Open: %w", err)
	}
...
// one place.
func (d *ic7760Driver) open(ctx context.Context, eng *transport.Engine, stats civ.AccumulatorStatsReporter, id driver.Identity) (*Session, error) {
	p := civic7760.Profile()
	...
	if err != nil {
		return nil, fmt.Errorf("ic7760: Open: building the 19 00 read: %w", err)
	}
...
func probeSlot(ctx context.Context, eng *transport.Engine, p civ.Profile, a civ.ChannelAddress) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("ic7760: Open: building the 1A 00 read for %s: %w", a, err)
	}
