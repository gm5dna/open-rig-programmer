...
func probeSlot(ctx context.Context, eng *transport.Engine, p civ.Profile, a civ.ChannelAddress) ([]byte, bool, error) {
	...
	if err != nil {
		...
		if errors.As(err, &lengthErr) {
			return nil, false, &RecordLengthMismatchError{icom.RecordLengthMismatchError{Err: lengthErr, Got: lengthErr.Got, Want: civic7760.RecordOnlyLength, Slot: a}}
		}
		return nil, false, fmt.Errorf("ic7760: Open: probing %s: %w", a, err)
	}
	if got != a {
		return nil, false, &AnswerMismatchError{Model: "ic7760", Requested: a, Answered: got}
	}
