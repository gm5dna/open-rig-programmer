...
// the same seven core/civ/ic7760's layout() carries spans for, in that
...
// field class by field class from an IC-7760's OWN trial evidence, AND the
...
//
// THE IC-7760 HAS NO REGISTERED SIBLING (matrix S4), so there is one
// constant and one row, not a pair.
func TestWriteTrialsComplete_PinnedFalse(t *testing.T) {
	if writeTrialsComplete {
		t.Fatal("writeTrialsComplete is true - no IC-7760 has ever answered a frame; see doc.go")
	}
}
...
func TestBaseline_Shape(t *testing.T) {
	...
	if caps.Model != "IC-7760" {
		t.Errorf("Model = %q, want %q", caps.Model, "IC-7760")
	}
	...
	if caps.TagLen != civic7760.Profile().NameLength() {
		t.Errorf("TagLen %d and the codec's NameLength %d disagree", caps.TagLen, civic7760.Profile().NameLength())
	}
	if caps.TagCharset != civic7760.NameCharset {
	...
	for _, b := range caps.Banks {
		if b.NoBlank {
			t.Errorf("bank %s has NoBlank true - nothing in this document says an IC-7760 memory or scan edge must stay populated", b.ID)
		}
...
// codecEnum returns the enum map core/civ/ic7760's layout carries for f.
//
...
func codecEnum(t *testing.T, f civ.FieldID) map[byte]string {
	t.Helper()
	layouts := civic7760.Profile().Layouts()
	if len(layouts) != 1 {
...
//
// caps.go writes the ten names out because core/civ/ic7760's modeEnum is
...
func TestBaseline_WrittenDownZeroes(t *testing.T) {
	zeroes := []struct {
		...
		{spec.FieldErase, "spec D4 Erase: the wire form EXISTS on this radio, in two shapes, and NO IC-7760 has ever been asked to use one; spec.ConsentUnverifiedWrites structurally never consents this field"},
...
// this radio has is writable. No IC-7760 has ever answered a frame, so a
