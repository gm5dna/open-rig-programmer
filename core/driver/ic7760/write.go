...
func (e *OutOfDomainError) Error() string {
	return fmt.Sprintf(
		"ic7760: %s = %d is outside what this radio's record can encode (maximum %d) — refused by the driver. This is a DRIVER-level refusal, not the outbound gate: civ.FieldSpan carries no numeric domain, so the gate offers no such check (see doc.go, the deferred gate-domain gap)",
