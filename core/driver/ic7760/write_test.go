...
func TestMemorySetSpec_IsWriteWithAckAndNeverRetries(t *testing.T) {
	sp := civ.CIVWriteWithAckSpec(civic7760.Profile().AcknowledgementMatcher())
...
func TestWriteChannel_FARejectionIsReported(t *testing.T) {
	t.Run("FA from 0xB2 is a rejection", func(t *testing.T) {
		img := ackingRadio()
		img.ackSets = false
		img.rejectSets = true
...
func TestWriteChannel_RefusesASlotWhoseUnmappedRegionsDiffer(t *testing.T) {
	for _, tt := range []struct {
		...
		{"byte 0 low = 2, SELECT ★2", withRecord(0, 0x02), 0, "low", 0x02},
		{"byte 0 low = 1, SELECT ★1", withRecord(0, 0x01), 0, "low", 0x01},
		{"byte 0 high = 1, not the printed Fixed 0", withRecord(0, 0x10), 0, "high", 0x01},
		{"byte 8 high = 2, DATA 2", withRecord(8, 0x21), 8, "high", 0x02},
		{"byte 8 high = 1, DATA 1", withRecord(8, 0x11), 8, "high", 0x01},
	} {
		t.Run(tt.name, func(t *testing.T) {
			img := ackingRadio()
...
func TestWriteChannel_EmptySlotWritesAgainstTheTemplate(t *testing.T) {
	...
	if rec[0] != tmpl[0] {
		t.Errorf("the set's byte %d is %#02x, want the template's %#02x", 0, rec[0], tmpl[0])
	}
	if rec[8]>>4 != tmpl[8]>>4 {
		t.Errorf("the set's byte %d high nibble is %#x, want the template's %#x", 8, rec[8]>>4, tmpl[8]>>4)
	}
}
...
func TestWriteChannel_ConsentedWritesTheFullRecord(t *testing.T) {
	...
	want := append([]byte{0xFE, 0xFE, 0xB2, 0xE0, 0x1A, 0x00, 0x00, 0x42}, goldenRecord...)
...
func TestNumericRefusalIsDefenceInDepthNotTheGate(t *testing.T) {
	...
	frame := append([]byte{0xFE, 0xFE, 0xB2, 0xE0, 0x1A, 0x00, 0x00, 0x42}, overRange...)
	frame = append(frame, 0xFD)

	if !civic7760.Profile().AllowedCommand(frame) {
