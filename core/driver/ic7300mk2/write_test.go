...
// The SCAN bank's printed constraint: ③ must be zero for P1 and P2 (PDF
// p.17, "ⓘ Set 00 for P1 and P2.").
//
...
func TestWriteChannel_TimeoutIsNEVERRetransmitted(t *testing.T) {
	...
	sets := 0
	for _, f := range peer.Received() {
		if cn, sc, ok := civ.FrameCommand(f); ok && cn == 0x1A && sc == 0x00 && len(f) == 54 {
			sets++
		}
	}
	if sets != 1 {
...
func TestSplitNibbleIsUnmapped(t *testing.T) {
	...
	l := layouts[0]
	if len(l.Fixed) != 45 {
		t.Fatalf("Fixed is %d bytes, want a FULL-LENGTH template of 45 — civ's V8 permits an empty template or one of exactly Length bytes, and only the full-length one can declare the unmapped nibble", len(l.Fixed))
	}
...
func TestWriteChannel_NonKnownMandatoryFieldsAreRefused(t *testing.T) {
	for _, tc := range []struct {
		...
		{spec.FieldTag, func(d *codeplug.ChannelData) { d.Tag = "SEVENTEEN CHARS!!" }, "⑱ ~ ㉝ is sixteen bytes, and truncating would write a name the caller did not choose"},
