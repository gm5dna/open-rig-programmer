...
//
// In the FIELD the IC-7300MK2 and the IC-7300 cannot confuse each other: they
// answer at B6h and 94h, and a driver addressing one gets no answer at all
...
// ic7300Record is a 39-byte record in the IC-7300's own shape, inside that
// fake's derived vocabularies so WithChannel will store it. Its CONTENT is
...
func ic7300Record() []byte {
	...
	copy(rec[12:15], []byte{0x00, 0x12, 0x30})        // ⑮–⑰
	copy(rec[15:20], []byte{0x00, 0x00, 0x25, 0x14, 0})
	rec[20] = 0x01
	rec[21] = 0x01
	rec[22] = 0x00
	copy(rec[23:26], []byte{0x00, 0x08, 0x85})
	copy(rec[26:29], []byte{0x00, 0x12, 0x30})
	for i := 29; i < 39; i++ {
		rec[i] = 0x20 // ⑱–㉗ — the space, which is a printed name byte
	}
	return rec
}
...
func TestSibling_A39ByteRecordAtB6hIsWrongRadioProvisionally(t *testing.T) {
	...
	if wrong.GotModel != "IC-7300 (provisional)" {
		t.Errorf("GotModel = %q, want %q", wrong.GotModel, "IC-7300 (provisional)")
	}
	...
	for _, want := range []string{"39", "45", "ASSUMED"} {
...
func TestSibling_CIVRefusesTheForeignRecordLength(t *testing.T) {
	...
	if rle.Got != 39 || len(rle.Want) != 1 || rle.Want[0] != 45 {
		t.Errorf("RecordLengthError = {Want:%v Got:%d}, want {Want:[45] Got:39}", rle.Want, rle.Got)
	}
}
