...
//
// IT IS DELIBERATELY NOT internal/fakeic7300mk2. A fake radio models a radio's
...
// refused one draws FA is doc.go's `ic7300mk2-memory-set-accepted`; that a
...
// (core/civ/ic7300mk2/testdata/ic7300mk2-vectors.golden, line
// "set-record-name-with-space"): SELECT OFF and Split OFF in ③, 14.100 MHz,
// USB, FIL1, data mode OFF, tone mode OFF, 88.5 Hz on the repeater tone and
// 100.0 Hz on the tone squelch, the same frequency in the transmit block,
// and the sixteen-character name "TESTING NAME0123".
//
...
//
// THE TWO TONE SPANS CARRY DIFFERENT VALUES ON PURPOSE, and only one of them
// discriminates byte order: 88.5 Hz encodes as `00 08 85`, which
...
var populatedRecord = []byte{
	...
	0x00, 0x10, 0x00, // ⑮ ~ ⑰ — 100.0 Hz
	0x00, 0x00, 0x10, 0x14, 0x00, // ❹ ~ ⑧ — the transmit frequency
	0x01,             // ❾
	0x01,             // ❿
	0x00,             // ⓫
	0x00, 0x08, 0x85, // ⓬ ~ ⓮
...
// real IC-7300MK2 gets — nothing writable — so a test that passes
...
func channelFor(slot string) codeplug.Channel {
	return codeplug.Channel{
		...
		Data: &codeplug.ChannelData{
			...
			Tag:        "TESTING NAME0123",
			TagDisplay: codeplug.BoolField{State: codeplug.Unavailable},
			ScanSkip:   codeplug.BoolField{State: codeplug.Unavailable},

			TxFreqHz:     codeplug.FreqField{State: codeplug.Known, Value: 14_100_000},
