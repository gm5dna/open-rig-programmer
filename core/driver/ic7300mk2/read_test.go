...
func TestReadChannel_PopulatedMemorySlot(t *testing.T) {
	...
	d := ch.Data
	if d.FreqHz != 14_100_000 {
		t.Errorf("FreqHz = %d, want 14100000 — ④ ~ ⑧, five packed-BCD bytes, least significant pair first", d.FreqHz)
	}
	if d.Mode != "USB" {
		t.Errorf("Mode = %q, want %q", d.Mode, "USB")
	}
	if d.Tag != "TESTING NAME0123" {
		t.Errorf("Tag = %q, want %q — SIXTEEN bytes on this model, trimmed of its 0x20 pad, and a space INSIDE it survives", d.Tag, "TESTING NAME0123")
	}
	if d.TxFreqHz != (codeplug.FreqField{State: codeplug.Known, Value: 14_100_000}) {
		t.Errorf("TxFreqHz = %+v, want Known 14100000 — ❹ ~ ⑧ is a DISTINCT field, so a split channel round trips", d.TxFreqHz)
	}
	...
	if d.ToneTx != (codeplug.ToneField{State: codeplug.Known, Value: spec.Tone(885)}) {
		t.Errorf("ToneTx = %+v, want Known 88.5 Hz — ⑫ ~ ⑭, BCD TENTHS of a hertz. The ⑫ ~ ⑭ heading prints no encoding at all (§3.16 A6), which is register entry ic7300mk2-tone-tx-encoding", d.ToneTx)
	}
	if d.ToneRx != (codeplug.ToneField{State: codeplug.Known, Value: spec.Tone(1000)}) {
		t.Errorf("ToneRx = %+v, want Known 100.0 Hz — ⑮ ~ ⑰, a DIFFERENT value from the repeater tone, so the two spans cannot be confused with each other", d.ToneRx)
	}
...
func TestReadChannel_WrongLengthFailsTheRead(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("ReadChannel succeeded (empty = %v) on a 39-byte record — the length fingerprint is CONTINUOUS: a wrong-model session cannot be opened once and then trusted", ch.Empty())
	}
...
func TestReadChannel_ScanEdges(t *testing.T) {
	...
	for _, slot := range []string{"P1", "P2"} {
		...
		if ch.Empty() || ch.Data.FreqHz != 14_100_000 {
			t.Errorf("ReadChannel(%q) = %+v, want the same 45-byte record shape a memory channel has (ASSUMED, ic7300mk2-scan-edge-record-layout, lift MK2-R10 — §3.16 A5 never states it)", slot, ch)
		}
	}
...
func TestReadChannel_AbsentFieldsAreUnavailable(t *testing.T) {
	...
	if d.ClarHz != 0 || d.RxClar || d.TxClar {
		t.Errorf("clarifier = %d/%v/%v, want all zero — the 1A 00 record has no clarifier field (matrix §1 #6, #7)", d.ClarHz, d.RxClar, d.TxClar)
	}
	if d.CTCSS != "" {
		t.Errorf("CTCSS = %q, want empty — ctcss_state is displaced by tone_mode on Icom models (spec D4)", d.CTCSS)
	}
	if d.Shift != "" {
		t.Errorf("Shift = %q, want empty — no shift or duplex field exists on this model (matrix §1 #14)", d.Shift)
	}
...
func TestReadChannel_ZeroToneReadsBackUnknown(t *testing.T) {
	...
	if ch.Data.ToneTx != want {
		t.Errorf("ToneTx = %+v, want %+v — 0 Hz is not a tone, the declared domain starts at 1 deciHz, and a Known zero would be refused by the very next validator (ic7300mk2-zero-tone-means-unset)", ch.Data.ToneTx, want)
	}
...
// The MK2's name charset admits 0x60, whose GLYPH the document does not
// establish (§3.16 A2: the p.18 Symbols table draws the same glyph against
// 27 and 60). It round-trips as a BYTE and is never rewritten to 0x27.
//
// civ.ProfileConfig.NameCharset is a byte SET, not a glyph map, so nothing
...
func TestReadChannel_Byte0x60RoundTripsWithoutBeingRenamed(t *testing.T) {
	...
	if got := ch.Data.Tag; got != string(name) {
		t.Errorf("Tag = %q (% X), want % X — the byte at 0x60 is carried through unchanged; its GLYPH is not established and must never be silently rendered or rewritten as 0x27", got, got, name)
	}
}
