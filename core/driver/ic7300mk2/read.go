...
// unverified empty-channel answers (lift MK2-R3), and a record that
...
func (s *Session) ReadChannel(ctx context.Context, slot string) (codeplug.Channel, error) {
	...
	if !ok {
		return codeplug.Channel{}, fmt.Errorf("ic7300mk2: ReadChannel: %q is not a slot this radio has — its slots are 001..099, P1 and P2", slot)
	}
	cmd, err := s.p.BuildMemoryRead(want)
	if err != nil {
		return codeplug.Channel{}, fmt.Errorf("ic7300mk2: ReadChannel %s: %w", slot, err)
	}
	...
	if errors.Is(err, transport.ErrRejected) {
		// D5 entry 2(a), ASSUMED, lift MK2-R2: an unwritten channel answers
		// FE FE E0 B6 FA FD. An EMPTY channel — Data nil — and never an
...
func (s *Session) channelData(rec civ.MemoryRecord) (*codeplug.ChannelData, error) {
	...
	return &codeplug.ChannelData{
		...
		// The 1A 00 record has NO clarifier field (matrix §1 #6, #7), and
		...
		// shift has no field on this record at all (matrix §1 #14).
		CTCSS: "",
		Shift: "",
		...
		// ⑱ ~ ㉝ — SIXTEEN bytes on this model — trimmed of its 0x20 pad by
		// the codec. 0x60 IS A LEGAL BYTE HERE whose glyph the document does
		// not establish (§3.16 A2, plan decision D13): NameCharset is a byte
		// SET, not a glyph map, so the byte round-trips and is never
		// silently rendered or rewritten as 0x27.
		Tag: name,
		...
		// ⑫ ~ ⑭ and ⑮ ~ ⑰. The ⑫ ~ ⑭ heading prints no encoding at all
		// (§3.16 A6), which is register entry `ic7300mk2-tone-tx-encoding`,
		// lift MK2-R17.
		ToneTx: toneTx,
		ToneRx: toneRx,
...
// same domain. Registered as `ic7300mk2-zero-tone-means-unset`, lift
// `ic7300mk2-zero-tone-read`.
