...
// is itself ASSUMED (`ic7300-scan-edge-record-shape`, lift
// `ic7300-scan-edge-record`): this document never states whether a scan-edge
// record carries the same 39 bytes, so a short answer — or an FA — from
// 1A 00 01 00 would be a fact about the scan-edge bank rather than a fault,
// and the probe must not learn the length fingerprint from a record whose
...
// wherever the claim is: the IC-7300's document prints NO character format
// anywhere (matrix §3.1), and an Icom manual's "8 bit / 1 stop" line about
// the DATA/RTTY port is NOT evidence about CI-V — they are different ports
// with different jobs. internal/wiring refuses any value other than 1 or 2,
// so this is a statement rather than a hint.
func (d *ic7300Driver) StopBits() int { return 1 }
...
func (d *ic7300Driver) probeForFingerprint(ctx context.Context, eng *transport.Engine, p civ.Profile, id driver.Identity, probe *probeReport) error {
