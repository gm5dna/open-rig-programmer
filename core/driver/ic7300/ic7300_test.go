...
func TestOpen_FingerprintConfirmed(t *testing.T) {
	...
	if got, want := sess.Identity().CATID, "94:00"; got != want {
...
func TestOpen_EmptyRadioOpensOnAddressEvidence(t *testing.T) {
	...
	if got := sess.Identity().CATID; got != "94:00" {
		t.Errorf("Identity().CATID = %q, want %q — an empty radio still identified itself", got, "94:00")
	}
}

// 3. A record of the sibling's length is WrongRadioError with PROVISIONAL
// attribution naming the IC-7300MK2, and the error text says both lengths
// are ASSUMED derivations.
func TestOpen_ForeignRecordLengthIsWrongRadio(t *testing.T) {
	peer := newRespondingPort(t, withRecordOfLength(1, 45))
	...
	for _, want := range []string{"provisional", "45", "39", "ASSUMED"} {
...
func TestOpen_UnknownRecordLengthIsRefusedWithoutAttribution(t *testing.T) {
	peer := newRespondingPort(t, withRecordOfLength(1, 37))
	_, err := New(Simulated).Open(context.Background(), peer, driver.Identity{Port: "test"})
	if err == nil {
		t.Fatal("Open succeeded against a radio answering a 37-byte record — no model in this tier declares that length")
	}
	var wrong *driver.WrongRadioError
	if errors.As(err, &wrong) {
		t.Errorf("Open error is a *driver.WrongRadioError naming %q — 37 bytes is in no model's table, and this driver must claim NO model at all rather than guess one", wrong.GotModel)
	}
...
func TestStopBits(t *testing.T) {
	...
	if got := r.StopBits(); got != 1 {
		t.Errorf("StopBits() = %d, want 1 — ASSUMED at spec D5 entry 8, lift ic7300-framing. This document prints no character format anywhere (matrix §3.1); an Icom manual's \"8 bit / 1 stop\" line about the DATA/RTTY port is NOT evidence about CI-V.", got)
	}
}
...
var _ driver.SerialFramingReporter = (*ic7300Driver)(nil)

// The probe is BOUNDED and confined to MEM. A P1/P2 record's shape is
// itself ASSUMED (ic7300-scan-edge-record-shape), so the probe must not
// learn the length fingerprint from a record whose layout is not
// established.
