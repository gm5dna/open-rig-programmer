...
// against a fake IC-7300 written by an author who never saw core/civ/ic7300,
// core/driver/ic7300 or this plan, and who re-derived the record's length,
// offsets and vocabularies from the same two committed evidence artefacts the
// dialect was built from.
//
...
//
// STILL NOT HARDWARE. No IC-7300 has been asked anything by this project, and
// two agreeing readings of a manual are not a radio.

// e2eRecordBytes builds the 39-byte record this file seeds and expects.
//
...
func e2eRecordBytes(sel byte, hz uint64, tag string) []byte {
	...
	copy(rec[12:15], toneBCD(1230)) // ⑮–⑰ — 123.0 Hz, DIFFERENT from ⑫–⑭ so a
	//                                 span swap cannot hide behind one value
	...
	name := rec[29:39]              // ⑱–㉗
	for i := range name {
		name[i] = 0x20 // the pad, which is also a printed name byte
	}
	copy(name, tag)
	return rec
}
...
// port), then the radio.
func openFake(t *testing.T, radio *fakeic7300.Radio) driver.Session {
	t.Helper()
	sess, err := ic7300.New(ic7300.Simulated, ic7300.WithConsentedUnverifiedWrites()).
...
func civDiag(t *testing.T, sess driver.Session) ic7300.CIVDiagnostics {
	t.Helper()
...
func TestE2E_ProbeFingerprintsAgainstTheFake(t *testing.T) {
	radio := fakeic7300.New(fakeic7300.WithChannel("001", e2eRecordBytes(0x00, 14_250_000, "TEST CHAN1")))
	defer radio.Close()
	...
	if got := sess.Identity().CATID; !strings.HasPrefix(got, "94:") || len(got) <= len("94:") {
		t.Errorf("Identity().CATID = %q, want %q followed by the observed token — the 19 00 reply VALUE is undocumented and is recorded, never matched", got, "94:")
	}
...
func TestE2E_ReadAllSlots(t *testing.T) {
	seeded := map[string]struct {
		...
		"001": {0x00, 14_250_000, "TEST CHAN1"},
		"042": {0x01, 7_100_000, "FORTY TWO"},
		"099": {0x02, 28_500_000, "LAST"},
		"P1":  {0x00, 14_000_000, "EDGE LOW"},
		"P2":  {0x00, 14_350_000, "EDGE HIGH"},
	}
	var opts []fakeic7300.Option
	for slot, s := range seeded {
		opts = append(opts, fakeic7300.WithChannel(slot, e2eRecordBytes(s.sel, s.hz, s.tag)))
	}
	radio := fakeic7300.New(opts...)
	defer radio.Close()
	sess := openFake(t, radio)
...
func TestE2E_WriteOneAndReadItBack(t *testing.T) {
	...
	const newHz = 14_255_000
	const newTag = "REWRITTEN"
	...
	if len(stored) != 39 {
		t.Fatalf("the fake stored %d bytes, want 39", len(stored))
	}
...
func TestE2E_RefusesErase(t *testing.T) {
	radio := fakeic7300.New(fakeic7300.WithChannel("001", e2eRecordBytes(0x02, 14_250_000, "TEST CHAN1")))
	defer radio.Close()
	sess := openFake(t, radio)
	...
	if _, err := sess.WriteChannel(context.Background(), e2eExpected("001", 14_255_000, "REWRITTEN")); err != nil {
		t.Fatalf("WriteChannel: %v", err)
	}
	if _, err := sess.ReadChannel(context.Background(), "001"); err != nil {
		t.Fatalf("ReadChannel: %v", err)
...
func TestE2E_SurvivesAContinuousFlood(t *testing.T) {
	radio := fakeic7300.New(
		fakeic7300.WithChannel("001", e2eRecordBytes(0x00, 14_250_000, "TEST CHAN1")),
		fakeic7300.WithTransceiveBroadcasts(2*time.Millisecond),
	)
	defer radio.Close()
	sess := openFake(t, radio)
...
func TestE2E_SelectByteSurvivesAWrite(t *testing.T) {
	for _, sel := range []byte{0x00, 0x01, 0x02, 0x03} {
		radio := fakeic7300.New(fakeic7300.WithChannel("001", e2eRecordBytes(sel, 14_250_000, "TEST CHAN1")))
		sess := openFake(t, radio)
