...
var ErrUnmappedRegion = errors.New("ic7610: the slot's unmapped record regions differ from this profile's Fixed template")
...
func (s *Session) WriteChannel(ctx context.Context, ch codeplug.Channel) (driver.WriteResult, error) {
	...
	if ch.Data == nil {
		return refused(ch.Slot, []spec.Field{spec.FieldErase},
			"this radio's memory-clear forms are printed in its document but no IC-7610 has ever been asked to use one, so FieldErase carries the zero FieldSupport and consent structurally never reaches it (spec D4 \"Erase\"; matrix §3.13)")
	}
	d := *ch.Data

	a, bank, err := slotToAddress(ch.Slot)
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}}, fmt.Errorf("ic7610: WriteChannel: %w", err)
	}
	...
	if len(unwritable) > 0 {
		return refused(ch.Slot, unwritable,
			"this session cannot write these fields on bank "+string(bank)+": either no IC-7610 has ever been written to by this project and no consent was recorded, or the field is one ruling E6 leaves UNMAPPED (scan_skip, data_mode) and therefore one this driver refuses rather than collapses 4->2")
	}
	...
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}},
			fmt.Errorf("ic7610: WriteChannel %s: the pre-write preservation read: %w", ch.Slot, err)
	}
	...
	cmd, err := civic7610.Profile().BuildMemorySet(rec)
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}},
			fmt.Errorf("ic7610: WriteChannel %s: building the 1A 00 set: %w", ch.Slot, err)
	}
	...
	switch {
	case err == nil:
		// FB arrived. Register entry ic7610-1a00-set-ack, matrix lift
	...
	case errors.Is(err, transport.ErrRejected):
		...
		return driver.WriteResult{Steps: steps},
			fmt.Errorf("ic7610: WriteChannel %s: the radio rejected the %s set: %w", ch.Slot, memorySetStep, err)
	...
	default:
		...
		return driver.WriteResult{Steps: steps},
			fmt.Errorf("ic7610: WriteChannel %s: the %s set was written and never acknowledged, so its outcome is unknown: %w", ch.Slot, memorySetStep, err)
	}
}
...
func missingMandatory(d codeplug.ChannelData) (spec.Field, string) {
	...
	if len(d.Tag) > civic7610.Profile().NameLength() {
		return spec.FieldTag, fmt.Sprintf("the tag is %d characters and this radio's name span ⑱~㉗ holds %d", len(d.Tag), civic7610.Profile().NameLength())
	}
	return "", ""
}
...
//
// The template is asked for by name (civic7610.FixedTemplate) rather than
...
func unmappedRegionsDiffer(raw []byte) error {
	...
	for _, chk := range []struct {
		...
		{civic7610.SelectNibbleOffset, "high", func(b byte) byte { return b >> 4 }},
		{civic7610.SelectNibbleOffset, "low", func(b byte) byte { return b & 0x0F }},
		{civic7610.DataModeNibbleOffset, "high", func(b byte) byte { return b >> 4 }},
	} {
