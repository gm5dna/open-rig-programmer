...
// respondingPort makes the same argument). internal/fakeic7610 arrives at
...
// OBSERVED RADIO TRANSCRIBED. No IC-7610 has ever been connected to this
...
type radioImage struct {
	...
	// source-address check). Zero means 0x98.
	setFrom byte
}
...
func (p *scriptedPort) startFlood(to byte) {
	...
	go func() {
		...
		// fate one layer down.
		frame := []byte{civ.PreambleByte, civ.PreambleByte, to, 0x98, 0x19, 0x00, 0x98, civ.EndByte}
...
func memAnswerFrame(ch int, rec []byte) []byte {
	...
	out := []byte{civ.PreambleByte, civ.PreambleByte, civ.ControllerAddressDefault, 0x98, civ.CmdMemory, civ.SubMemoryContents, hi, lo}
	out = append(out, rec...)
