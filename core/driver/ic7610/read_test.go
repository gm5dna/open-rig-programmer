...
func recordAtFrequency(t *testing.T, hz uint64) []byte {
	t.Helper()
	cmd, err := civic7610.Profile().BuildMemorySet(civ.MemoryRecord{
...
// one contiguous space that core/civ/ic7610's profile declares as
...
//
// Register entries ic7610-mode-code-completeness (matrix lift R19: that
// the ten printed codes are the complete set, so 06 and 09-11 name
// nothing) and ic7610-filter-value-set (that byte ⑩ carries only 01, 02
