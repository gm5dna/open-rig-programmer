...
func slotToAddress(slot string) (civ.ChannelAddress, spec.BankID, error) {
	...
	if len(slot) != 3 {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7700: %q is not a slot on this radio: a memory is three digits (\"001\"..\"099\") and a scan edge is \"P1\" or \"P2\"", slot)
	}
	n, err := strconv.Atoi(slot)
	if err != nil {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7700: %q is not a slot on this radio: %w", slot, err)
	}
	if n < 1 || n > lastMemoryChannel {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7700: %q is outside this radio's memory range \"001\"..\"099\"", slot)
	}
...
func addressToSlot(a civ.ChannelAddress) (string, error) {
	if a.Group != 0 {
		return "", fmt.Errorf("ic7700: %s carries a group index; this radio's channel selector is a flat two-byte number", a)
	}
	switch a.Channel {
	...
	if a.Channel < 1 || a.Channel > lastMemoryChannel {
		return "", fmt.Errorf("ic7700: channel %d is outside this radio's addressable space (1..99, plus 100 and 101 for the scan edges)", a.Channel)
	}
	return fmt.Sprintf("%03d", a.Channel), nil
}

// recordIsAbsent reports whether raw is an all-0xFF record — register
// entry ic7700-all-ff-empty's reading of an unwritten channel (matrix
// §3.8(b)).
//
// IT IS A PRE-PARSE HOOK, and it must run before the record parser,
...
// §3.8(a)) is the FA reading; ic7700-all-ff-empty is this one, whose lift
// asks whether that same answer was instead a full-length record of all
// FF. Both lifts name a MEMORY channel, so neither covers P1 or P2.
//
...
func (s *Session) readRaw(ctx context.Context, a civ.ChannelAddress) (civ.MemoryRecord, []byte, bool, error) {
	...
	if err != nil {
		return civ.MemoryRecord{}, nil, false, fmt.Errorf("ic7700: building the 1A 00 read for %s: %w", a, err)
	}
	...
	if got != a { // T2, BEFORE any use of raw
		...
		return civ.MemoryRecord{}, nil, false, &AnswerMismatchError{Model: "ic7700", Requested: a, Answered: got}
	}
...
func (s *Session) ReadChannel(ctx context.Context, slot string) (codeplug.Channel, error) {
	...
	if err != nil {
		return codeplug.Channel{}, fmt.Errorf("ic7700: ReadChannel: %w", err)
	}
	rec, _, empty, err := s.readRaw(ctx, a)
	if err != nil {
		return codeplug.Channel{}, fmt.Errorf("ic7700: ReadChannel %s: %w", slot, err)
	}
	if empty {
	...
	if !ok {
		...
		return codeplug.Channel{}, fmt.Errorf("ic7700: ReadChannel %s: the record carries no frequency", slot)
	}
	...
	if !ok {
		return codeplug.Channel{}, fmt.Errorf("ic7700: ReadChannel %s: the record carries no mode", slot)
	}
	name, _ := rec.Name.Get()
	txFreq, ok := rec.TXFreqHz.Get()
	if !ok {
		// Unreachable through this profile: the layout maps idx15-19
		// unconditionally, so a successfully parsed record always
		// carries a TX-duplicate frequency.
		return codeplug.Channel{}, fmt.Errorf("ic7700: ReadChannel %s: the record carries no TX-duplicate frequency", slot)
	}
	...
	data := &codeplug.ChannelData{
		...
		// idx15-19, the TX-duplicate block's OWN frequency span — ALWAYS
		// present on the wire (matrix §1 row 4's "still necessary" even
		// with Split OFF), so ALWAYS decoded Known, never Unavailable.
		// SimplexTxEqualsRx (caps.go) states what a Split-OFF write PUTS
		// there; this read simply reports whatever the radio held.
