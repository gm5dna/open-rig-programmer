...
// New returns the IC-7700 driver built with profile.
//
// NO MODEL ENUM: this family has one member (matrix §4, "no registered
// sibling"), so which radio a driver is for is fixed by the package rather
// than by a value a caller could get wrong.
func New(profile Profile, opts ...Option) driver.Driver {
	d := &ic7700Driver{Base: driver.Base{Profile: profile}}
...
type Option func(*ic7700Driver)

// WithConsentedUnverifiedWrites records the user's consent to writes this
// project has never verified against an IC-7700.
//
...
func WithConsentedUnverifiedWrites() Option {
	return func(d *ic7700Driver) { d.Consented = true }
}
...
func WithSimulatedProfile() Option {
	return func(d *ic7700Driver) { d.Profile = Simulated }
}

// ic7700Driver implements driver.Driver for the Icom IC-7700.
type ic7700Driver struct {
	driver.Base
}

// Model implements driver.Driver. It must equal Capabilities().Model and
// the Wave-4 registry key.
func (d *ic7700Driver) Model() string { return "IC-7700" }

// Capabilities implements driver.Driver: the STATIC baseline for this
// driver's profile, before any radio has been probed.
func (d *ic7700Driver) Capabilities() spec.Capabilities {
	var caps spec.Capabilities
	switch d.Profile {
	case Simulated:
		caps = capabilitiesSimulated()
	case RealHardware:
		// This model's write-trial guard is false, so there is no hardware-verified
...
type OpenReport struct {
	...
	// matched: PDF p.204 (folio 14-4)'s command table prints the 19 00
...
func (e *RecordLengthMismatchError) Error() string {
	return fmt.Sprintf(
		"ic7700: %s answered a %d-byte memory record, want %d — the expected length is itself a derivation from one document (matrix §3.11), and this refusal names no other model because cross-model record-length distinctness is a Wave-4 tier check",
		e.Slot, e.Got, e.Want)
}
...
// error.
func (d *ic7700Driver) Open(ctx context.Context, port transport.Port, id driver.Identity) (driver.Session, error) {
	...
	// sessions.
	framing, err := civ.NewFraming(civic7700.Profile())
	if err != nil {
		_ = port.Close()
		return nil, fmt.Errorf("ic7700: framing: %w", err)
	}
	...
	if !ok {
		_ = port.Close()
		return nil, fmt.Errorf("ic7700: the CI-V framing does not report accumulator stats — this driver's diagnostics require civ.AccumulatorStatsReporter")
	}
	...
	if err != nil {
		...
		_ = port.Close()
		return nil, fmt.Errorf("ic7700: Open: %w", err)
	}
...
// one place.
func (d *ic7700Driver) open(ctx context.Context, eng *transport.Engine, stats civ.AccumulatorStatsReporter, id driver.Identity) (*Session, error) {
	p := civic7700.Profile()
	...
	if err != nil {
		return nil, fmt.Errorf("ic7700: Open: building the 19 00 read: %w", err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("ic7700: Open: 19 00 identity probe: %w", err)
	}
...
func probeSlot(ctx context.Context, eng *transport.Engine, p civ.Profile, a civ.ChannelAddress) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("ic7700: Open: building the 1A 00 read for %s: %w", a, err)
	}
	...
	if err != nil {
		return nil, false, fmt.Errorf("ic7700: Open: probing %s: %w", a, err)
	}
	got, raw, err := p.MemoryAnswerRecord(frame)
	if err != nil {
		var lengthErr *civ.RecordLengthError
		if errors.As(err, &lengthErr) {
			return nil, false, &RecordLengthMismatchError{Err: lengthErr, Got: lengthErr.Got, Want: civic7700.RecordOnlyLength, Slot: a}
		}
		return nil, false, fmt.Errorf("ic7700: Open: probing %s: %w", a, err)
	}
	if got != a {
		return nil, false, &AnswerMismatchError{Model: "ic7700", Requested: a, Answered: got}
	}
	return raw, false, nil
}

// Session is one open, probed connection to an IC-7700.
type Session struct {
...
// seams it claims.
var (
	_ driver.Driver                = (*ic7700Driver)(nil)
	_ driver.SerialFramingReporter = (*ic7700Driver)(nil)
...
//
// ASSUMED, ON NO EVIDENCE FROM THIS RADIO'S DOCUMENT. The IC-7700
// Instruction Manual says nothing about serial framing anywhere: "stop
// bit", "start bit", "parity", "data bit", "8 bit" and "flow control" have
// zero hits across the swept pages (matrix §3.1), about either physical
// CI-V path. UNLIKE EVERY OTHER MODEL IN THIS TIER, THE IC-7700 HAS NO
// USB-CDC CI-V PORT AT ALL (matrix §3.1/§3.15(3)): CI-V reaches it only
// through the [REMOTE] jack (behind an external CT-17 level converter) or
// directly through the [RS-232C] D-sub port set to its factory "CI-V"
// output mode.
//
// Register entry: ic7700-framing-8n1.
//
// THE ONE LIFT, on an IC-7700 itself: with the radio at its factory
// RS-232C-Function=CI-V setting, open [RS-232C] at 8-N-1, send
// FE FE 74 E0 19 00 FD, and confirm an address-matched answer; then repeat
// at other framings and record which the radio answers under. SCOPE: that
// capture settles which framing THAT radio's [RS-232C] port accepts and
// nothing wider — the [REMOTE]+CT-17 path is separate hardware this driver
...
// 8-N-2 either.
func (d *ic7700Driver) StopBits() int { return 1 }
