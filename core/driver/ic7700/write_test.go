...
func TestE6RefusesNonTemplateUnmappedBytes(t *testing.T) {
	for _, tc := range []struct {
		...
		{"a ★2 select-group marker", civic7700.SplitSelectByteOffset, 0x02, "low"},
		{"a Split-ON flag", civic7700.SplitSelectByteOffset, 0x10, "high"},
		{"a DATA 1 data mode", civic7700.DataModeNibbleOffset, 0x10, "high"},
		{"a non-zero TX-dup mode byte (idx20)", civic7700.TXDupUnmappedOffset, 0x01, "whole"},
		{"a non-zero TX-dup byte at the tail of the block (idx28)", civic7700.TXDupUnmappedOffset + civic7700.TXDupUnmappedLength - 1, 0x01, "whole"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			raw := make([]byte, civic7700.RecordOnlyLength)
...
// defence before a radio sees bytes — admits a set carrying a tone above
// this driver's declared 254.9 Hz ceiling, provided the digits fit the
// span (which reaches 299.9 Hz). The driver's rung 4 is what refuses it.
// FREQUENCY HAS NO SUCH GAP ON THIS MODEL: MaxFreqHz is set to the
// record's own encoding ceiling exactly (matrix §1 rows 15-16's CHOICE),
// so there is nothing above it the field could still encode. DO NOT
// DELETE THIS TEST TO "FIX" IT: the enabler that closes the tone gap
// should turn it red.
func TestNumericRefusalIsDefenceInDepthNotTheGate(t *testing.T) {
	p := civic7700.Profile()
	// 260.0 Hz (2600 deci-Hz): above the declared 254.9 Hz ceiling and
	// below the 299.9 Hz the tone span's four BCD digits can encode.
	const above = 2600
	cmd, err := p.BuildMemorySet(civ.MemoryRecord{
		Address: civ.ChannelAddress{Channel: 1}, RXFreqHz: civ.Available[uint64](14_250_000), TXFreqHz: civ.Available[uint64](14_250_000),
		Mode: civ.Available("USB"), Filter: civ.Available("FIL1"), ToneMode: civ.Available("TONE"),
		ToneTXDeciHz: civ.Available[uint64](above), ToneRXDeciHz: civ.Available[uint64](1000),
	...
	d := validChannelData()
	d.ToneTx = codeplug.ToneField{State: codeplug.Known, Value: spec.Tone(above)}
	s := consentedSession()
	if _, err := s.WriteChannel(t.Context(), codeplug.Channel{Slot: "001", Data: &d}); !errors.Is(err, ErrOutOfDomain) {
		t.Fatalf("the driver admitted a %d deci-Hz tone: %v", above, err)
	}
}
...
func TestWriteChannel_NoClearFrameIsReachable(t *testing.T) {
	...
	for _, frame := range [][]byte{
		{0xfe, 0xfe, 0x74, 0xe0, 0x1a, 0x00, 0x00, 0x01, 0xff, 0xfd},
		{0xfe, 0xfe, 0x74, 0xe0, 0x0b, 0xfd},
		{0xfe, 0xfe, 0x74, 0xe0, 0x09, 0xfd},
		{0xfe, 0xfe, 0x74, 0xe0, 0x0a, 0xfd},
	} {
