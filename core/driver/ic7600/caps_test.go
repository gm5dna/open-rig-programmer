...
// the same seven core/civ/ic7600's layout() carries spans for, in that
...
// field class by field class from an IC-7600's OWN trial evidence, AND the
...
//
// THE IC-7600 HAS NO REGISTERED SIBLING (matrix S4), so there is one
// constant and one row, not a pair.
func TestWriteTrialsComplete_PinnedFalse(t *testing.T) {
	if writeTrialsComplete {
		t.Fatal("writeTrialsComplete is true - no IC-7600 has ever answered a frame; see doc.go")
	}
}
...
func TestBaseline_Shape(t *testing.T) {
	...
	if caps.Model != "IC-7600" {
		t.Errorf("Model = %q, want %q", caps.Model, "IC-7600")
	}
	...
	// port exists.
	if caps.CATID != "7a" {
		t.Errorf("CATID = %q, want %q - the address alone, lower case to match this driver's own Identity construction; the 19 00 token is a per-session observation", caps.CATID, "7a")
	}
	...
	if caps.TagLen != civic7600.Profile().NameLength() {
		t.Errorf("TagLen %d and the codec's NameLength %d disagree", caps.TagLen, civic7600.Profile().NameLength())
	}
	if caps.TagCharset != civic7600.NameCharset {
	...
	for _, b := range caps.Banks {
		if b.NoBlank {
			t.Errorf("bank %s has NoBlank true - nothing in this document says an IC-7600 memory or scan edge must stay populated", b.ID)
		}
	...
	if len(caps.RequiredSlots) != 0 {
		t.Errorf("RequiredSlots = %v, want empty - no IC-7600 slot is documented as un-erasable", caps.RequiredSlots)
	}

	// S1 row 9 - the six rates printed for the CI-V link.
	wantBauds := []int{300, 1200, 4800, 9600, 19200}
	if !reflect.DeepEqual(caps.Bauds, wantBauds) {
		t.Errorf("Bauds = %v, want %v", caps.Bauds, wantBauds)
	}
	// OQ2 - ASSUMED, register home ic7600-default-baud, matrix lift R11.
...
// codecEnum returns the enum map core/civ/ic7600's layout carries for f.
//
...
func codecEnum(t *testing.T, f civ.FieldID) map[byte]string {
	t.Helper()
	layouts := civic7600.Profile().Layouts()
	if len(layouts) != 1 {
...
//
// caps.go writes the ten names out because core/civ/ic7600's modeEnum is
...
func TestBaseline_WrittenDownZeroes(t *testing.T) {
	zeroes := []struct {
		...
		{spec.FieldErase, "spec D4 Erase: the wire form EXISTS on this radio, in two shapes, and NO IC-7600 has ever been asked to use one; spec.ConsentUnverifiedWrites structurally never consents this field"},
...
// this radio has is writable. No IC-7600 has ever answered a frame, so a
