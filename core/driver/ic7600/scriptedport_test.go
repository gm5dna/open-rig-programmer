...
// respondingPort makes the same argument). internal/fakeic7600 arrives at
...
// OBSERVED RADIO TRANSCRIBED. No IC-7600 has ever been connected to this
...
type radioImage struct {
	...
	// source-address check). Zero means 0x7A.
	setFrom byte
}
...
func (p *scriptedPort) startFlood(to byte) {
	...
	go func() {
		...
		// fate one layer down.
		frame := []byte{civ.PreambleByte, civ.PreambleByte, to, 0x7A, 0x19, 0x00, 0x7A, civ.EndByte}
...
// restartFlood stops any running flood, waits for it to fully exit, and
// starts a fresh one addressed to `to`. Needed because floodStop/
// floodOnce are single-use: calling startFlood again after stopFlood on
// the same port would race the old goroutine's select on a channel this
// method is about to replace, which is exactly the data race a bare
// field reset from outside this file would risk. e2e_test.go's
// addressed-flood-then-quiet-then-flood-again test is the one caller.
func (p *scriptedPort) restartFlood(to byte) {
	p.stopFlood()
	p.floodDone.Wait()
	p.floodStop = make(chan struct{})
	p.floodOnce = sync.Once{}
	p.startFlood(to)
}
...
//
//	1A 00 read : FE FE 7A E0 1A 00 <ch-hi> <ch-lo> FD          — 9 bytes
//	1A 00 set  : FE FE 7A E0 1A 00 <ch-hi> <ch-lo> <25> FD     — 34 bytes
//	19 00 read : FE FE 7A E0 19 00 FD                          — 7 bytes
const (
	memReadFrameLen = 9
	memSetFrameLen  = 34
	idReadFrameLen  = 7
...
func memAnswerFrame(ch int, rec []byte) []byte {
	...
	out := []byte{civ.PreambleByte, civ.PreambleByte, civ.ControllerAddressDefault, 0x7A, civ.CmdMemory, civ.SubMemoryContents, hi, lo}
	out = append(out, rec...)
