...
func TestDriverImplementsSerialFramingReporter(t *testing.T) {
	...
	if !ok {
		t.Fatal("the IC-7600 driver does not implement driver.SerialFramingReporter - spec D3.1 requires every Icom driver to report its framing")
	}
...
//
// The IC-7600's own Instruction Manual says NOTHING about serial framing —
