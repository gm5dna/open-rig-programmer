...
func (e *UnmappedRegionError) Error() string {
	...
	switch {
	case e.Offset == civic7600.SelectByteOffset && e.Nibble == "whole":
		what = "the SELECT-group marker (0=OFF, 1=★1, 2=★2, 3=★3)"
	case e.Offset == civic7600.DataModeNibbleOffset && e.Nibble == "high":
		what = "the data mode (0=OFF, 1=DATA 1, 2=DATA 2, 3=DATA 3)"
	}
	region := e.Nibble + " nibble"
	if e.Nibble == "whole" {
		// This radio's own page draws byte (3) as one undivided enum
		// cell, not the IC-7610's two-nibble split with a stated Fixed
		// high nibble (matrix S3.15(a); core/civ/ic7600's
		// SelectByteOffset comment) - so the comparison and this message
		// are stated in terms of the WHOLE byte, not a nibble.
		region = "whole byte"
	}
	return fmt.Sprintf(
		"ic7600: this slot's record byte %d carries %#x in its %s where the profile's Fixed template carries %#x — %s. Ruling E6: a slot may be written ONLY when its unmapped regions equal the template, and anything else is REFUSED with the reason named, never rewritten. This channel cannot be written by this programme at all; it is not downgraded and not cleared",
		e.Offset, e.Got, region, e.Want, what)
}
...
func (e *OutOfDomainError) Error() string {
	return fmt.Sprintf(
		"ic7600: %s = %d is outside what this radio's record can encode (maximum %d) — refused by the driver. This is a DRIVER-level refusal, not the outbound gate: civ.FieldSpan carries no numeric domain, so the gate offers no such check (see doc.go, the deferred gate-domain gap)",
...
func (s *Session) WriteChannel(ctx context.Context, ch codeplug.Channel) (driver.WriteResult, error) {
	...
	if ch.Data == nil {
		return refused(ch.Slot, []spec.Field{spec.FieldErase},
			"this radio's memory-clear forms are printed in its document but no IC-7600 has ever been asked to use one, so FieldErase carries the zero FieldSupport and consent structurally never reaches it (spec D4 \"Erase\"; matrix §3.13)")
	}
	d := *ch.Data

	a, bank, err := slotToAddress(ch.Slot)
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}}, fmt.Errorf("ic7600: WriteChannel: %w", err)
	}
	...
	if len(unwritable) > 0 {
		return refused(ch.Slot, unwritable,
			"this session cannot write these fields on bank "+string(bank)+": either no IC-7600 has ever been written to by this project and no consent was recorded, or the field is one ruling E6 leaves UNMAPPED (scan_skip, data_mode) and therefore one this driver refuses rather than collapses 4->2")
	}
	...
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}},
			fmt.Errorf("ic7600: WriteChannel %s: the pre-write preservation read: %w", ch.Slot, err)
	}
	...
	// (register entry ic7600-full-record-mandatory, matrix lift R15: the
	...
	cmd, err := civic7600.Profile().BuildMemorySet(rec)
	if err != nil {
		return driver.WriteResult{Steps: []driver.WriteStep{}},
			fmt.Errorf("ic7600: WriteChannel %s: building the 1A 00 set: %w", ch.Slot, err)
	}
	...
	switch {
	case err == nil:
		// FB arrived. Register entry ic7600-1a00-set-ack, matrix lift
	...
	case errors.Is(err, transport.ErrRejected):
		...
		return driver.WriteResult{Steps: steps},
			fmt.Errorf("ic7600: WriteChannel %s: the radio rejected the %s set: %w", ch.Slot, memorySetStep, err)
	...
	default:
		...
		return driver.WriteResult{Steps: steps},
			fmt.Errorf("ic7600: WriteChannel %s: the %s set was written and never acknowledged, so its outcome is unknown: %w", ch.Slot, memorySetStep, err)
	}
}
...
func missingMandatory(d codeplug.ChannelData) (spec.Field, string) {
	...
	if len(d.Tag) > civic7600.Profile().NameLength() {
		return spec.FieldTag, fmt.Sprintf("the tag is %d characters and this radio's name span ⑱~㉗ holds %d", len(d.Tag), civic7600.Profile().NameLength())
	}
	return "", ""
}
...
//
// THE UNMAPPED SET: record byte 0 ENTIRELY — printed ③, this radio's own
// undivided four-valued SELECT-group marker, compared WHOLE because this
// page prints no nibble split for it (matrix S3.15(a); a GENUINE
// DIVERGENCE from the IC-7610's own two-nibble-split page, recorded in
// core/civ/ic7600's SelectByteOffset comment) — and record byte 8's HIGH
// nibble, printed ⑪'s left nibble, the four-valued data mode (no
// divergence from the IC-7610 here — matrix S2 row 20). Byte 8's LOW
// nibble is tone_mode and IS mapped.
//
// The template is asked for by name (civic7600.FixedTemplate) rather than
...
func unmappedRegionsDiffer(raw []byte) error {
	...
	if len(raw) < len(tmpl) {
		...
		return fmt.Errorf("ic7600: internal: the E6 comparison was handed a %d-byte record where the profile declares %d — the length fingerprint should have refused this answer before it reached here", len(raw), len(tmpl))
	}
	for _, chk := range []struct {
		...
		{civic7600.SelectByteOffset, "whole", func(b byte) byte { return b }},
		{civic7600.DataModeNibbleOffset, "high", func(b byte) byte { return b >> 4 }},
	} {
...
// because core/civ/ic7600's enum tables are unexported, and
