...
// New returns the IC-7600 driver built with profile.
//
...
type Option func(*ic7600Driver)

// WithConsentedUnverifiedWrites records the user's consent to writes this
// project has never verified against an IC-7600.
//
...
func WithConsentedUnverifiedWrites() Option {
	return func(d *ic7600Driver) { d.Consented = true }
}

// ic7600Driver implements driver.Driver for the Icom IC-7600.
type ic7600Driver struct {
	driver.Base
}

// Model implements driver.Driver. It must equal Capabilities().Model and
// the Wave-4 registry key.
func (d *ic7600Driver) Model() string { return "IC-7600" }

// Capabilities implements driver.Driver: the STATIC baseline for this
// driver's profile, before any radio has been probed.
func (d *ic7600Driver) Capabilities() spec.Capabilities {
	switch d.Profile {
	case Simulated:
...
// OpenReport is what Open observed while probing, kept on the ic7600
...
// error.
func (d *ic7600Driver) Open(ctx context.Context, port transport.Port, id driver.Identity) (driver.Session, error) {
	...
	// sessions.
	framing, err := civ.NewFraming(civic7600.Profile())
	if err != nil {
		_ = port.Close()
		return nil, fmt.Errorf("ic7600: framing: %w", err)
	}
	...
	if !ok {
		_ = port.Close()
		return nil, fmt.Errorf("ic7600: the CI-V framing does not report accumulator stats — this driver's diagnostics require civ.AccumulatorStatsReporter")
	}
	...
	if err != nil {
		...
		_ = port.Close()
		return nil, fmt.Errorf("ic7600: Open: %w", err)
	}
...
// one place.
func (d *ic7600Driver) open(ctx context.Context, eng *transport.Engine, stats civ.AccumulatorStatsReporter, id driver.Identity) (*Session, error) {
	p := civic7600.Profile()
	...
	if err != nil {
		return nil, fmt.Errorf("ic7600: Open: building the 19 00 read: %w", err)
	}
	...
	if err != nil {
		...
		// a wrong byte (OQ2).
		return nil, fmt.Errorf("ic7600: Open: 19 00 identity probe: %w", err)
	}
...
func probeSlot(ctx context.Context, eng *transport.Engine, p civ.Profile, a civ.ChannelAddress) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("ic7600: Open: building the 1A 00 read for %s: %w", a, err)
	}
	...
	if got != a {
		return nil, false, &AnswerMismatchError{Model: "ic7600", Requested: a, Answered: got}
	}
	return raw, false, nil
}

// Session is one open, probed connection to an IC-7600.
type Session struct {
...
// seams it claims.
var (
	_ driver.Driver                = (*ic7600Driver)(nil)
	_ driver.SerialFramingReporter = (*ic7600Driver)(nil)
...
//
// ASSUMED, ON NO EVIDENCE FROM THIS RADIO'S DOCUMENT. The IC-7600 CI-V
...
//
// LIFT R8 — Stage R capture ic7600-framing-8n1: with an IC-7600 at its
// factory CI-V settings, open its USB CI-V endpoint at 8-N-1 and then at
// 8-N-2, send FE FE 7A E0 19 00 FD at each, and record which framing
...
// 8-N-2 either.
func (d *ic7600Driver) StopBits() int { return 1 }
