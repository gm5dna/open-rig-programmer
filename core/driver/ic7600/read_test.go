...
func recordAtFrequency(t *testing.T, hz uint64) []byte {
	t.Helper()
	cmd, err := civic7600.Profile().BuildMemorySet(civ.MemoryRecord{
...
// one contiguous space that core/civ/ic7600's profile declares as
...
//
// Register entries ic7600-mode-code-completeness (matrix lift R19: that
// the ten printed codes are the complete set, so 06 and 09-11 name
// nothing) and ic7600-filter-value-set (that byte ⑩ carries only 01, 02
