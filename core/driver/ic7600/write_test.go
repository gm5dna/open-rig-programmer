...
func TestMemorySetSpec_IsWriteWithAckAndNeverRetries(t *testing.T) {
	sp := civ.CIVWriteWithAckSpec(civic7600.Profile().AcknowledgementMatcher())
...
//
// Register entry ic7600-1a00-set-ack, matrix lift R14: THE CODES ARE
...
func TestWriteChannel_FARejectionIsReported(t *testing.T) {
	t.Run("FA from 0x7A is a rejection", func(t *testing.T) {
		img := ackingRadio()
		img.ackSets = false
		img.rejectSets = true
...
func TestWriteChannel_RefusesASlotWhoseUnmappedRegionsDiffer(t *testing.T) {
	for _, tt := range []struct {
		...
		{"byte 0 = 2, SELECT ★2", withRecord(0, 0x02), civic7600.SelectByteOffset, "whole", 0x02},
		{"byte 0 = 1, SELECT ★1", withRecord(0, 0x01), civic7600.SelectByteOffset, "whole", 0x01},
		{"byte 0 = 0x10, no documented meaning but still non-zero", withRecord(0, 0x10), civic7600.SelectByteOffset, "whole", 0x10},
		{"byte 8 high = 2, DATA 2", withRecord(8, 0x21), civic7600.DataModeNibbleOffset, "high", 0x02},
		{"byte 8 high = 1, DATA 1", withRecord(8, 0x11), civic7600.DataModeNibbleOffset, "high", 0x01},
	} {
		t.Run(tt.name, func(t *testing.T) {
			img := ackingRadio()
...
func TestWriteChannel_EmptySlotWritesAgainstTheTemplate(t *testing.T) {
	...
	if rec[civic7600.SelectByteOffset] != tmpl[civic7600.SelectByteOffset] {
		t.Errorf("the set's byte %d is %#02x, want the template's %#02x", civic7600.SelectByteOffset, rec[civic7600.SelectByteOffset], tmpl[civic7600.SelectByteOffset])
	}
	if rec[civic7600.DataModeNibbleOffset]>>4 != tmpl[civic7600.DataModeNibbleOffset]>>4 {
		t.Errorf("the set's byte %d high nibble is %#x, want the template's %#x", civic7600.DataModeNibbleOffset, rec[civic7600.DataModeNibbleOffset]>>4, tmpl[civic7600.DataModeNibbleOffset]>>4)
	}
}
...
func TestWriteChannel_ConsentedWritesTheFullRecord(t *testing.T) {
	...
	want := append([]byte{0xFE, 0xFE, 0x7A, 0xE0, 0x1A, 0x00, 0x00, 0x42}, goldenRecord...)
...
func TestWriteChannel_NoClearFrameIsReachable(t *testing.T) {
	// (a) FE FE 7A E0 1A 00 <ch-hi> <ch-lo> FF FD, and
	// (b) FE FE 7A E0 0B FD.
...
func TestNumericRefusalIsDefenceInDepthNotTheGate(t *testing.T) {
	...
	frame := append([]byte{0xFE, 0xFE, 0x7A, 0xE0, 0x1A, 0x00, 0x00, 0x42}, overRange...)
	frame = append(frame, 0xFD)

	if !civic7600.Profile().AllowedCommand(frame) {
...
// Register entry ic7600-default-tone-undocumented.
func TestWriteChannel_CreateWithoutToneIsRefused(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(refusal.Reason, "ic7600-default-tone-undocumented") {
...
func TestUnreachableRefusalsTellTheTruth(t *testing.T) {
	...
	t.Run("the tone refusal names the situation it is in", func(t *testing.T) {
		...
		if !strings.Contains(create, "no prior record") || !strings.Contains(create, "ic7600-default-tone-undocumented") {
