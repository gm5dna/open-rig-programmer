...
// contiguous space with three printed forms — and core/civ/ic7600's
...
func slotToAddress(slot string) (civ.ChannelAddress, spec.BankID, error) {
	...
	if len(slot) != 3 {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7600: %q is not a slot on this radio: a memory is three digits (\"001\"..\"099\") and a scan edge is \"P1\" or \"P2\"", slot)
	}
	n, err := strconv.Atoi(slot)
	if err != nil {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7600: %q is not a slot on this radio: %w", slot, err)
	}
	if n < 1 || n > lastMemoryChannel {
		return civ.ChannelAddress{}, "", fmt.Errorf("ic7600: %q is outside this radio's memory range \"001\"..\"099\"", slot)
	}
...
func addressToSlot(a civ.ChannelAddress) (string, error) {
	if a.Group != 0 {
		return "", fmt.Errorf("ic7600: %s carries a group index; this radio's channel selector is a flat two-byte number", a)
	}
	switch a.Channel {
	...
	if a.Channel < 1 || a.Channel > lastMemoryChannel {
		return "", fmt.Errorf("ic7600: channel %d is outside this radio's addressable space (1..99, plus 100 and 101 for the scan edges)", a.Channel)
	}
...
func (s *Session) readRaw(ctx context.Context, a civ.ChannelAddress) (civ.MemoryRecord, []byte, bool, error) {
	...
	if err != nil {
		return civ.MemoryRecord{}, nil, false, fmt.Errorf("ic7600: building the 1A 00 read for %s: %w", a, err)
	}
	...
	if got != a { // T2, BEFORE any use of raw
		...
		return civ.MemoryRecord{}, nil, false, &AnswerMismatchError{Model: "ic7600", Requested: a, Answered: got}
	}
...
func (s *Session) ReadChannel(ctx context.Context, slot string) (codeplug.Channel, error) {
	...
	if err != nil {
		return codeplug.Channel{}, fmt.Errorf("ic7600: ReadChannel: %w", err)
	}
	rec, _, empty, err := s.readRaw(ctx, a)
	if err != nil {
		return codeplug.Channel{}, fmt.Errorf("ic7600: ReadChannel %s: %w", slot, err)
	}
	if empty {
	...
	if !ok {
		...
		return codeplug.Channel{}, fmt.Errorf("ic7600: ReadChannel %s: the record carries no frequency", slot)
	}
	...
	if !ok {
		return codeplug.Channel{}, fmt.Errorf("ic7600: ReadChannel %s: the record carries no mode", slot)
	}
	name, _ := rec.Name.Get()
