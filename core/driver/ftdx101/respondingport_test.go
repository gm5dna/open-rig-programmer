...
//
// It is the M9d-2 plan's D7 seam — the M9c-6 command-parsing responder,
// PARAMETERISED BY CAT ID so that ONE helper serves both models — and tasks
// 3 (write) and 4 (settings) reuse it. It exists because a fixed-transcript
// stub CANNOT test this driver at all: every session begins with Open's
// full choreography — the AI0 init, the ID probe, and then ~100 discovery
// probes across the whole declared 5xx range plus EMG — so a test that
// wants to exercise one read has to answer a hundred frames first, in
// whatever order the driver chooses to send them. Scripting that by hand
...
// foreign one, an out-of-vocabulary kind byte, an answer naming the wrong
...
type slotImage struct {
	...
	//
	// The FTdx10's equivalent helper defaults an empty catID to its one
	// radio's. THIS package has two, so a default would silently pick a
	// side: an MP test that forgot to say so would open an MP driver
	// against a D-answering radio and fail as a wrong-radio refusal several
	// screens from the omission. openSession fills this in from the model
	// being opened, which is the only place that knows which radio the test
	// means; a test scripting a DIFFERENT radio (the wrong-sibling cases)
	// sets it explicitly and openSession leaves it alone.
	catID string
	...
	// for a read of it. Task 4's surface; nil serves "?;" for every
	// address.
...
func newRespondingPort(t *testing.T, img slotImage) *respondingPort {
	t.Helper()
	if img.catID == "" {
		t.Fatal("slotImage.catID is empty — this package drives TWO models, so a scripted radio must say which one it is (openSession fills it from the model being opened; a test scripting the other model, or a foreign radio, sets it explicitly)")
	}
...
// Frame lengths this helper matches on, from the FTDX101MP/FTDX101D CAT
// manual's own position charts (rev 2308-L): the MT read is "MT" + a 3-byte
// slot + ';' (MT's frame block at layout 1311-1345, and core/cat's
// mtReadLen), the EX read is "EX" + a 6-digit address + ';' (EX's frames at
// layout 699-708, core/cat's exReadLen). Written out here because a TEST
// fixture that derived its frame shapes from the code under test would
// answer whatever that code asked for, including a wrong shape.
const (
	mtReadFrameLen = 6
	exReadFrameLen = 9
)
...
// by field, so a test states what the radio says rather than what a builder
...
// frame assembles the 41-byte combined MT answer BY POSITION, from THIS
// radio's own MT chart (rev 2308-L, layout 1311-1330, printed pages 16-17;
// the same 41 positions counted independently off 300 dpi raster renders in
// core/cat/ftdx101/testdata/geometry-witness.csv): "MT" + the 28-position
// shared field block + P11 + a 12-byte tag field + ';'.
//
...
// there it would be a length the driver believed instead of one the dialect
// derived.
func (f mtAnswerFields) frame() string {
	...
	b[22] = f.kind
	b[23] = f.ctcss
	copy(b[24:26], "00") // P9, positions 25-26, documented fixed "00" (layout 1326)
	b[26] = f.shift
	p11 := f.p11
	if p11 == 0 {
		p11 = '0'
...
// kind '1' Memory of the combined record's {'0','1'} read pair, a clarifier
// magnitude that is a multiple of the dialect's ASSUMED 10 Hz step).
//
...
// malformedAnswer is a 41-byte MT answer for slot whose P7 kind byte is
// '2' — inside core/cat's structural read vocabulary but OUTSIDE the
// combined record's own documented {'0' VFO, '1' Memory} pair (MT's P7
// legend, layout 1324) — so the dialect's parser refuses it with a
// *cat.ParseError naming P7.
//
// A wrong-vocabulary byte rather than a truncated frame, deliberately: a
// short frame would never match the transport's ExpectLen and would fail as
// a TIMEOUT, which tests the transport rather than the driver's refusal.
// This one arrives, matches, and is refused by the parser — which is the
// path a real radio answering something this protocol does not define would
// take.
func malformedAnswer(slot string) string {
	return mtAnswerFields{
		slot: slot, freq: "014250000",
...
// wrongSlotAnswer is a well-formed 41-byte MT answer that names answered
// while being served for a read of some other slot: the stale-reply shape
// the transport's quarantine discipline is meant to prevent, checked anyway
// because mapping an answer onto the wrong channel would corrupt a codeplug
// silently.
func wrongSlotAnswer(answered string) string {
	return mtAnswerFields{
		slot: answered, freq: "014250000",
		clarSign: '+', clarMag: "0000", rxClar: '0', txClar: '0',
