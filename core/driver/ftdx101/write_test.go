...
// values are arbitrary and never reach a wire — this driver's capability map
// has no entry for any of the seventeen, so the gate refuses them all, and the
// vocabulary-keyed ones are refused a rung earlier still — but the STATE is
...
// the pre-erratum judge let it straight through, on both models.
//
// CTCSSTONE IS THE PROOF FIELD, the reviewer's own reproduction: this
// radio's 41-byte combined Set has no CTCSS-tone position at all
...
func TestRequestedFields_MembershipAndOrder(t *testing.T) {
	...
	// carried the identical ten-entry gap. It covers BOTH models by
	// construction: tierRequestedFields is model-independent, as its own
	// doc comment says.
	//
...
//
// BOTH MODELS, for the reason TestWriteChannel_KnownTierFieldRefusedBeforeWire
// gives: the gate is the capability profile's and the profiles are per model.
