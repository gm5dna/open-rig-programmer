...
func readOnlyFields(base spec.Capabilities) map[spec.Field]spec.FieldSupport {
	...
	// effectiveCapabilities, which is reached only from Open (ftdx101.go,
	// with d.Capabilities()) and from SynthesiseDiscoveredBanks (which
	// re-passes d.Capabilities() on purpose, so live and offline synthesis
	// cannot drift). TestBaseline_Shape asserts Bank(spec.BankMemory)
	// succeeds on both profiles of both models.
	//
...
func cloneCapabilities(caps spec.Capabilities) spec.Capabilities {
	...
	for _, b := range caps.Banks {
		...
		// running it over both profiles of both models. A zero Bank
		// reaching out would be quiet rather than loud: no Slots, no
		// Fields, so a bank the app cannot show and, FieldSupport's zero
		// being Unsupported, one nothing may be written to.
		cp, _ := caps.Bank(b.ID)
