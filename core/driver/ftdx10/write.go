...
func (s *Session) WriteChannel(ctx context.Context, ch codeplug.Channel) (driver.WriteResult, error) {
	...
	// a value) is refused, not interpreted. TagDisplay is included for
	// exactly the same reason as its neighbours and for no other: an
	...
	// and its comment quotes the pre-fix transcript (the 41-byte Set sent
	// unchanged, TxFreqHz simply absent from it).
	//
