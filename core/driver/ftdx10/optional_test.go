...
// app then renders no discovered banks for an offline FTdx10 codeplug — no
...
var (
	_ driver.Driver                    = (*ftdx10Driver)(nil)
	_ driver.Session                   = (*Session)(nil)
	_ driver.DiscoveredBankSynthesizer = (*ftdx10Driver)(nil)
...
// hardware-confirmed, partly still assumed. No FTdx10 inventory has ever
// been observed, so there is no vocabulary this driver could report
// honestly, and borrowing the FT-710's would mean answering a question
// about one radio with another radio's evidence. Callers already tolerate
// absence (the capability is reached by a two-result type assertion), so
// codeplug.RadioInfo.Region simply stays unset for an FTdx10.
//
// A Stage R session that enumerates a real 5xx bank is what would make a
// region vocabulary possible — at which point this test is deleted with
// the same commit that adds the method, and the evidence goes in the commit
// message. It is NOT a gap to be filled meanwhile, which is exactly why the
// absence is asserted rather than merely mentioned.
func TestSession_DoesNotReportRegion(t *testing.T) {
	var sess driver.Session = &Session{}
	if rr, ok := sess.(driver.RegionReporter); ok {
		t.Errorf("*ftdx10.Session implements driver.RegionReporter (Region() = %q) — this driver has no honest region vocabulary; see this test's doc comment and doc.go before adding one", rr.Region())
	}
}
...
// and slots a static bank already claims.
//
...
// ftdx10_test.go's discovery test — the one place the input is data neither
// test invented.
func TestSynthesiseDiscoveredBanks_MatchesLiveDiscovery(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		},
		{
			...
			// bank: "600" is three digits but outside every declared range,
		...
		},
		{
			...
			// or PMS ("P1L"-"P9U") wire form classifies as neither 60m nor
			// EMG, so it would be omitted even with the exclusion deleted
			// (verified by deleting it: this case still passed). The case
			// therefore pins the OUTCOME the contract promises, not the
			// mechanism. The explicit exclusion earns its keep against a
			// future static bank that claimed a 5xx or EMG wire form — a
			// shape this driver's capability data cannot currently produce,
			// which is why no test here can exercise it — where its
			// absence would put one slot in two banks and fail
...
// for a compatibility reason of its own (the pre-M9a hand-rolled app
// synthesis it replaced did so); that is its history, not a rule this
// driver inherits. Pinned so it stays a decision rather than an accident of
// whichever loop shape was written first.
