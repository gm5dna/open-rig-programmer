...
//
// SIMPLER than every sibling driver's helper of the same name: this radio
// has no MT command, no EX settings surface and no 60m/EMG discovery, so
// the only frames this package ever builds are AI (Init), ID (the
// handshake) and MR/MW (read.go, write.go).
