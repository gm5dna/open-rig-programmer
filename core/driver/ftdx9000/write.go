...
// NO MT COMMAND (matrix §2; dialect.go), so — unlike every 4-rig
// MT-family sibling, whose write is one combined MT frame or an
// MW-then-MT pair — this write is a single plain MW
// (yaesu.MRWriteChannel/BuildMWCommand, not yaesu.WriteChannel/
// BuildWriteCommand, which build only the combined MT form).
...
// call site) so a future write_test.go could exercise it directly — the
// same shape every migrated sibling driver's write.go keeps.
