...
// respondingPort is this package's scripted radio — see
// core/driver/ftdx3000's own identical helper for the full reasoning.
...
type slotImage struct {
	// catID defaults to this dialect's own CAT ID ("0582") when empty —
	// a test that wants the OTHER accepted ID ("0583") sets it explicitly.
	catID string
...
type mrAnswerFields struct {
	...
	kind     byte
	ctcss    byte
	tone     string // always "00" on this radio — P9 is fixed both ways
	shift    byte
}
...
// 14.250 MHz USB memory channel, no clarifier, CTCSS off, P9 fixed "00",
// simplex.
