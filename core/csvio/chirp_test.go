...
func ftdx3000LikeCapabilities() spec.Capabilities {
	...
	if err != nil {
		panic(fmt.Sprintf("chirp_test: wiring.StaticCapabilities(%q): %v", wiring.FTdx3000Model, err))
	}
	return caps
}
...
// ft450dLikeCapabilities is the FT-450D's own REGISTERED capabilities
// (v1.8.0 Yaesu trio, third and last row).
func ft450dLikeCapabilities() spec.Capabilities {
	caps, err := wiring.StaticCapabilities(wiring.FT450DModel)
	if err != nil {
		panic(fmt.Sprintf("chirp_test: wiring.StaticCapabilities(%q): %v", wiring.FT450DModel, err))
	}
	return caps
}

// ft890LikeCapabilities is the FT-890's own REGISTERED capabilities
// (v1.9.0 binary-CAT four, first row).
func ft890LikeCapabilities() spec.Capabilities {
	caps, err := wiring.StaticCapabilities(wiring.FT890Model)
	if err != nil {
		panic(fmt.Sprintf("chirp_test: wiring.StaticCapabilities(%q): %v", wiring.FT890Model, err))
	}
	return caps
}

// ft900LikeCapabilities is the FT-900's own REGISTERED capabilities
// (v1.9.0 binary-CAT four, second row).
func ft900LikeCapabilities() spec.Capabilities {
	caps, err := wiring.StaticCapabilities(wiring.FT900Model)
	if err != nil {
		panic(fmt.Sprintf("chirp_test: wiring.StaticCapabilities(%q): %v", wiring.FT900Model, err))
	}
	return caps
}
...
// ftx1LikeCapabilities is the FTX-1's own REGISTERED capabilities
// (v1.10.0).
func ftx1LikeCapabilities() spec.Capabilities {
	caps, err := wiring.StaticCapabilities(wiring.FTX1Model)
	if err != nil {
		panic(fmt.Sprintf("chirp_test: wiring.StaticCapabilities(%q): %v", wiring.FTX1Model, err))
	}
	return caps
}
