...
// TestProfileShape pins the values THE ONE TABLE of the IC-7800 plan names,
...
func TestProfileShape(t *testing.T) {
	...
	if got, want := p.Model(), "IC-7800"; got != want {
		t.Errorf("Model() = %q, want %q", got, want)
	}
	if got, want := p.RadioAddress(), byte(0x6A); got != want {
	...
	if ic7800.RecordOnlyLength != 25 || ic7800.DataAreaLength != 27 || ic7800.AddressBytes != 2 {
		t.Errorf("length constants are %d/%d/%d, want 25 record-only, 27 data-area, 2 address bytes",
			ic7800.RecordOnlyLength, ic7800.DataAreaLength, ic7800.AddressBytes)
	}
	if ic7800.RecordOnlyLength+ic7800.AddressBytes != ic7800.DataAreaLength {
...
func TestLayoutOffsetsFollowTheStatedRule(t *testing.T) {
	...
	layouts := ic7800.Profile().Layouts()
	if len(layouts) != 1 {
...
func TestUnmappedRegionsAreTheTwoE6Nibbles(t *testing.T) {
	fixed := ic7800.FixedTemplate()
	if len(fixed) != ic7800.RecordOnlyLength {
		t.Fatalf("FixedTemplate() is %d bytes, want %d - the template describes the whole record or none of it",
			len(fixed), ic7800.RecordOnlyLength)
	}
	for i, b := range fixed {
		if b != 0x00 {
	...
	if ic7800.SelectNibbleOffset != 0 || ic7800.DataModeNibbleOffset != 8 {
		t.Errorf("the E6-unmapped offsets are %d and %d, want 0 (printed (3)) and 8 (printed (11) high nibble)",
			ic7800.SelectNibbleOffset, ic7800.DataModeNibbleOffset)
	}

	// No span may claim either unmapped region.
	for _, sp := range ic7800.Profile().Layouts()[0].Fields {
		if sp.Offset == ic7800.SelectNibbleOffset {
			t.Errorf("%s claims record byte 0, which E6 rules unmapped on this model", sp.Field)
		}
		if sp.Offset == ic7800.DataModeNibbleOffset && sp.Nibble != civ.NibbleHigh {
			t.Errorf("%s claims record byte 8 other than its HIGH nibble; only tone_mode (high) is mapped there - UNLIKE the IC-7610 family, whose equivalent byte maps tone_mode in the LOW nibble (matrix §1b)", sp.Field)
		}
	}
}
...
func TestChannelWireForms(t *testing.T) {
	...
	tests := []struct {
		...
		{"memory 01", 1, []byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00, 0x00, 0x01, 0xFD}},
		{"memory 99", 99, []byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00, 0x00, 0x99, 0xFD}},
		{"scan edge P1", 100, []byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00, 0x01, 0x00, 0xFD}},
		{"scan edge P2", 101, []byte{0xFE, 0xFE, 0x6A, 0xE0, 0x1A, 0x00, 0x01, 0x01, 0xFD}},
	}
...
func TestNameCharsetIsTheTwoPrintedTablesPlusSpace(t *testing.T) {
	got := map[byte]bool{}
	for i := 0; i < len(ic7800.NameCharset); i++ {
		b := ic7800.NameCharset[i]
		if got[b] {
	...
	if !got[';'] {
		t.Error("charset omits ';', which PDF p.209's symbol table prints - CI-V has no ';' terminator and the Yaesu default rule must not be inherited here")
	}
}
...
func TestConformance(t *testing.T) { civtest.Run(t, ic7800.Profile()) }
