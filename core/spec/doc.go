...
//
// The central piece of that design is the Support four-state
// (Unsupported/Unverified/Supported/Inert — Inert added at M5b for a
// field whose value a fixed-layout write frame must always transmit but
// the radio silently ignores, see Support's own doc comment) and
// FieldSupport.CanWrite: this project hard-gates all writes to real
// hardware behind hardware verification sessions, and CanWrite()
// returning true only for a field whose Write support is Supported is
// the mechanism that enforces that gate — Inert, like Unverified and
...
// shown in a UI), but it can never authorise a write.
//
