...
A minor release: the programme can now register a radio whose memory write route has no channel-name field at all, using a new NoTag capability; six Icom models and twelve Kenwood and Yaesu models join the supported tier, all manual-derived and never bench-connected, writes switched off until you switch them on.

- **NoTag: a radio without a channel name isn't forced through name-field handling it doesn't have.** A profile can now declare that its memory write route carries no channel-name field, rather than being made to fit the tag-field machinery every other radio uses. On such a radio the desktop grid hides the Tag column, since there is nothing there to show or edit. A CHIRP import onto a NoTag radio drops the file's `Name` column and prints one warning per import instead of refusing the file, and the programme's own native CSV export writes an empty `Name` column so the file keeps the same shape as every other radio's export. Validation rejects a profile that claims NoTag but whose bank still lists a tag field, so a profile can never say two contradictory things about its own channel names.
- **Six Icom models join the supported tier, all opt-in and manual-derived.** IC-7800 (CI-V 6Ah) and IC-7600 (CI-V 7Ah) are HIGH-proximity clones of the IC-7610's record shape at their own addresses. IC-7410 (CI-V 80h) is not a clone: a 40-byte record with a genuine TX-duplicate block, whose write mirrors the receive frequency into an unset transmit frequency rather than refusing. IC-7700 (CI-V 74h) reads and writes the transmit (split) frequency on the already-registered IC-7300's 39-byte record shape. IC-9100 (CI-V 7Ch) is one band, richer than its siblings (duplex, offset, DTCS code and polarity). IC-7200 (CI-V 76h) is NoTag — no channel-name field over CI-V at all, and no tone or scan-skip field of any kind, on a 17-byte record. None of the six has ever answered a frame from this program; every one stays refused for writing until you switch it on for that model.
- **Twelve Kenwood and Yaesu models join the supported tier, all opt-in and manual-derived.** TS-2000, TS-2000X and TS-B2000 share one 50-byte record and are the only package here with a channel name (eight characters); a channel is written back only after being read first, so this driver cannot create a new one. TS-570D, TS-570S and TS-570DG share a 28-byte record with no channel name; the DG row is UNVERIFIED-BY-INHERITANCE, its own values inferred from its siblings rather than read from a DG-specific manual page. TS-870S has its own 22-byte record, again with no channel name. FT-2000, FT-2000D, FTdx5000, FTdx9000 and FT-950 share a 27-byte record family, none of them with a channel name, and none of their Control Command Lists documents a memory-tag write command at all; FTdx9000 is also sold under the "FT-9000" name, which its own manual never actually prints, so this program treats it as descriptive text only. None of the twelve has ever answered a frame from this program; every one stays refused for writing until you switch it on for that model.
- Every Icom write now also runs the field-safety check every other driver already ran, refusing rather than silently dropping a value its own radio never read.
- Thirty-nine models are now registered in total, up from twenty-one. Nothing changes for any existing radio: byte identity is verified across every captured artefact (1,063 in total); the only movers are the bare-usage banner, `help`, an unrecognised `--model` and an unrecognised subcommand, plus the TS-2000/TS-870S capture legs moving from an unrecognised-model error to a real reading now that both are registered.
