...
- **TS-2000, TS-2000X and B2000** gain a Satellite Memory bank (`SA`/`SI`, 10 channels): a name plus three per-channel flags — band swap, trace and trace-reverse. The record carries no frequency; satellite operation reads that live off `FA`/`FB` instead. The radio answers `SA` only for the currently selected channel, so a whole-radio read skips this bank and a single-channel read of the selected slot works. Unverified, consent-gated, on the same terms as the ordinary memory bank.
