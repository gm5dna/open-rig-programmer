...
<!-- TASK 18 GATE: the bullet below is v1.4.1's byte-identity claim and is
     ...
     479 artefacts on each side: 476 identical by hash, and the three that
     moved are model-list surfaces that gain exactly two rows, TS-890S and
     TS-990S. A fourth model-list surface, the "unknown model" refusal,
     moves the same way and by the same amount. Say that, name the two
     rows, and do not write "identical" without the exception. Then delete
     this comment: the marker is cleared before tagging. -->
