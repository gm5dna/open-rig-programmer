...
- **Linux**: on Debian, Ubuntu or Mint, `sudo apt install ./<file>.deb`; on Fedora or openSUSE, `sudo dnf install ./<file>.rpm`
  (built but untested on a real Fedora install); on any other distribution with Flatpak, `flatpak install --user ./<file>.flatpak`
  (amd64 and aarch64 bundles are both published). Elsewhere, the tarball is a single static binary.
